...
//-------------------------------Get Method------------------------------
  ...
  User=req.getParameter("txIdUser2");
  Hour=req.getParameter("txTime2");
	Day=req.getParameter("txDay2");
	Date=req.getParameter("txDate2");
	Month=req.getParameter("txMonth2");
	Year=req.getParameter("txYear2");
  BrowserName=req.getParameter("txBrowser2");
	Platform=req.getParameter("txPlatform2");
	ScreenWidth=req.getParameter("txScreenW2");
	ScreenHeight=req.getParameter("txScreenH2");
	Referer=req.getParameter("txReferer2");
  OpSys=req.getParameter("txOs2");
  Ver=req.getParameter("txVersion2");
  ...
  if ((!(adminlogin.equals("admin")))||(!(passwdlogin.equals("admin")))) {
	...
	try {
      ...
      &&(Ver.equals(""))) {
        ...
        if (!(User.equals(""))) SEARCH += "IdUser = '"+ User +"', ";
        if (!(Hour.equals(""))) SEARCH += "Time = '"+ Hour +"', ";
        if (!(Day.equals(""))) SEARCH += "Day = '"+ Day +"', ";
        if (!(Date.equals(""))) SEARCH += "Date = '"+ Date +"', ";
        if (!(Month.equals(""))) SEARCH += "Month = '"+ Month +"', ";
        if (!(Year.equals(""))) SEARCH += "Year = '"+ Year +"', ";
        if (!(BrowserName.equals(""))) SEARCH += "Browser = '"+ BrowserName +"', ";
        if (!(Platform.equals(""))) SEARCH += "Platform = '"+ Platform +"', ";
        if (!(ScreenWidth.equals(""))) SEARCH += "ScreenWidth = '"+ ScreenWidth +"', ";
        if (!(ScreenHeight.equals(""))) SEARCH += "ScreenHeight = '"+ ScreenHeight +"', ";
        if (!(Referer.equals(""))) SEARCH += "Referer = '"+ Referer +"', ";
        if (!(OpSys.equals(""))) SEARCH += "OS = '"+ OpSys +"', ";
        if (!(Ver.equals(""))) SEARCH += "Version = '"+ Ver +"', ";
...
///////////////////////////////////////////////////
   ...
   // Create a Statement
        ...
        out.println("<br><h4>Update complete in "+elapsed+ " mS</i></h4>");
        out.println("</html>");
      } else {
