...
dept_id =  mscsPage.RequestString("dept_id")
	...
	area = mscsPage.RequestNumber("area", "", 1, 20000)
    if IsNull(area) then
        errorList.Add "area must be between 1 and 20000"
    end if
	body = mscsPage.RequestNumber("body", "", 1, 20000)
    if IsNull(body) then
        errorList.Add "body must be between 1 and 20000"
    end if
	...
	description = mscsPage.RequestString("description", "" ,0 ,100)
    if IsNull(description) then
        errorList.Add "description must be less than or equal to 100 characters"
    end if
...
'        errorList.Add "list_price must be between 1 and 255 characters"
	...
	name_2= image_first
	name_3= image_second
'	Response.Write name_1
'	Response.Write name_2
...
<TR>
	<TD ROWSPAN="3" >	<FONT SIZE="-1">Put file name</TD>
	<TD WIDTH="300"><%Response.Write name_1%></TD>
</TR>
<TR><TD><%Response.Write name_2%></TD></TR>
<TR><TD><%Response.Write name_3%></TD></TR>
<TR><TD COLSPAN="2"><BR>Place file in <%= Server.MapPath("/" & mscsPage.SiteRoot) %>\assets\product_images  or   &nbsp &nbsp </FONT>
...
<%
		...
		rsExe("area").Value = mscsPage.RequestNumber("area", "", 1, 20000)
		rsExe("body").Value = mscsPage.RequestNumber("body", "", 1, 20000)
...
<A HREF="Upload_Product.asp?image_file=<%=name_1%>&image_first=<%=name_2%>&image_second=<%=name_3%>"> <FONT FACE="Arial, sans-serif" COLOR="red" STYLE="{font-family: Arial, sans-serif; color: red; font-weight: bold; font-size: 14pt}">Upload flie</FONT> </A> 
</TD></TR>
</TABLE>
