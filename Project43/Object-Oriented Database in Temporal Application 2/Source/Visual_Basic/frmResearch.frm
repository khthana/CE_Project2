...
Public Sub LoadResearch()
    ...
    lvEObj.ColumnHeaders.Add , , "Research", 2400, lvwColumnLeft
...
Private Sub cmdAdd_Click()
    ...
    frmNewTraining.lvTraining.ListIndex = 1
End Sub
...
Private Sub cmdDelete_Click()
    ...
    If (cboResearch <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Research", "Name", no, cboResearch.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboResearch <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Research", "Name", no, cboResearch.Text, FromDate, ToDate)
        'MsgBox st
        frmResearch.LoadResearch
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboResearch <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RResearchs", no, FromDate) Then
            Per.RAppend "Research", "Name", no, cboResearch.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboResearch <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Research", "Research", no, Indicator, cboResearch.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RResearchs.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RResearchs.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboResearch, lvEObj.SelectedItem.Text
    Kong = False
