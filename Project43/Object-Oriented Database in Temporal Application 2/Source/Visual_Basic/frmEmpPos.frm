...
Public Sub LoadEmpPos()
    ...
    lvEObj.ColumnHeaders.Add , , "Position Number", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboEmpPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "EmpPos", "PosNo", no, cboEmpPos.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboEmpPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("EmpPos", "PosNo", no, cboEmpPos.Text, FromDate, ToDate)
        'MsgBox st
        frmEmpPos.LoadEmpPos
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboEmpPos <> "") And (FromDate <> "") Then
        If Per.CheckFrom("REmpPoss", no, FromDate) Then
            Per.RAppend "EmpPos", "PosNo", no, cboEmpPos.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboEmpPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("EmpPos", "PosNo", no, Indicator, cboEmpPos.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.REmpPoss.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.REmpPoss.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboEmpPos, lvEObj.SelectedItem.Text
    Kong = False
