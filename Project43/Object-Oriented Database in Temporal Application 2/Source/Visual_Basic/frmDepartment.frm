...
Public Sub LoadDepartment()
    ...
    lvEObj.ColumnHeaders.Add , , "Department", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboDepartment <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Department", "DeptName", no, cboDepartment.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboDepartment <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Department", "DeptName", no, cboDepartment.Text, FromDate, ToDate)
        'MsgBox st
        frmDepartment.LoadDepartment
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboDepartment <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RDepartments", no, FromDate) Then
            Per.RAppend "Department", "DeptName", no, cboDepartment.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboDepartment <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Department", "DeptName", no, Indicator, cboDepartment.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RDepartments.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RDepartments.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboDepartment, lvEObj.SelectedItem.Text
    Kong = False
