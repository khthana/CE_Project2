...
Begin VB.Form frmWorkStatus 
   Caption         =   "Work Status History"
   ClientHeight    =   4665
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6570
   ...
   End
End
Attribute VB_Name = "frmWorkStatus"
...
Public Sub LoadWorkStatus()
    ...
    lvEObj.ColumnHeaders.Add , , "WorkStatus", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboWorkStatus <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "WorkStatus", "WorkStatus", no, cboWorkStatus.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboWorkStatus <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("WorkStatus", "WorkStatus", no, cboWorkStatus.Text, FromDate, ToDate)
        'MsgBox st
        frmWorkStatus.LoadWorkStatus
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboWorkStatus <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RWorkStatuss", no, FromDate) Then
            Per.RAppend "WorkStatus", "WorkStatus", no, cboWorkStatus.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboWorkStatus <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("WorkStatus", "WorkStatus", no, Indicator, cboWorkStatus.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RWorkStatuss.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RWorkStatuss.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboWorkStatus, lvEObj.SelectedItem.Text
    Kong = False
