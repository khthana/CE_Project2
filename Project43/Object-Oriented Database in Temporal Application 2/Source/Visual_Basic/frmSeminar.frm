...
Begin VB.Form frmSeminar 
   Caption         =   "Seminar History"
   ClientHeight    =   4680
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6585
...
Public Sub LoadSeminar()
    ...
    lvEObj.ColumnHeaders.Add , , "Seminar", 2400, lvwColumnLeft
...
Private Sub cmdAdd_Click()
    ...
    frmNewTraining.lvTraining.ListIndex = 2
End Sub
...
Private Sub cmdDelete_Click()
    ...
    If (cboSeminar <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Seminar", "Name", no, cboSeminar.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboSeminar <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Seminar", "Name", no, cboSeminar.Text, FromDate, ToDate)
        'MsgBox st
        frmSeminar.LoadSeminar
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboSeminar <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RSeminars", no, FromDate) Then
            Per.RAppend "Seminar", "Name", no, cboSeminar.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboSeminar <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Seminar", "Seminar", no, Indicator, cboSeminar.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RSeminars.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RSeminars.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboSeminar, lvEObj.SelectedItem.Text
    Kong = False
