...
Begin VB.Form frmMngPos 
   Caption         =   "Management Position History"
   ClientHeight    =   4635
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6600
...
Public Sub LoadMngPos()
    ...
    lvEObj.ColumnHeaders.Add , , "Management Position", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboMngPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "MngPos", "MngPos", no, cboMngPos.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboMngPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("MngPos", "MngPos", no, cboMngPos.Text, FromDate, ToDate)
        'MsgBox st
        frmMngPos.LoadMngPos
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboMngPos <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RMngPoss", no, FromDate) Then
            Per.RAppend "MngPos", "MngPos", no, cboMngPos.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboMngPos <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("MngPos", "MngPos", no, Indicator, cboMngPos.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RMngPoss.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RMngPoss.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboMngPos, lvEObj.SelectedItem.Text
    Kong = False
