...
Begin VB.Form frmLeaveType 
   Caption         =   "ประวัติการลา"
   ClientHeight    =   4680
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6600
   ...
   End
End
Attribute VB_Name = "frmLeaveType"
...
Public Sub LoadLeaveType()
    ...
    lvEObj.ColumnHeaders.Add , , "LeaveType", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboLeaveType <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "LeaveType", "LeaveType", no, cboLeaveType.Text, FromDate, ToDate
        Per.Delete "LeaveDay", "LeaveDay", no, txtLeaveday.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboLeaveType <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("LeaveType", "LeaveType", no, cboLeaveType.Text, FromDate, ToDate)
        st = Per.Insert("LeaveDay", "LeaveDay", no, txtLeaveday.Text, FromDate, ToDate)
        'MsgBox st
        frmLeaveType.LoadLeaveType
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboLeaveType <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RLeaveTypes", no, FromDate) Then
            Per.RAppend "LeaveType", "LeaveType", no, cboLeaveType.Text, FromDate
            Per.Append "LeaveDay", "LeaveDay", no, txtLeaveday.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboLeaveType <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("LeaveType", "LeaveType", no, Indicator, cboLeaveType.Text, FromDate, ToDate)
        st = Per.Update("LeaveDay", "LeaveDay", no, Indicator, txtLeaveday.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RLeaveTypes.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RLeaveTypes.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboLeaveType, lvEObj.SelectedItem.Text
    Kong = False
    txtLeaveday = Per.LeaveDay.GetAt(Indicator).LeaveDay
