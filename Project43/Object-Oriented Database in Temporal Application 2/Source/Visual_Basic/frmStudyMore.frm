...
Begin VB.Form frmStudyMore 
   Caption         =   "การลาศึกษาต่อ"
   ClientHeight    =   5655
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6570
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   2880
      Width           =   5295
      Begin VB.TextBox txtScholarDetail 
         Height          =   315
         Left            =   960
         TabIndex        =   33
         Top             =   1440
      ...
      Begin VB.TextBox txtMajor 
         ...
         Width           =   2055
      End
      Begin VB.ComboBox cboEducateLevel 
         Height          =   330
         Left            =   960
   ...
   End
End
Attribute VB_Name = "frmStudyMore"
...
Public Sub LoadStudyMore()
    ...
    i = 1
    Set ehist = Per.StudyMore.GetAt(1)
    Do While Not ehist Is Nothing
        Set item = lvEObj.ListItems.Add
        st = ehist.EducateLevel + " สาขาวิชา" + ehist.Major + " สถาบัน" + ehist.Institute + " " + ehist.Country + " ระยะเวลา " + ehist.LongYear + " ทุน" + ehist.ScholarDetail
        item.Text = st
        item.SubItems(1) = ehist.FromDate
        item.SubItems(2) = ehist.ToDate
        i = i + 1
        Set ehist = Per.StudyMore.GetAt(i)
    Loop
    clearEdit
...
Private Sub cmdDelete_Click()
    ...
    If (cboEducateLevel <> "") And (txtMajor <> "") And (txtIns <> "") _
    And (txtCountry <> "") And (txtLongYear <> "") And (txtScholarDetail <> "") _
    And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.StudyMoreDelete no, FromDate, ToDate, cboEducateLevel.Text, txtMajor.Text, txtIns.Text, txtCountry.Text, txtLongYear.Text, txtScholarDetail.Text
        LoadStudyMore
        Exit Sub
    Else
...
Private Sub cmdInsert_Click()
    ...
    And (txtCountry <> "") And (txtLongYear <> "") And (txtScholarDetail <> "") _
    And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.StudyMoreInsert(no, FromDate, ToDate, cboEducateLevel.Text, txtMajor.Text, txtIns.Text, txtCountry.Text, txtLongYear.Text, txtScholarDetail.Text)
        'MsgBox st
        LoadStudyMore
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboEducateLevel <> "") And (txtMajor <> "") And (txtIns <> "") _
    And (txtCountry <> "") And (txtLongYear <> "") And (txtScholarDetail <> "") _
    And (FromDate <> "") And (ToDate <> "") Then
        If Per.CheckFrom("StudyMore", no, FromDate) Then
            Per.StudyMoreAppend no, FromDate, ToDate, cboEducateLevel.Text, txtMajor.Text, txtIns.Text, txtCountry.Text, txtLongYear.Text, txtScholarDetail.Text
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    And (txtCountry <> "") And (txtLongYear <> "") And (txtScholarDetail <> "") _
    And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.StudyMoreUpdate(no, Indicator, FromDate, ToDate, cboEducateLevel.Text, txtMajor.Text, txtIns.Text, txtCountry.Text, txtLongYear.Text, txtScholarDetail.Text)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.StudyMore.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.StudyMore.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboEducateLevel, Per.StudyMore.GetAt(Indicator).EducateLevel
    Kong = False
    txtMajor.Text = Per.StudyMore.GetAt(Indicator).Major
    txtIns.Text = Per.StudyMore.GetAt(Indicator).Institute
    txtCountry.Text = Per.StudyMore.GetAt(Indicator).Country
    txtLongYear.Text = Per.StudyMore.GetAt(Indicator).LongYear
    txtScholarDetail.Text = Per.StudyMore.GetAt(Indicator).ScholarDetail
