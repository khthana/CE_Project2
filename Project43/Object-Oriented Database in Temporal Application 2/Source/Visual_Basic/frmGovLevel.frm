...
Public Sub LoadGovLevel()
    ...
    lvEObj.ColumnHeaders.Add , , "ระดับ", 700, lvwColumnLeft
    lvEObj.ColumnHeaders.Add , , "ขั้นเงินเดือน", 1050, lvwColumnLeft
    lvEObj.ColumnHeaders.Add , , "อัตราเงินเดือน", 1250, lvwColumnLeft
    ...
    i = 1
    Set ehist = Per.RGovLevels.GetAt(1)
    Do While Not ehist Is Nothing
        Set item = lvEObj.ListItems.Add
        item.Text = ehist.GovLevel.GovLevel
        item.SubItems(1) = ehist.GovLevel.SalStep
        item.SubItems(2) = ehist.GovLevel.SalRate
        item.SubItems(3) = ehist.FromDate
        item.SubItems(4) = ehist.ToDate
        i = i + 1
        Set ehist = Per.RGovLevels.GetAt(i)
    Loop
    clearEdit
End Sub
...
Private Sub cmdDelete_Click()
    ...
    If (cboGovLevel <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.GovLevelDelete "GovLevel", "GovLevel", no, cboGovLevel.Text, cboSalStep.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboGovLevel <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.GovLevelInsert("GovLevel", "GovLevel", no, cboGovLevel.Text, cboSalStep.Text, FromDate, ToDate)
        'MsgBox st
        frmGovLevel.LoadGovLevel
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboGovLevel <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RGovLevels", no, FromDate) Then
            Per.GovLevelAppend no, cboGovLevel.Text, cboSalStep.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboGovLevel <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.GovLevelUpdate("GovLevel", "GovLevel", no, Indicator, cboGovLevel.Text, cboSalStep.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RGovLevels.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RGovLevels.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboGovLevel, lvEObj.SelectedItem.Text
    Kong = False
    findCombo cboSalStep, lvEObj.SelectedItem.SubItems(1)
    Kong = False
