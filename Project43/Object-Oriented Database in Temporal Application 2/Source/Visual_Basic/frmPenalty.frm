...
Public Sub LoadPenalty()
    ...
    lvEObj.ColumnHeaders.Add , , "Penalty", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboPenalty <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Penalty", "Penalty", no, cboPenalty.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboPenalty <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Penalty", "Penalty", no, cboPenalty.Text, FromDate, ToDate)
        'MsgBox st
        frmPenalty.LoadPenalty
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboPenalty <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RPenaltys", no, FromDate) Then
            Per.RAppend "Penalty", "Penalty", no, cboPenalty.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboPenalty <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Penalty", "Penalty", no, Indicator, cboPenalty.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RPenaltys.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RPenaltys.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboPenalty, lvEObj.SelectedItem.Text
    Kong = False
