...
Begin VB.Form frmDecorate 
   Caption         =   "Decorate History"
   ClientHeight    =   4665
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6585
...
Public Sub LoadDecorate()
    ...
    lvEObj.ColumnHeaders.Add , , "เครื่องราชอิสริยาภรณ์", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboDecorate <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Decorate", "DecName", no, cboDecorate.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboDecorate <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Decorate", "DecName", no, cboDecorate.Text, FromDate, ToDate)
        'MsgBox st
        frmDecorate.LoadDecorate
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboDecorate <> "") And (FromDate <> "") Then
        If Per.CheckFrom("RDecorates", no, FromDate) Then
            Per.RAppend "Decorate", "DecName", no, cboDecorate.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboDecorate <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Decorate", "DecName", no, Indicator, cboDecorate.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.RDecorates.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.RDecorates.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboDecorate, lvEObj.SelectedItem.Text
    Kong = False
