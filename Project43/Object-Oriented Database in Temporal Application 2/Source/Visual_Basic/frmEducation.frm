...
Public Sub LoadEducation()
    ...
    lvEObj.ColumnHeaders.Add , , "Education", 2400, lvwColumnLeft
...
Private Sub cmdDelete_Click()
    ...
    If (cboEducation <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        Per.RDelete "Education", "Education", no, cboEducation.Text, FromDate, ToDate
...
Private Sub cmdInsert_Click()
    ...
    Dim st As Integer
    If (cboEducation <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RInsert("Education", "Education", no, cboEducation.Text, FromDate, ToDate)
        'MsgBox st
        frmEducation.LoadEducation
        Exit Sub
    Else
...
Private Sub cmdNew_Click()
    ...
    If (cboEducation <> "") And (FromDate <> "") Then
        If Per.CheckFrom("REducations", no, FromDate) Then
            Per.RAppend "Education", "Education", no, cboEducation.Text, FromDate
            clearEdit
        Else
...
Private Sub cmdUpdate_Click()
    ...
    Dim st As String
    If (cboEducation <> "") And (FromDate <> "") And (ToDate <> "") And (FromLessThanTo = True) Then
        st = Per.RUpdate("Education", "Education", no, Indicator, cboEducation.Text, FromDate, ToDate)
...
Private Sub lvEObj_Click()
    ...
    FromArray = Split(Per.REducations.GetAt(Indicator).FromDate, "/", -1, 1)
    ToArray = Split(Per.REducations.GetAt(Indicator).ToDate, "/", -1, 1)
    
    findCombo cboEducation, lvEObj.SelectedItem.Text
    Kong = False
