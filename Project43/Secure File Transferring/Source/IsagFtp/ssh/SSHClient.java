  ...
  public void bootSSH(boolean haveCnxWatch) throws IOException {
    try {
	  ...
	  interactor.connected(this);	 
       if (debug)
       {  console.print("[command] Recieved and Calculation session Key ...");
       }
	  receiveServerData();
      initiatePlugins();	 
	...
	throw new IOException("Sorry, server does not support the '" +
      ...
      {  console.println("complete.");
	     console.println("[command] Send Session Key succuss.");
	     console.println("SSH Tunnel have been created.\n");
      }
      // !!!
...
*/
	  ...
	  while ( true )
	  {
	     try
	     {
		  if (debug)
		  { 
		    console.println("[command] Create Local Port : "+localForwardPort); 
		  }
  		   addLocalPortForward(localForwardPort,serverAddrStr,21,"ftp");
		  break;
	     }
	     catch (IOException e)
	     {  
	      ...
	      console.println("-> " + msg);
		  localForwardPort++;
		  console.println("Switch to port : "+localForwardPort);
  ...
  void negotiateVersion() throws IOException {
    ...
    if(srvVersionMajor > 1) {
      throw new IOException("IsagFTP do not support SSHv2 yet, enable SSHv1 compatibility in server");
    ...
    if (debug)
    {    console.print("[command] Recieve String version: " + srvVersionStr);  }
	// Strip white-space
    ...
    verStr += "\n";
	if (debug)
	{ console.print("[command] Send String version: "+verStr); }
    buf    = verStr.getBytes();
    sshOut.write(buf);
    sshOut.flush();
  }
  ...
  public void requestLocalPortForward(String localHost, int localPort, String remoteHost, int remotePort, String plugin)
      throws IOException {
