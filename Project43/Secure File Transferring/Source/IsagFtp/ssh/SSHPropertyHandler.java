...
import IsagFtp.util.EncryptedProperties;


public final class SSHPropertyHandler implements SSHClientUser, SSHAuthenticator {
...
/*	if(SSH.NETSCAPE_SECURITY_MODEL) {
	...
	*/
	...
	try {
	    ...
	    if(!sshDir.exists()) {
		if(interactor.askConfirmation("IsagFtp home directory: '" + sshHomeDir +
...
//	    }
	...
	if(savePasswords) {
	    ...
	    props.save(f, "IsagFTP ssh settings",
...
*/
	...
	if(host == null || host.equals("")) {
		...
		host = currentAlias;
	    do {
		host = interactor.promptLine("Enter SSH Server host : ", host);
		host = host.trim();
	    } while ("".equals(host));
          if(false) {
	 //   if(autoLoadProps) {
