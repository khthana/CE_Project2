// IsagSign.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CIsagSignApp, CWinApp)
	//{{AFX_MSG_MAP(CIsagSignApp)
...
/////////////////////////////////////////////////////////////////////////////
// CIsagSignApp construction

CIsagSignApp::CIsagSignApp()
{	m_user = "";
	m_pic = NULL;
	m_pDataOb = NULL;
}
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CIsagSignApp object

CIsagSignApp theApp;

static const CLSID clsid =
{ 0x8a5f6383, 0xf56b, 0x11d4, { 0xb0, 0x29, 0x0, 0x50, 0xbf, 0xb, 0xfe, 0xf1 } };

/////////////////////////////////////////////////////////////////////////////
// CIsagSignApp initialization

BOOL CIsagSignApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CIsagSignDoc),
		RUNTIME_CLASS(CMainFrame),       // main SDI frame window
		RUNTIME_CLASS(CIsagSignView));
	...
	if (!ProcessShellCommand(cmdInfo))
		return FALSE;
	m_pMainWnd->SetWindowText("ISAG SIGN");
...
// ************************* Init *******************************
void CIsagSignApp::InitRegistry()
...
//	SetSecurityDescriptorOwner(&sd, , TRUE);
	RegCreateKeyEx(HKEY_LOCAL_MACHINE, "Software\\IsagSign",
		0, "IsagSign", REG_OPTION_NON_VOLATILE, KEY_READ|KEY_WRITE,
		NULL, &mainhk, &todo);
	RegQueryInfoKey(mainhk, NULL, NULL, NULL, &signum, NULL,
		NULL, NULL, NULL, NULL, NULL, NULL);
	if (signum == 0)
	{	addUser("IsagSign", "", 1, 2, "");
		addUserKey("TEMP", "", 2);
	}
...
bool CIsagSignApp::addUser(CString name, CString surname, BYTE algid, BYTE sigsize, CString picfile)
{	HKEY mainhk = NULL, hk = NULL;
	CString kname;
	bool succ = false;
	CWaitDlg waitdlg;
	...
	if (surname.GetLength() != 0) kname = name+" "+surname;
	else kname = name;
	...
	{	if (addUserKey(name, surname, sigsize))
		{	setRegVal(kname, "Alg", REG_BINARY, &algid, 1);
			...
			{	setRegVal(kname, "Picfile", REG_SZ, (BYTE *)(picfile.GetBuffer(0)), picfile.GetLength());
				if (m_isagsig.SignFile("IsagSign", picfile, false))
...
bool CIsagSignApp::addUserKey(CString name, CString surname, BYTE sigsize)
{   //HKEY_CURRENT_USER\SOFTWARE\Microsoft\Cryptography\UserKeys\ong
	//HKEY_LOCAL_MACHINE\Software\Microsoft\Protected Storage System Provider\ong
	//HKEY_USERS\.DEFAULT\SOFTWARE\Microsoft\Cryptography\UserKeys\ong
	CString kname;
	HCRYPTPROV hProv;
	HCRYPTKEY hKey;
	...
	{	// Create Signature Key Pair
		...
		{	DWORD myflag = CRYPT_USER_PROTECTED;
			...
			if (kname == "TEMP" || kname == "IsagSign") myflag = 0;
...
bool CIsagSignApp::deleteUser(CString name)
{	HKEY mainhk = NULL, hk = NULL;
	DWORD todo;
	...
	else if (RegOpenKeyEx(mainhk, name, 0, NULL, &hk) == ERROR_SUCCESS)
...
bool CIsagSignApp::deleteUserKey(CString name)
{   //HKEY_CURRENT_USER\SOFTWARE\Microsoft\Cryptography\UserKeys\ong
	//HKEY_LOCAL_MACHINE\Software\Microsoft\Protected Storage System Provider\ong
	//HKEY_USERS\.DEFAULT\SOFTWARE\Microsoft\Cryptography\UserKeys\ong
	HCRYPTPROV hProv;
...
bool CIsagSignApp::LoadPicFile(CString user)
{	HKEY mainhk = NULL, hk = NULL;
	...
	{	size = 2048;
		...
		{	BYTE *alg;
			...
			alg = theApp.getRegVal("IsagSign", "Alg", REG_BINARY, &alglen);
			m_isagsig.SetSign(fsig, size, *alg);
			if (m_isagsig.VerifySig("IsagSign", fname, false) == TRUE)
...
HKEY CIsagSignApp::hasUser(CString name)
{	HKEY mainhk = NULL, hk = NULL;
	bool succ = false;
		
	RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\IsagSign",
		0, NULL, &mainhk);
	if (mainhk != NULL)
...
bool CIsagSignApp::createReg(CString name)
{	HKEY mainhk = NULL, hk = NULL;
	DWORD todo, result;
	bool succ = false;
	...
	{	// Not found this name
		...
		result = RegCreateKeyEx(mainhk, name, 0, "", REG_OPTION_NON_VOLATILE, KEY_READ|KEY_WRITE,
					NULL, &hk, &todo);
		if (result == ERROR_SUCCESS) succ = true;
	}
	RegCloseKey(mainhk);
	RegCloseKey(hk);
	return succ;
...
bool CIsagSignApp::setRegVal(CString name, LPTSTR valname, DWORD type,
...
BYTE* CIsagSignApp::getRegVal(CString name, CString valname, DWORD type,
						   DWORD *pdatalen)
{	HKEY mainhk = NULL, hk = NULL;
	BYTE *pdata = NULL;
	DWORD mtype, result;
...
BYTE* CIsagSignApp::getUserKey(CString name, DWORD *plen)
{	HCRYPTPROV hProv;
	HCRYPTKEY hKey;
	BYTE *buff;
...
// This function didn't work because import public key
// only work with context that import key itself.
// For this to work fine this must return 
//both hKey and hProv
HCRYPTKEY CIsagSignApp::putUserKey(BYTE* buff, DWORD len)
{	HCRYPTPROV hProv;
	HCRYPTKEY hKey = NULL;
	
	if(!CryptAcquireContext(&hProv, "TEMP", NULL, PROV_RSA_FULL, 0))
	{	AfxMessageBox("Error during:\r\n\tAcquire Context IsagSign Key Container");
		exit(1);
	}
	if(!CryptImportKey(hProv, buff, len, 0, 0, &hKey))
	{	AfxMessageBox("Error during:\r\n\tImport public key");
		exit(1);
	}
	return hKey;
