...
<title>View Computer Details-<<ดูข้อมูลเครื่อง>></title>
...
%>
<font class = blackfont>ข้อมูลเครื่อง</font><font class=bluefont> | </font>
...
<% Do While not RS.EOF %>	
	<div id=<% =RS("ID") %> style="position:absolute; left:<% =RS("XPosition") %>px; top:<% =RS("YPosition") %>px; width:30px; height:30px; z-index:1"><img src=<% =Select_Status(RS("Status")) %> width="30" height="30" onMouseOver="MM_showHideLayers('T<% =RS("ID") %>','','show')" onMouseOut="MM_showHideLayers('T<% =RS("ID") %>','','hide')"></div> 		
<% 
RS.MoveNext 
Loop
RS.Close
...
Function Select_S(ab) 'Select Menu On/Off
		...
		if ab=2 then  Select_S="Login"   'Boring		
End Function
</Script>
