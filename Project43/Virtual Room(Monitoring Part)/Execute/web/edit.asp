<Html>
<Head>
<Title>Edit Computer Details-<<แก้ไขข้อมูลเครื่อง>></Title>
...
<% Do While not RS.EOF %>	
	<div id=<% =RS("ID") %> style="position:absolute; left:<% =RS("XPosition") %>px; top:<% =RS("YPosition") %>px; width:30px; height:30px; z-index:1"><a href=editpage.asp?thisid=<% =RS("ID")%>><img src=<% =Select_Status(RS("Status")) %> width="30" height="30" onMouseOver="MM_showHideLayers('T<% =RS("ID") %>','','show')" onMouseOut="MM_showHideLayers('T<% =RS("ID") %>','','hide')"></a></div> 		
<% 
RS.MoveNext 
Loop
RS.Close
