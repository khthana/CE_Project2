...
fpm(V,Ts,weight(WA,WBs),Ts,weight(WA,WBs),[],[]) :-
	WA=<V,!.
fpm(V,true(A,Bs),weight(WA,WBs),M,WM,[true(A,R1s)],[weight(WA1,WR1s)]) :-
	fpm(V,Bs,WBs,M,WM,R1s,WR1s),
	WM=weight(WM1,WMs),
	WA1 is WA-WM1.
fpm(V,[],[],[],weight(0,[]),[],[]).
...
fp([true(A,_)],[weight(1,_)],incorrect(A)) :- !.
fp(T,[weight(WA,WBs)],X) :-
	V is int((WA+1)/2),
	fpm(V,T,[weight(WA,WBs)],M,WM,R,WR),
	M=true(A,Bs),
	query(program,oracle,forall,A,YN),
	(YN=no->fp([M],[WM],X);fp(R,WR,X)).
