...
if($appoint_again=="นัดหมายอีกครั้ง")
{ 
	 header("Location:app_add_again.php?app_id=$app_id&friend_id=$friend_id&apr_status=$apr_status");
}else
{
?>
...
if($appoint_ok=="ตกลง")
{
		echo "ทำการจัดการนัดหมายลงในตารางเวลาของคุณเรียบร้อยแล้ว <br><br>";
		$apr_status="OD";
		...
		$result_find=mysql_db_query("magazine",$sql_findsch);
		$numrow=@mysql_num_rows($result_find);
		if($numrow<1)
		{
//ใส่การนัดหมายลงในตารางเวลา ต่อเมื่อยังไม่เคยใส่ลงไปเลย
			$sql_selectapp="select app_time,app_date,app_title,app_note,app_duration,app_remind_before,app_remind_before_hour,app_remind_mail from appointment where app_id=$app_id and mem_id=$user_id";
...
}elseif($del_appoint=="ลบการนัดหมาย")
{
		if($apr_status=="PS")
		{
			...
			$timenow=date("H:i:s");
			$mess_id=9;
			$mess_title="$sendername ได้ทำการลบการนัดหมายที่เลยเวลาแล้ว";
...
//ทำการลบการนัดหมาย
		...
		$result=mysql_db_query("magazine",$sql_selectapp);
		$numrow=@mysql_num_rows($result);
		if($numrow>0)
		{
			echo "ลบการนัดหมายไปยังบุคคลนี้เรียบรอ้ยแล้ว แต่ <br>";
			echo " ระบบยังไม่สามารถลบการนัดหมายนี้ได้ทั้งหมดเนี่องจากมีผู้ถูกนัดคนอื่นอีก<br><br>";
		}else
		{
			$sql_del="delete from appointment where app_id=$app_id and mem_id=$user_id ";
			mysql_db_query("magazine",$sql_del);
			echo "การนัดหมายนี้ถูกลบเรียบร้อยแล้ว<br><br>";
		}
}
