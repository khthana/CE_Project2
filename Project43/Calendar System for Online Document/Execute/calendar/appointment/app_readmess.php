...
<? //-------------หมดquickbutton------- ?>
          ...
          <TD width="79%"> 
      <form method="post" action="app_replymess.php">
<?
$host="localhost";
...
mysql_select_db("magazine") or die("ไม่สามารถติดต่อแฟ้มข้อมูลได้"); 
		...
		$sql_job="select mem_id,app_title,app_note,app_duration,app_remind_before,app_remind_before_hour,app_remind_mail from appointment where app_id=$app_id";
...
$app_note=$row[2];
$app_duration=$row[3];
$app_remind_before=$row[4];
$app_remind_before_hour=$row[5];
$app_remind_mail=$row[6];
$app_time=$time.":00น.";
$sql_sender="select mem_fname,mem_lname,mem_nickname from member where mem_id=$sender_id";
$result=mysql_db_query("magazine",$sql_sender);
$row=mysql_fetch_array($result);
$send_fname=$row[0];
$send_lname=$row[1];
$send_nickname=$row[2];
...
?>
              <table width="90%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"><font size="1" face="MS Sans Serif, Microsoft Sans Serif"><b><font color="#666666">จาก 
                    : </font></b><font  face="MS Sans Serif, Microsoft Sans Serif" size="2" color="#0033CC">&nbsp; 
                    <? echo $send_fname," ",$send_lname," ( ",$send_nickname," )"; ?></font></font></td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"><font size="1"><b><font face="MS Sans Serif, Microsoft Sans Serif" color="#666666">ถึง 
                    : </font></b><font face="MS Sans Serif, Microsoft Sans Serif" color="#0033CC">&nbsp; 
                    <?
$sql_receiver="select mem_fname,mem_nickname from appoint_receiver a1,member m1 where mem_id=friend_id and app_id=$app_id order by mem_fname";
$result=mysql_db_query("magazine",$sql_receiver);
$numrow=@mysql_num_rows($result);
for($i=0;$i<$numrow;$i++)
{
		$row=mysql_fetch_array($result);
...
?> </font></font></td>
                ...
                <tr> 
                  ...
                  <td colspan="2"><font color="#666666"><b><font size="1" face="MS Sans Serif, Microsoft Sans Serif">วันนัดหมาย 
                    ...
                    <? echo $appdate," ",$thaimonth[$appmonth-1]," ",$appyear+543; ?> 
                    <b><font color="#666666" size="1">&nbsp;&nbsp;เวลา : </font></b><font color="#0000FF">&nbsp;<? echo $app_time; ?></font></font></font></td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                ...
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#666666"><b> 
                    กำหนดให้มีการเตือนการนัดก่อน : </b><font color="#0033CC" size="2">&nbsp; 
                    <? echo $app_remind_before; ?></font></font> <b><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#666666">วัน</font></b> 
                    <font face="MS Sans Serif, Microsoft Sans Serif" size="1" color="#666666"><font color="#0033CC" size="2">&nbsp;&nbsp; 
                    <? echo $app_remind_before_hour; ?></font><b> ชม.</b></font> 
                  </td>
                </tr>
                <tr>
...
?> 
                  <td colspan="2" height="1"><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#666666"><b>เตือนงานผ่านอีเมล์ 
                    : </b><font color="#0033CC" size="2">&nbsp; <? echo $tmp_remind_mail; ?></font></font></td>
                </tr>
                <tr> 
                  <td width="10%" height="1">&nbsp;</td>
                  <td colspan="2" height="1"><img src="../images/orange_point.gif" width="400" height="1"></td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"><b><font color="#666666" size="1" face="MS Sans Serif, Microsoft Sans Serif">ความคิดเห็นตอบกลับ 
                    : </font></b><font color="#666666" size="1" face="MS Sans Serif, Microsoft Sans Serif"><font color="#FF6600">(จะส่งให้ผู้นัดเมื่อมีการยอม/ไม่ยอมรับการนัดหมายเท่านั้น)</font></font></td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"> 
                    <textarea name="comment" wrap="PHYSICAL" rows="3" cols="60"></textarea>
                  </td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"> 
                    <input type="hidden" name="sender" value="<? echo $send_fname." ".$send_lname." (".$send_nickname.")"; ?>">
                    <input type="hidden" name="app_id" value="<? echo $app_id; ?>">
                    <input type="hidden" name="sender_id" value="<? echo $sender_id; ?>">
                  </td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
                  <td colspan="2"> 
                    <input type="submit" name="accept" value="ยอมรับการนัด">
                    <input type="submit" name="notacc" value="ไม่ยอมรับการนัด">
                    <input type="submit" name="postpone" value="ขอคิดดูก่อน">
                  </td>
                </tr>
              </table>
            </form>
