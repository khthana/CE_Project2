...
$sql_allapp="select count(*) from appointment where mem_id=$user_id";
...
{
		$num_app=0;
} // เช็คว่ามีappoint ของผู้ใช้ ทั้งหมดเท่าไร
//$first =ตัวแรกของลิสต์ที่จะselect
//$indexpage= หน้านี้เป็นหน้าที่เท่าไร
if ($first=="")
{ 
	$first=0;
	$indexpage=1;
	$lastpage= ceil($num_app/15);
	$num_lastpage=$num_app % 15;
}
...
?>
		...
		{
			$first_index=($j-1)*15; //เริ่ม ตัวแรกของแต่ละหน้า
			echo "<a href=app_list.php?first=$first_index&indexpage=$j&num_lastpage=$num_lastpage&lastpage=$lastpage><font color=#CC33FF>"."หน้าที่".$j."</font></a>";
			echo "&nbsp;  ";				
		}
?> </font></td>
              </tr>
              <tr> 
                ...
                <td width="95%"> 
                    ...
                    <table width="95%" border="1" cellspacing="0" cellpadding="0">
                      <tr bgcolor="#CC99FF"> 
                        <td height="30" width="3%"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2"></font></b></font></td>
                        <td height="30" width="17%"> 
                          <div align="center"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2">สถานะ</font></b></font></div>
                        </td>
                        <td height="30" width="15%"> 
                          <div align="center"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2">วันที่นัดหมาย</font></b></font></div>
                        </td>
                        <td height="30" width="10%"> 
                          <div align="center"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2">เวลา</font></b></font></div>
                        </td>
                        <td height="30" width="50%"> 
                          <div align="center"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2">เรื่อง</font></b></font></div>
                        </td>
                        <td height="30" width="5%"> 
                          <div align="center"><font color="#660066"><b><font face="MS Sans Serif, Microsoft Sans Serif" size="2">ระยะเวลา</font></b></font></div>
                        </td>
                      </tr>
                      <?
$sql_applist="select app_date,app_time,app_title,app_duration,app_id from appointment where mem_id=$user_id order by app_date desc,app_time desc limit $first,$numlist";
$result=mysql_db_query("magazine",$sql_applist);
$numrow=@mysql_num_rows($result);

for ($i=0;$i<$numrow;$i++)
{
		$row=mysql_fetch_array($result);
		$apptime= (int) substr($row[1],0,2);
		$apptime2=$apptime.":00";

		$app_date=(int) substr($row[0],8,2);
		$app_month=(int) substr($row[0],5,2);
		$app_year= (int) substr($row[0],0,4)+543;
		...
		// เช็คว่า เป็น OD รึยังถ้าเป็น คือ เรียบร้อยแล้ว
		$sql_chkapp="select apr_status from appoint_receiver  where app_id=$app_id and apr_status<>'OD'";
		$resultchk=mysql_db_query("magazine",$sql_chkapp);
		$numrow_chk=@mysql_num_rows($resultchk);
		if ($numrow_chk>0)
		{	$status="<font color=#FF0000>ยังไม่เรียบร้อย</font>";
			$trcolor="";
		}else
		{
			$status="<font color=#000000>เรียบร้อยแล้ว</font>";
			$trcolor="bgcolor=#EEEEEE";
		}
...
?> 
                      <tr <? echo $trcolor; ?>> 
                        <td> <? 			
		if ($status=="<font color=#000000>เรียบร้อยแล้ว</font>")
		{
?> 
                          <input type="checkbox" name="<? echo "appdel[".$row[4]."]" ?>" value="yes">
                          <?
		} //if $status
?> </td>
                        <td>&nbsp;<font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#9900CC"><? echo $status; ?></font></td>
                        <td>&nbsp;<font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#9900CC"><? echo $appdate2; ?></font></td>
                        <td>&nbsp;<font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#9900CC"><? echo $apptime2; ?></font></td>
                        <td>&nbsp;<a href="app_edit.php?app_id=<? echo $app_id; ?>&frompage=app_list"><font face="MS Sans Serif, Microsoft Sans Serif" size="1" color="#0000FF"><? echo $app_title; ?></font></a></td>
                        <td>&nbsp;<font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#9900CC"><? echo $app_duration."วัน"; ?></font></td>
                      </tr>
                      <?
} //for
?> 
              ...
              <tr>
                <td width="5%" height="35">&nbsp;</td>
                  <td width="95%" height="35"> <font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#000000">เรียบร้อยแล้ว</font><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#660066"> 
                    <font color="#666666">= การนัดหมายนั้นมีการตอบรับเรียบร้อยหมดแล้ว 
                    และ ทำการจัดตารางให้ผู้ใช้แล้ว กรณีนี้<font color="#FF0000">สามารถทำการลบข้อมูลการนัดหมายได้</font></font><br>
                    <font color="#FF0000">ยังไม่เรียบร้อย</font><font color="#666666">= 
                    การนัดหมายยังมีการตอบรับไม่ครบหรือยังไม่มีการตอบกลับใดๆจากผู้รับ 
                    ซึ่งในกรณีนี้จะ<font color="#FF0000">ไม่อนุญาติให้ลบการนัดหมายนี้</font></font><font color="#FF0000"> 
                    </font></font></td>
              </tr>
              <tr> 
                <td width="5%">&nbsp;</td>
                <td width="95%">&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="submit" name="Submit" value="ลบการนัดหมาย">&nbsp;&nbsp;
                    <a href="../welcome.php"><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#0000FF">กลับไปหน้าแรก</font></a> 
                  </td>
              </tr>
            </table>
                  </form>
