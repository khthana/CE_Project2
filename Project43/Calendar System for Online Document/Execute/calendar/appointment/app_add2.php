...
//------------------------------------------
function sendmail($list_receiver,$mail_date,$mail_time,$title,$mem_id)
{
	...
	$num=count($list_receiver); //เริ่มที่1 เพราะมีเลขรองอาร์เรย์
	for($i=1;$i<$num;$i++)
	{
		$sql_email="select mem_username,mem_fname,mem_lname,mem_email from member where mem_id=$list_receiver[$i] ";
...
if ($edit_butt=="กลับไปแก้ไข")
{
		...
		header("Location:../appointment/app_add1.php?time=$time&schday=$schday&schdate=$schdate&schmonth=$schmonth&schyear=$schyear&edit_butt=Y");
...
$apptime2=$time.":00:00";
	 $sqlinsert="insert into appointment(mem_id,app_date,app_time, app_title,app_note,app_duration,app_remind_before,app_remind_before_hour,app_remind_mail)
	 values('$user_id','$appdate2','$apptime2','$app_title','$app_note','$app_duration','$app_remind_before','$app_remind_before_hour','$tmp_remind_mail')";
...
if($grp_num<>"all") //ลิสต์รายชื่อเพื่อนทั้งหมด
{
...
sendmail($listfriend,$maildate,$mailtime,$app_title,$user_id);
...
for($i=0;$i<$numrow;$i++)
{
	   $row=mysql_fetch_array($result);		
		for($j=1;$j<$numfriend_chk;$j++)
		{
			...
			{	 $sqlinsert_friend="insert into appoint_receiver(app_id,sender_id,friend_id,apr_status) values($app_id,$user_id,$row[0],'NE')";
					mysql_db_query("magazine",$sqlinsert_friend);				
			}
		}
} //for $numrow
...
<? //-------------หมดquickbutton------- ?>
          ...
          <TD width="79%"> 
      <form method="post" action="app_add2.php?time=<? echo $time; ?>&schday=<? echo $schday; ?>&schdate=<? echo $schdate; ?>&schmonth=<? echo $schmonth; ?>&schyear=<? echo $schyear; ?>&grp_num=<? echo $grp_num; ?>">
<?
$host="localhost";
...
?> </font></font></td>
                ...
                <tr> 
                  ...
                  <td colspan="2"><font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#666666"><b>กำหนดให้มีการเตือนการนัดก่อน 
                    ...
                    <font size="1" face="MS Sans Serif, Microsoft Sans Serif" color="#666666"><b>วัน</b> 
					<font color="#0033CC" size="2">&nbsp; <? echo $app_remind_before_hour; ?></font><b> ชม.</b> </font></td>
                </tr>
                <tr> 
                  <td width="10%">&nbsp;</td>
<?
...
?>
                  <td><font color="#666666" size="1" face="MS Sans Serif, Microsoft Sans Serif"><b>เตือนงานผ่านอีเมล์: 
