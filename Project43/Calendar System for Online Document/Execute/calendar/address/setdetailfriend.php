...
$sql="select count(*) from contact where mem_id=$user_id&&friend_id=$friend_id&&grp_id=$group_id";
$result=mysql_db_query($db,$sql);
$data=mysql_fetch_array($result);
if($data[0]==0){header("Location:./addressbook.php");}
if($lastpage=='setdetail')
{
if($remind_bd=='check') $remind='Y';
else $remind='N';

$sql="update contact set con_remind_bd='$remind',con_note='$note' where grp_id=$group_id&&mem_id=$user_id&&friend_id=$friend_id";
$result=mysql_db_query($db,$sql);
mysql_close($link);
...
{
$sql="select t1.mem_id,mem_username,mem_fname,mem_lname,mem_nickname,t2.con_remind_bd,t2.con_note from member t1,contact t2 where t1.mem_id=t2.friend_id &&t2.grp_id=$group_id &&t2.friend_id=$friend_id";
$result=mysql_db_query($db,$sql);
$data=mysql_fetch_array($result);
$friend_id=$data[0];
$username=$data[1];
$name="คุณ $data[2] $data[3]";
$nickname=$data[4];
$old_remind=$data[5];
$note=$data[6];
}
          ...
          <TD vAlign=top width="727" height="497"> 
            <form method="post" action="">
              <table width="74%" border="1" cellspacing="0" cellpadding="0">
                <tr bgcolor="#33CCCC" align="center" valign="top"> 
                  <td width="28%"><b><font color="#FFFFFF" size="2" face="MS Sans Serif">ข้อมูล</font></b></td>
                  <td width="72%"><b><font color="#FFFFFF" size="2" face="MS Sans Serif">รายละเอียด</font></b></td>
                </tr><b>
                <tr> 
                  <td width="28%" align="right" height="27"><font size="2" color="#666666" face="MS Sans Serif">ชื่อผู้ใช้ 
                    &nbsp; : </font></td>
                  <td width="72%" align="left" height="27"><font face="MS Sans Serif" size="1" color="#000000">&nbsp;<? echo$username;?></font></td>
                </tr>
                <tr> 
                  <td width="28%" align="right" height="28"><font size="2" color="#666666" face="MS Sans Serif">ชื่อ-นามสกุล 
                    &nbsp; : </font></td>
                  <td width="72%" align="left" height="28"><font face="MS Sans Serif" size="1" color="#000000">&nbsp;<? echo$name;?></font></td>
                </tr>
                <tr> 
                  <td width="28%" align="right" height="32"><font size="2" color="#666666" face="MS Sans Serif">ชื่อเล่น&nbsp; 
                    : </font></td>
                  <td width="72%" align="left" height="32"><font face="MS Sans Serif" size="1" color="#000000">&nbsp;<? echo$nickname;?></font></td>
                </tr>
                <tr align="left" valign="top"> 
                  <td width="28%" align="right" height="144"><font size="2" color="#666666" face="MS Sans Serif">รายละเอียด&nbsp; 
                    : </font></td>
                  <td width="72%" height="144"> &nbsp;
                    <textarea name="note" cols="40" rows="6"><? echo$note;?></textarea>
                  </td>
                </tr></b>
                <tr> 
                  <td width="28%" align="right" height="34"> 
                    <input type="checkbox" name="remind_bd" value="check" <? if($old_remind=='Y') echo'checked';?> >
                    &nbsp;
                  </td>
                  <td width="72%" height="34"><b><font face="MS Sans Serif" size="1" color="#0000FF">&nbsp;ให้ระบบเตือนว่าใกล้วันเกิดของเพื่อนคนนี้</font></b></td>
                </tr>
                <tr> 
                  <td width="28%" align="right" height="37">&nbsp;</td>
                  <td width="72%" height="37" valign="middle"> 
                    <input type="submit" name="Submit" value="ตกลง">
                    <a href="../message/sendmessage.php?send_name=<? echo$username;?>"><b><font face="MS Sans Serif" size="2"><font color="#666666">&nbsp;ส่งข้อความหาเพื่อน</font></font></b></a> 
                    <input type="hidden" name="lastpage" value="setdetail">
