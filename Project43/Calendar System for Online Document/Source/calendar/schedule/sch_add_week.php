...
?> </font></a> </font></div>
                                  ...
                                  <tr bgcolor=#ffffff> 
                                    ...
                                    <td align=middle bgcolor=#FFCC99 width="25%" height="12"><a href="sch_add_week.php?schday=<? echo $schday; ?>&schdate=<? echo $schdate; ?>&schmonth=<? echo $schmonth; ?>&schyear=<? echo $schyear; ?>"><font color="#FF0000">&nbsp;<font face="MS Sans Serif, Microsoft Sans Serif" size="2">สัปดาห</font></font><font face="MS Sans Serif, Microsoft Sans Serif" size="2"><font color="#009900">์</font></font></a></td>
...
//include ไฟล์หาปฏิทิน
include("sch_day_of_month.inc");//$day_of_month[0-34] or [0-42] และ$num_week
$tmp=0;
for($i=0;$i<$num_week;$i++)
{
		for($j=0;$j<7;$j++)
		{
			$tmp2date=(int) substr($day_of_month[$tmp],0,2);
			$tmp2month=(int) substr($day_of_month[$tmp],3,2);
			if( $schdate == $tmp2date and $schmonth==$tmp2month )
			{
				$position_week=$i;
			}
			$tmp=$tmp+1;
...
$position_first=$position_week*7;
$first_of_week=$day_of_month[$position_first];
$last_of_week=$day_of_month[$position_first+6];
$first_date= (int) substr($first_of_week,0,2);
$first_month=(int) substr($first_of_week,3,2);
$first_year=(int) substr($first_of_week,6,4);

$last_date= (int) substr($last_of_week,0,2);
$last_month=(int) substr($last_of_week,3,2);
$last_year=(int) substr($last_of_week,6,4);

//-----next week
	$next_week= date("d/m/Y",mktime(1,1,0,$schmonth,$schdate+7,$schyear));
	$nx_day=(int) date("w",mktime(1,1,0,$schmonth,$schdate+7,$schyear));
	$nx_date=(int) substr($next_week,0,2);
	$nx_month=(int) substr($next_week,3,2);
	$nx_year= substr($next_week,6,4);
//----previous week
	$pre_week= date("d/m/Y",mktime(1,1,0,$schmonth,$schdate-7,$schyear));
	$pr_day=(int) date("w",mktime(1,1,0,$schmonth,$schdate-7,$schyear));
	$pr_date=(int) substr($pre_week,0,2);
	$pr_month=(int) substr($pre_week,3,2);
	$pr_year= substr($pre_week,6,4);


?>

          <td>
                  <div align="right"><a href="sch_add_week.php?schday=<? echo $pr_day; ?>&schdate=<? echo $pr_date; ?>&schmonth=<? echo $pr_month; ?>&schyear=<? echo $pr_year; ?>"><img src="../images/left_butt.gif" width="14" height="27" border="0"></a></div>
          </td>
          <td width="70%"> 
            <div align="center"> 
<?
		...
		echo $first_date," ",$thaimonth[$first_month-1]	," ",$first_year+543," - ",$last_date," ",$thaimonth[$last_month-1]	," ",$last_year+543;
		echo "</font></b>";

	?> &nbsp;</div>
          </td>
          <td> 
                  <div align="left"><a href="sch_add_week.php?schday=<? echo $nx_day; ?>&schdate=<? echo $nx_date; ?>&schmonth=<? echo $nx_month; ?>&schyear=<? echo $nx_year; ?>"><img src="../images/right_butt.gif" width="14" height="28" border="0"></a></div>
          </td>
        </tr>
      </table>
        ...
        <TR> 
          <TD width="79%" height="13"><img src="../images/sch_dayline.gif" width="85%" height="10"></TD>
        </TR>
        <TR> 
...
<?
//------ ตารางสัปดาห์
//----$position_week-- 0=สัปดาห์ที่1
$position=(int) $position_week*7;
?>

            <table width="95%" border="1" cellspacing="0" cellpadding="1" bordercolor="#006666">

<? 
for($i=0;$i<7;$i++)
{
	  $tmp_day=$i;
		$tmp_date=(int) substr($day_of_month[$position],0,2);
	  $tmp_month=(int) substr($day_of_month[$position],3,2);
	  $tmp_year=(int) substr($day_of_month[$position],6,4);
			
?>
              <tr bgcolor="#006666"> 
                <td>
 <a href="sch_add.php?schday=<? echo $tmp_day; ?>&schdate=<? echo $tmp_date; ?>&schmonth=<? echo $tmp_month; ?>&schyear=<? echo $tmp_year; ?>">	
<font face="MS Sans Serif, Microsoft Sans Serif" size="1" color="#66FFFF">
<?
			echo $thaiday[$tmp_day]," ",$tmp_date," ",$thaimonth[$tmp_month-1]	," ",$tmp_year+543;
			$position=$position+1;
?>
			</font></a></td>
              </tr>
<?
		$tmp_date2=$tmp_year."-".$tmp_month."-".$tmp_date;
		$sql_job_day="select sch_duration,sch_time,sch_title,sch_id from schedule where mem_id=$user_id and sch_date='$tmp_date2'";
...
?>
                <td>
				<font color="#999999" face="MS Sans Serif, Microsoft Sans Serif" size="1">
 <?
		for($j=0;$j<$numrow;$j++)
		{
				  $row=mysql_fetch_array($result);
					...
					$tmp_id=$row[3];
					echo "<a href=sch_edit1.php?time=$tmp_time&schday=$tmp_day&schdate=$tmp_date&schmonth=$tmp_month&schyear=$tmp_year&sch_id=$tmp_id><font color=#0000FF>",$row[2],"</font></a><br>";
		} //for $j
