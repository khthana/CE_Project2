...
$sql='select count(*) ,count(*)/10 from message,message_receiver where message.msg_id=message_receiver.msg_id 
...
<? //-------------หมดquickbutton------- ?>
          ...
          <TD vAlign=top width="727" height="497">
            ...
            <form name="form1" method="post" action="./deletemessage.php">
              ...
              <table width="100%" border="1">
                ...
                <tr bgcolor="#3366FF" align="center"> 
                  ...
                  <td width="8%"><font color="#FFFFFF" face="MS Sans Serif"></font></td>

                  <td width="35%"><font size="2"><b><font color="#FFFFFF" face="MS Sans Serif">หัวเรื่อง</font></b></font></td>

                  <td width="27%"><font size="2"><b><font color="#FFFFFF" face="MS Sans Serif">ชื่อผู้ส่ง</font></b></font></td>

                  <td width="25%"><font color="#FFFFFF" face="MS Sans Serif" size="2"><b>วันที่</b></font></td>
...
$link=mysql_connect($host,$user,$pass) or die("ไม่สามารถติดต่อฐานข้อมูลได้");
					...
					$sql='select t2.msg_id,t1.mem_username,t2.msg_title,t2.msg_date ,t3.msr_status 

						from member t1,message t2,message_receiver t3 where t2.msg_id=t3.msg_id &&t1.mem_id=t2.mem_id&& t3.friend_id='.$user_id.' order by msg_date desc,msg_time desc '.$sql_tail;
					...
					echo '<input type="checkbox" name="'.$data[0].'" value="'.$data[0].'">';
...
if($data[4]=='new')
{
echo '<td width="8%"><font color="#000000" size="1" face="MS Sans Serif">new</td>';
echo '<td width="38%"><a href="./viewmessage.php?message_id='.$data[0].'"><font color="#000000" size="2" face="MS Sans Serif" size="1"><b>'.$data[2].'</b></font></a></td>';
}
else{
echo '<td width="8%"><font color="#666666" size="1" face="MS Sans Serif">read</td>';
echo '<td width="35%"><a href="./viewmessage.php?message_id='.$data[0].'"><font color="#333333" size="1" face="MS Sans Serif">'.$data[2].'</font></a></td>';
}

					echo '<td width="27%"><font color="#000000" size="1" face="MS Sans Serif">'.$data[1].'</font></td>';

					$year=substr($data[3],0,4)+543;

					$month=$thaimonth[substr($data[3],5,2)-1];

					$date=substr($data[3],8,2).' '.$month.' '.$year;

					echo '<td width="25%"><font color="#000000" size="1" face="MS Sans Serif">'.$date.'</font></td>';
                ...
                <tr align="right"> 
				  ...
				  <font color="#333333" size="2" face="MS Sans Serif">แสดงข้อความที่

				  <?php 
					if($num_of_row==0)$start=0;
				 else  $start=($page-1)*10+1;
				  ...
				  echo ' '.$start.' ถึง '.$end.' จากข้อความทั้งหมด '.$num_of_row.' ข้อความ<br>';

				  echo' <a href="./readmessage.php?page=1"><font color="#666666" size="2" face="MS Sans Serif">ไปที่หน้าแรก</font></a>';

				  $prepage=$page-1;

				  if($page!=1){echo' | <a href="./readmessage.php?page='.$prepage.'"><font color="#666666" size="2" face="MS Sans Serif">ไปที่หน้าที่แล้ว</font></a>';}

					$nextpage=$page+1;

				  if($nextpage<=$num_of_page){echo ' | <a href="./readmessage.php?page='.$nextpage.'"><font color="#666666" size="2" face="MS Sans Serif">ไปที่หน้าต่อไป</font></a>';}

					echo' | <a href="./readmessage.php?page='.$num_of_page.'"><font color="#666666" size="2" face="MS Sans Serif">ไปที่หน้าสุดท้าย</font></a>';
mysql_close($link);
				  ?>
            ...
            <input type="submit" name="delete2" value="ลบข้อความ">
