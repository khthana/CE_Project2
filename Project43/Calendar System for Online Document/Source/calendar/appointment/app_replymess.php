          ...
          <TD width="79%"> 
            ...
            <table width="80%" border="0" cellspacing="2" cellpadding="0">
              ...
              <tr> 
                <td width="5%">&nbsp;</td>
                <td width="75%"> 
                  <div align="left"><font size="2" face="MS Sans Serif, Microsoft Sans Serif"><font color="#FF6600"> 
                    <?
		echo "<font size=1 color=#666666><b>ผู้ส่ง : </b></font>",$sender,"<br>";
		echo "<font size=1 color=#666666><b>สถานะ : </b></font>";

if($accept=="ยอมรับการนัด")
{
		echo "ยอมรับการนัดหมาย <br><br>";
		echo "<font size=1 color=#666666><b>ผล: </b></font><font size=1> ส่งข้อความไปบอกผู้ส่งถึงสถานะการนัดหมาย และ<br>";
		echo "&nbsp;&nbsp;&nbsp; ทำการใส่การนัดหมายลงไปในตารางเวลาของคุณเรียบร้อยแล้ว</font>";
		$apr_status="AC";
}elseif($notacc=="ไม่ยอมรับการนัด")
{
		echo "ปฏิเสธการนัดหมาย <br><br>";
		echo "<font size=1 color=#666666><b>ผล: </b></font><font size=1> ส่งข้อความไปบอกผู้ส่งถึงสถานะการนัดหมายเรียบร้อยแล้ว</font>";
		$apr_status="UN";
}elseif($postpone=="ขอคิดดูก่อน")
{
		echo "ขอคิดดูก่อน <br><br> ";
		echo "<font size=1 color=#666666><b>ผล: </b></font><font size=1> มีการเตือนการนัดหมายนี้จนถึงวันที่นัด<br>";
		echo "&nbsp;&nbsp;&nbsp; และถ้าเลยเวลานัดเราจะส่งข้อความอัตโนมัติเกี่ยวกับสถานะของการนัดไปยังผู้ส่ง</font>";
		$apr_status="PS";
}
?> </font></font> </div>
<?
...
if($apr_status=="AC")
{
 		$sql_app="update appoint_receiver set apr_status='AC',apr_comment='$comment' where app_id=$app_id and friend_id=$user_id";
// ใส่การนัดลงในตารางเวลา
		$sql_selectapp="select app_time,app_date,app_title,app_note,app_duration,app_remind_before,app_remind_before_hour,app_remind_mail from appointment where app_id=$app_id and mem_id=$sender_id";
		$result_app=mysql_db_query("magazine",$sql_selectapp);
		$row=mysql_fetch_array($result_app);
		$tmp_title="นัด: ".$row[2];
		 $sql_sch="insert into schedule(mem_id,sch_time,sch_date, sch_title,sch_note,sch_duration,sch_remind_before,sch_remind_before_hour,sch_remind_mail,sch_sendmail,app_id)
		 values('$user_id','$row[0]','$row[1]','$tmp_title','$row[3]','$row[4]','$row[5]','$row[6]','$row[7]','N',$app_id)";
		mysql_db_query("magazine",$sql_sch);

}elseif($apr_status=="UN")
{
 		$sql_app="update appoint_receiver set apr_status='UN',apr_comment='$comment' where app_id=$app_id and friend_id=$user_id";
}elseif($apr_status=="PS")
{
 		$sql_app="update appoint_receiver set apr_status='PS' where app_id=$app_id and friend_id=$user_id";
}
mysql_db_query("magazine",$sql_app);
mysql_close();
?>
                </td>
              </tr>
              ...
              <tr> 
                <td width="5%">&nbsp;</td>
                <td width="75%"> 
                  <div align="center"><a href="../welcome.php"><font face="MS Sans Serif, Microsoft Sans Serif" size="1" color="#0000FF">กลับไปหน้าแรก</font></a></div>
                </td>
              </tr>
            </table>
