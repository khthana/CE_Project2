...
<script language="vbscript" runat="server">
 ...
 DIM VL							'for VENDOR_LOCALITY table
	...
	Response.write"</head>"
	 	...
	 	strHTML=strHTML&"<form  name=""Chose"" method=post action=items.asp>"
...
'***********************
	...
	'if (store="") then
		'store=Request.QueryString("StoreName")
	'end if
	'if (items="") then
		'items=Request.QueryString("ItemsName")
	'end if
	if (local="") then
	...
	strHTML2=strHTML2&"<tr>"
												...
												s=""
												s=s&"<br><br> ข้อมูลสินค้า : "&Request.QueryString("items")&"<br>"
												s=s&"<br><br>"
													For i=0 To IteMax-1
														if Request.QueryString("items")=Ite_name(i) then
														if (Template=1) then							
														strHTML2=strHTML2&"<br><td width=""50%"">"
														end if
														...
														strHTML2=strHTML2&Ite_maxprice(i)&"<br>"&"</font>"
														'**************
														'moreinfo and pic2===>
														...
														'moreinfo and pic2===>
														if(Ite_moreinfo(i) <> "") then
															strHTML2=strHTML2&"<a href=" & "OTOP2.asp?"&"info="&Ite_moreinfo(i)&"&pic2="&Ite_pic2(i)&"&name="&Ite_name(i)&"&StoreName="&store&"&ItemsName="&items&"&LocName="&local&"&page="&"2"& ">"&"<font size="&Text_size&"><font color="&Text_color&">"&"<font face="&Text_font&">" & "<br>รายละเอียดเพิ่มเติม >>  </font></font></font></a>"
														end if
														'*************
														if (Template=1) then
														strHTML2=strHTML2&"</td>"
													...
													s=s&strHTML2
														's=s& "<CENTER><a href=" & "OTOP.asp" & ">"&"<font size=+1><font color="&TxtClr&">"&"<font face=""Comic Sans MS"">" & "<< BACK </font></font></font></a><CENTER>"
													...
													if (Request.QueryString("store")="None") then
														...
														if (Request.QueryString("tumbon")<>"None") then
																s=s& "<CENTER><a href=" & "OTOP.asp?"&"store="&Request.QueryString("store")&"&tumbon="&Request.QueryString("tumbon")&"&items="&Request.QueryString("items")& ">"&"<font size="&Text_size&"><font color="&Text_color&">"&"<font face="&Text_font&">" & "<< ดูข้อมูลสินค้าในตำบล : "&Request.QueryString("tumbon") &">>"&" </font></font></font></a><CENTER>"
															s=s& "<CENTER><a href=" & "store.asp?"&"store="&Request.QueryString("store")&"&tumbon="&Request.QueryString("tumbon")&"&items="&Request.QueryString("items")& ">"&"<font size="&Text_size&"><font color="&Text_color&">"&"<font face="&text_font&">" & "<< ดูข้อมูลร้านค้า : "&Request.QueryString("store")&">>"&"</font></font></font></a><CENTER>"	
														else 
																s=s& "<CENTER><a href=" & "OTOP.asp?"&"store="&Request.QueryString("store")&"&tumbon="&Request.QueryString("tumbon")&"&items="&Request.QueryString("items")& ">"&"<font size="&Text_size&"><font color="&Text_color&">"&"<font face="&Text_font&">" & "<< BACK  </font></font></font></a><CENTER>"
														end if
													end if
