...
{
	BOOL Bomb;
	LONG CountLoop = Host_Send_Bomb.GetCount();//Amount Host
	...
	{
		i--;
		POSITION pos = Host_Send_Bomb.FindIndex(i);//Find index i
		Temp_Packet_Bomb.Count++;
		Host_Send_Bomb.SetAt(pos,Temp_Packet_Bomb);//Replace at pos
		if ( Temp_Packet_Bomb.Count >= Max_Packet_Per_Sec ) //#define Max_Packet_Per_Sec 2000
		{//If packet More than Max_Packet_Per_Sec Alert;
