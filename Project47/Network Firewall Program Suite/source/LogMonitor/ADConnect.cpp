...
bool ADConnect::GetLog(CString Path,CStringArray& Log)
{
// COM result variable
	HRESULT hr;
	...
	//Get firewallRule
	VARIANT var;
	hr = pIADs->GetEx(L"firewallLog", &var );
	if ( SUCCEEDED(hr) )
	{
		LONG lstart, lend;
		SAFEARRAY *sa = V_ARRAY( &var );
...
bool ADConnect::SetNewLog(CString Path, CString NewLog)
{
	// COM result variable
	HRESULT hr;
	// Interface Pointers
...
//	MessageBox(NULL,OLE2T(pszTmpPath),"nice",MB_OK);
	...
	if (SUCCEEDED(hr))
	{
		...
		Log.Add(T2OLE(NewLog));
		LPWSTR *sa2;
		sa2 = new LPWSTR[lend + 2];
		for(idx=lstart; idx <= (lend+1) ; idx++)
			sa2[idx] = T2OLE(Log.GetAt(idx));

		VariantClear(&var);		
		hr = ADsBuildVarArrayStr(sa2,lend + 2, &var);
		if (!SUCCEEDED(hr)) return false;
		hr = pIADs->Put(L"firewallLog", var);
		VariantClear(&var);

	}
	else
	{
		LPWSTR sa2 = T2OLE(NewLog);
		hr = ADsBuildVarArrayStr( &sa2, 1, &var );
		hr = pIADs->Put(L"firewallLog", var); 
		VariantClear(&var);
	}
...
bool ADConnect::SetDeleteLog(CString Path,int Order)
{
// COM result variable
	HRESULT hr;
	...
	if ( SUCCEEDED(hr) )
	{
		...
		if(lend != 0)
		{
			...
			for ( long idx=lstart; idx <= lend; idx++ )
			{
				...
				if(idx != (Order))
				{
					...
					if(idx != (Order))
					{			
						CString TmpNewLog = OLE2T(V_BSTR(&var));
						Rule.Add(TmpNewLog);
						icount++;
					}
					VariantClear(&var);
		...
		else
		{
			VariantClear(&var);
			V_VT(&var)=VT_NULL;
			hr = pIADs->PutEx( ADS_PROPERTY_CLEAR, L"firewallLog", var);
			hr = pIADs->SetInfo();  
		}
	}
	if(pIADs) pIADs->Release();
