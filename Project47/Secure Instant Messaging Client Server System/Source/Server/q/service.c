...
//*******************************************************************************************
//				Group of Functions to be used in Login Process
//*******************************************************************************************

// This Function is just to be used in request_info().
// To get the positions between  UserID , Email and Nickname
// returned form ISAGMQ 
...
void request_info(SSL *ssl,char user_id[],char email[],char nick_name[],char client_ip[])
{
	...
	if((buffer_read[0] == 0x02) && (buffer_read[5] == 0x22))
	{
		for(i=0 ; i< MAX_MESSAGE ; i++)
		{
...
//*******************************************************************************************
//				End of Group of Functions to be used in Login Process
//*******************************************************************************************



//*******************************************************************************************
//				This Function to manipulate all process of Authorization
//*******************************************************************************************

//-------------------------------------------------------------------------------------------
//There are 2 processes of Authorization
//	1 List all of pending authorizations
//	2 Ask user to grant them according to below
//		Yes		- grant user
//		No		- deny 
//		Cancle  - exit process
//-------------------------------------------------------------------------------------------

void authorize(SSL *ssl,char *current_status)
{
	unsigned char buffer_user[MAX_MESSAGE];
	unsigned char buffer_read[MAX_MESSAGE];
	unsigned char buffer[MAX_MESSAGE];
	...
	for(i = 0 ; i < 20 ; i++) 
	{
		...
		// Ask user to grant request authorization

		if((reply_ok == 1) && (empty == 1))
		{
			printf("\nDo u want to continue to accept ?(Y- yes, N- No) :");
			scanf("%s",ans);
			if((ans[0] == 0x79) || (ans[0] == 0x59))
			{
				...
				if((contact_ok == 1) && (ans_ok == 1))
				{
					...
					{
						get_data(buffer_read,message,7);
...
//-------------------------------------------------------------------------------------------
// These are on demand for user to Deny (Block) someone on his contact list 
// and also Admit(Unblock) whenever he wants;
//-------------------------------------------------------------------------------------------
...
//*******************************************************************************************
//						        End of Deny / Admit Function
//*******************************************************************************************



//*******************************************************************************************
//						  All Functions for Status Information
//*******************************************************************************************

//-------------------------------------------------------------------------------------------
// There are 2 function below to be used for
//		1 Status Update - Request all of contact list status
//		2 Set Status	- Change own status and affect to all own contact list user
//-------------------------------------------------------------------------------------------
...
//*******************************************************************************************
//						  End of Functions for Status Information
//*******************************************************************************************
...
//*******************************************************************************************
//						  End of Functions to Change Nickname
//*******************************************************************************************


//*******************************************************************************************
//						  All Functions for Delete Contact List
//*******************************************************************************************

//-------------------------------------------------------------------------------------------
// There are 2 function below to be used for
//		1 Interface to user to get Contact ID to be deleted		-Delete_contactlist()
//		2 Send and Receive Information between IsagQ and IsagMQ -de_con()
//-------------------------------------------------------------------------------------------
...
void delete_contactlist(SSL *ssl){
	...
	status(ssl);
  	fprintf(stdout, "\nType the User_id that you would like to delete from Contact_list\n");			
        fprintf(stdout, "   :");
	scanf("%s", temp);
	strncat(temp, "$", 1);
	...
	while(1)
	{
		printf("\n    Do you really want to Delete from Contact_list\n");
...
//*******************************************************************************************
//						  End of Functions for Delete Contact List
//*******************************************************************************************


//*******************************************************************************************
//								 Find Contact List Functions 
//*******************************************************************************************

//-------------------------------------------------------------------------------------------
// There are 4 categories that user can use
//		1 User ID 
//		2 Add Contact List with User ID
//		3 Add Contact List with Email 
//		4 Add Contact List with User ID
//-------------------------------------------------------------------------------------------
...
void find_contactlist(SSL *ssl){
	...
	strncat(mess, "#", 1);	
	fprintf(stdout, "    User ID (least than 10 char):");			
    scanf("%s", fuser_id);
	...
	strcat(mess, femail);
	strncat(mess, "#", 1);	
	fprintf(stdout, "First Name (least than 64 char):");
	scanf("%s", ffrist_name);
	...
	{			
		...
		if( (buffer_read[0] == 0x08) && (buffer_read[5] == 0x82) )
    	{
...
//*******************************************************************************************
//						  All Functions To Add Contact List
//*******************************************************************************************

//-------------------------------------------------------------------------------------------
// There are 2 function below to be used for
//		1 Add Contact List with Email 
//		2 Add Contact List with User ID
//-------------------------------------------------------------------------------------------
	...
	int i;
  	...
  	unsigned char test[MAX_MESSAGE];
		...
		if(type == 'a')
		{
			...
			if((buffer_user[0] == 0x03) && (buffer_user[5] == 0x32)) 
			{
				fprintf(stdout,"\nInsert Completed\n");
			}
			else
...
//*******************************************************************************************
//						  End of Functions To Add Contact List
//*******************************************************************************************
