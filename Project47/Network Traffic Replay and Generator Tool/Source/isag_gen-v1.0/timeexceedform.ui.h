...
MyThread timeex;
void timeexceedForm::GenerateTimeExceed()
{
    ...
    char order[500];
    sprintf(order,"isag_gen -1  -G icmp_timeexceed -i %s -s %s  -d %s  -c %s " ,device,sip,dip ,number);
    timeex.setfile(order);
    timeex.start();
    //system(order);
    this->progresstimeex();
...
void timeexceedForm::progresstimeex()
{
int check = 0;
