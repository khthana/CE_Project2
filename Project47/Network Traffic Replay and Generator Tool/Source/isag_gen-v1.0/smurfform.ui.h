...
MyThread smurf;
void smurfForm::GenerateSmurf()
{
    
    char device[10] ;
    strcpy(device,lineEdit_smurf_Device->text());
    
    char dip[16];
    strcpy(dip, dIPLineEdit->text());    
   
    char aip[16];
    strcpy(aip,networkIPlineEdit->text());     
    
    char number[10];
    strcpy(number,numberPacketlineEdit->text());
   
    char order[500];
    sprintf(order,"isag_gen -1 -i %s -G smurf  -d %s  -a %s -c %s ",device,dip ,aip ,number);
    //system(order);
    smurf.setfile(order);
    smurf.start();
    this->progresssmurf();
