...
import security.publickey.RSAAlgorithm;
...
public class SSHClient extends SSH {
  ...
  void negotiateVersion() throws IOException {
    ...
    if(srvVersionMajor > 1) {
      throw new IOException("This server doesn't support ssh1, connect with " +
			    "ssh2 enabled");
  ...
  void receiveServerData() throws IOException {
    ...
    srvServerKey = new security.publickey.RSAPublicKey(n, e);

    bits = pdu.readInt();
    e = pdu.readBigInteger();
    n = pdu.readBigInteger();
    srvHostKey = new security.publickey.RSAPublicKey(n, e);
