...
public final class TerminalWin extends Canvas
    implements Terminal, KeyListener, AdjustmentListener, MouseListener,
	       MouseMotionListener, ComponentListener, FocusListener
{
    ...
    int     lastKeyKludge     = 0;
    boolean lastKeyWasPressed = false;
    ...
    public int pasteButton = InputEvent.BUTTON2_MASK;
    ...
    volatile boolean repaintPending;
    ...
    public final static char[] byte2char = new char[256];
    ...
    public TerminalWin(Frame ownerFrame, TerminalInterpreter interpreter,
		       Properties initProps)
	throws IllegalArgumentException
    {
	this(ownerFrame, interpreter, initProps, false);
    }
	...
	if(setAsDefault) {
	    TerminalDefProps.setAsDefault(initProps);
	}
	...
	if(menuHandler != null) {
	    menuHandler.update();
	    for(int i = 0; i < OPT_LAST_MENU; i++) {
		menuHandler.setStateOpt(i, termOptions[i]);
	    }
	    menuHandler.setEnabledOpt(OPT_DECCOLM,termOptions[OPT_DEC132COLS]);
	}
    }
	...
	for(i = termOptions.length;
	    i < TerminalDefProps.defaultPropDesc.length; i++) {
	    name  = TerminalDefProps.defaultPropDesc[i][
	...
	setProperties(TerminalDefProps.defaultProperties, false);
    }
	...
	key  = TerminalDefProps.backwardCompatProp(key);
	return TerminalDefProps.defaultProperties.getProperty(key);
    }
	...
	for(i = 0; i < termOptions.length; i++) {
	    if(TerminalDefProps.defaultPropDesc[i][TerminalDefProps.PROP_NAME].
	       equals(key))
		break;
	}
	if(i < termOptions.length) {
		...
		if(value.equals("top")) {
		    ...
		    throw new IllegalArgumentException(
			"reszize gravity can be 'top' or 'bottom'");
		}
		this.resizeGravity = rg;
	    } else if(key.equals("delete-send")) {
		if(value.equals("DEL"))
		    delCharacter = (char)0x7f;
		else if(value.equals("BS")) {
		    delCharacter = (char)0x08;
		} else {
		    throw new IllegalArgumentException(
			"delete character can be 'DEL' or 'BS'");
		}
	    } else if(key.equals("backspace-send")) {
		if(value.equals("DEL"))
		    bsCharacter = (char)0x7f;
		else if(value.equals("BS")) {
		    bsCharacter = (char)0x08;
		} else {
		    throw new IllegalArgumentException(
			"backspace character can be 'DEL' or 'BS'");
		}
	    } else if(key.equals("geometry")) {
		setGeometry(value, true);
	    } else if(key.equals("select-delim")) {
		if(!(value.charAt(0) == '"'
		     && value.charAt(value.length() - 1) == '"')) {
		    value = "\"" + value + "\"";
		}
		selectDelims = value.substring(1, value.length());
	    } else if(key.equals("paste-button")) {
	    ...
	    } else if(key.equals("input-charset")) {
		setInputCharset(value);
	    } else if(key.equals("line-space-delta")) {
		lineSpaceDelta = Integer.parseInt(value);
	    } else {
    ...
    // !!! Ouch !!!
    //
    public void setGeometry(String geometry, boolean doResize)
	throws IllegalArgumentException {
	int ro, co, xPos, yPos, xSz, ySz, delX, delY,
	    delim = geometry.indexOf('x');
	...
	try {
		...
		// !!! We can only calculate right position when ownerFrame
		// isShowing otherwise the insets and everything is not set,
		// then we set pendingShow to be called later
		// from componentShown
		//
		if(isShowing())
	...
	String sb = getProperty("scrollbar");
	if(sb.equals("left"))
	    myPanel.add(scrollbar, BorderLayout.WEST);
	...
	*/
	...
	System.arraycopy(oldAutowraps, saveVisTop, autowraps, 0, rows);
	saveVisTop = 0;
	visTop     = 0;
	updateScrollbarValues();
	makeAllDirty(true);
	...
	autowraps  = new boolean[rows + saveLines];
    }
    ...
    protected synchronized final void updateDirtyArea(int top, int left,
	...
	synchronized(writeLock) {
		...
		for(int i = 0; i < n; i++) {
		    TerminalOutputListener outListener = (TerminalOutputListener)
		...
		repaintPending = false; // To force repaint()
		visTop         = saveVisTop;
		if(haveScrollbar)
	    ...
	    int iic;
	    if((iic = interpreter.interpretChar(c)) != TerminalInterpreter.IGNORE) {
		char ic;
		...
		screen[idxRow][curCol++]   = ic;
	    }

	    repaint();
	}
    }
    public final void write(char[] c, int off, int len) {
	synchronized(writeLock) {
	    waitForMore    = (len > 64);
	    repaintPending = true; // To inhibit repaint() until after for-loop
	    int end = off + len;
    ...
    public final void write(byte[] c, int off, int len) {
	synchronized(writeLock) {
	    waitForMore    = (len > 64);
	    repaintPending = true; // To inhibit repaint() until after for-loop
	    int end = off + len;
	...
	if(DEBUG) System.out.println("typedChar: " + c + "(" + (int)c + ")");
	if(inListeners != null) {
	...
	if(inListeners != null) {
		...
		TerminalInputListener inListener = (TerminalInputListener)
		    inListeners.elementAt(i);
		if(inListener != null) {
		    inListener.signalWindowChanged(rows, cols, vpixels, hpixels);
		}
	    }
	}
    }
	...
	if(complexScroll) {
		...
		if((visTop + sl) > saveLines) {
		    ...
		    System.arraycopy(autowraps, sl, autowraps, 0, saveLines + ll);
		...
		if(n < windowHeight) {
		    ...
		    System.arraycopy(autowraps, visTop + windowTop + n, autowraps,
				     visTop + windowTop, (windowHeight - n));
		}
		for(i = j; i < windowBottom; i++) {
	...
	if(complexScroll) {
		...
		System.arraycopy(attributes, visTop + windowTop, attributes, visTop + windowTop + n,
				 windowHeight - n);
		System.arraycopy(autowraps, visTop + windowTop, autowraps, visTop + windowTop + n,
				 windowHeight - n);
	    }
	    for(i = windowTop; i < j; i++) {
		screen[visTop + i]     = new char[cols];
	...
	if(complexScroll) {
		...
		System.arraycopy(attributes, visTop + curRow, attributes, visTop + edge,
				 windowBottom - edge);
		System.arraycopy(autowraps, visTop + curRow, autowraps, visTop + edge,
				 windowBottom - edge);
	    }
	    int[] attrLine = new int[cols];
	    for(i = 0; i < cols; i++)
	...
	if(complexScroll) {
		...
		System.arraycopy(attributes, visTop + curRow + n, attributes, visTop + curRow,
				 edge - curRow);
		System.arraycopy(autowraps, visTop + curRow + n, autowraps, visTop + curRow,
				 edge - curRow);
	    }
	    int[] attrLine = new int[cols];
	    for(i = 0; i < cols; i++)
	...
	termOptions[opt] = val;
	if(menuHandler != null && opt < Terminal.OPT_LAST_MENU)
    ...
    private void kludgeProcessKeyEvent(KeyEvent e) {
	int    virtKey = e.getKeyCode();
	int    mod     = e.getModifiers();
	char   c       = e.getKeyChar();

	// Consume TAB to be sure no one uses it so we lose focus
	//
	if(virtKey == KeyEvent.VK_TAB || virtKey == KeyEvent.VK_F10) {
	    e.consume();
	}
    ...
    private boolean isNewEvent(KeyEvent e, boolean byPressed) {
	int ch = (int) e.getKeyChar();

        if(lastKeyWasPressed != byPressed) {
            if(ch == lastKeyKludge)
                return false;
            // XXX: Are there any more cases like this?
            //
	...
	if(DEBUGKEYEVENT) {
        ...
        if(isNewEvent(e, false))
            kludgeProcessKeyEvent(e);
    }
	...
	if(DEBUGKEYEVENT) {
	    System.out.println("pressed: " + c + " (" + ((int)c) + ") code=" + virtKey);
	}

        if(isNewEvent(e, true))
            kludgeProcessKeyEvent(e);
    }
	...
	case KeyEvent.VK_BACK_SPACE:
	    typedChar(bsCharacter);
	    break;
	case KeyEvent.VK_DELETE:
	    typedChar(delCharacter);
	    break;
	...
	if(virtKey == KeyEvent.VK_SHIFT ||
	   virtKey == KeyEvent.VK_CONTROL ||
	   virtKey == KeyEvent.VK_CAPS_LOCK ||
	   virtKey == KeyEvent.VK_ALT) {
	...
	if(newRows != rows || newCols != cols) {
		...
		for(i = 0; i < copyRows; i++) {
		    ...
		    autowraps[i + toTop] = oldAutowraps[i + fromTop];
		}
	    }
	...
	if(mouseRow == selectRowLast && mouseCol == selectColLast && hasSelection) {
	    return;
	}
    ...
    volatile boolean waitForMore = false;
	...
	if(!repaintPending) {
	    // If we we don't have a repaint pending the cause for update must
	    // be "destroyed" window content.
	    //
	    dirtyTop    = 0;
	    dirtyBottom = rows;
	    dirtyLeft   = 0;
	    ...
	    int cr = (curRow >= rows ? rows - 1 : curRow);
	    int cc = (curCol >= cols ? cols - 1 : curCol);
	    if(dirtyTop == rows)
		dirtyTop = cr;
	    if(dirtyBottom == 0)
		dirtyBottom = cr + 1;
	...
	int    attr;
	int    attrMasked;
	for(int i = dirtyTop; i < dirtyBottom
                && (i+visTop) < attributes.length; i++) {
	    y = borderHeight + (i * charHeight);
	    attrRow = attributes[visTop + i];
	    charRow = screen[visTop + i];
	    for(int j = dirtyLeft; j < dirtyRight && j < attrRow.length; j++) {
		attr       = attrRow[j];
    ...
    */
    ...
    /* !!! Do we want this? Check Mac once again, this was for buggy AWT
