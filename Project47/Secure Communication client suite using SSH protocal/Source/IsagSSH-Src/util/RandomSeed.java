	       ...
	       KeyListener, FocusListener, ComponentListener
{
	...
	if(DEBUG) System.out.println("RandomSeed.mouseMoved: " + e);
	mouseCnt++;
	int dX = lastX - e.getX();
	int dY = lastY - e.getY();
	lastX  = e.getX();
	...
	if(DEBUG) System.out.println("RandomSeed.eventTick: " + o);
	evtCnt++;
	evtHash ^= o.hashCode();
	if((evtCnt % 5) == 0) {
	...
	terminal.TerminalWin terminal =
	    new terminal.TerminalWin(frame,
	    new terminal.TerminalXTerm());
