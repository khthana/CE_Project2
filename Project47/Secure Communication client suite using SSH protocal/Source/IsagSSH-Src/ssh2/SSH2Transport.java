...
public final class SSH2Transport {
    ...
    private final static int  PACKETS_BEFORE_REKEY = 2147483647;
    private final static int  BYTES_BEFORE_REKEY= 1073741824;
	...
	try {
		...
		if (txNumPacketsSinceKEX >= PACKETS_BEFORE_REKEY
		    || txNumBytesSinceKEX >= BYTES_BEFORE_REKEY) {
		    startKeyExchange();
		}
	    }
	} catch (ShortBufferException e) {
	...
	if (rxNumPacketsSinceKEX >= PACKETS_BEFORE_REKEY
	    || rxNumBytesSinceKEX >= BYTES_BEFORE_REKEY) {
	    startKeyExchange();
	}
    }
