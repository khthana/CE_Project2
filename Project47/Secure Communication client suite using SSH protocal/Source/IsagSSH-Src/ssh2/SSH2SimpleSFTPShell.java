...
public final class SSH2SimpleSFTPShell implements Runnable {
	...
	this.terminal   = new TerminalWin(frame, new TerminalXTerm());
	...
	try {
		...
		try {
			...
			*/
		    ...
		    } else if(cmd.equals("q") || cmd.equals("quit") || cmd.equals("bye") || cmd.equals("exit")) {
	...
	linereader.println("");
	linereader.println("bye\t\t\t\t\tquit this sftp session");
	linereader.println("cd <dir>\t\t\t\tchange current directory (remote)");
	linereader.println("dir\t\t\t\t\tlist current directory (remote)");
	linereader.println("exit\t\t\t\t\tquit this sftp session");	
	...
	linereader.println("ldir\t\t\t\t\tlist current directory (local)");
