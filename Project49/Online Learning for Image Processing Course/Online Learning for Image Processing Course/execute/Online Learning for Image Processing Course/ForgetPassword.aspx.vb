...
Partial Class ForgetPassword
    ...
    Protected Sub submit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles submit.Click
        ...
        For i = 0 To 3
            ...
            End If
            QueryStr = "select " & prefix & "password from " & tablename & _
                        " where " & prefix & "username = '" & username.Text & "' and " & prefix & "question = '" & _
                        questionDDL.SelectedItem.Value & "' and " & prefix & "answer = '" & answer.Text & "'"
            TableStr = "getpassword"
            If FillCommQ(QueryStr, TableStr) = True Then
                If sqlDS.Tables(TableStr).Rows.Count = 1 Then
                    For Each row As DataRow In sqlDS.Tables(TableStr).Rows
                        status = True
                        Dim pw As String = row(prefix & "password")
                        Response.Redirect("Result.aspx?msg=รหัสผ่านของคุณคือ " & pw)
                    Next
                Else
                    status = False
                End If
        ...
        If status = False Then
            warning.Text = "ข้อมูลของคุณไม่ถูกต้อง"
            Exit Sub
        End If
    End Sub
