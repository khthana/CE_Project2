...
Partial Class Student_StudentRegisterPage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim getbregister(2) As String
        Dim geteregister(2) As String
        If Session("loginstatus") = True And Session("user") <> 0 Then
            If Session("usertype") = "Student" Then
                If Not Page.IsPostBack Then
                    '************ Check Date Time to Registet
                    QueryStr = "select cal_id,cal_year,cal_term,cal_bregister,cal_eregister from LearningCalendar where cal_year = (select max(cal_year) from LearningCalendar) and cal_term = (select max(cal_term) from LearningCalendar where cal_year = (select max(cal_year) from LearningCalendar))"
                    ...
                    If FillCommQ(QueryStr, TableStr) = True Then
                        If sqlDS.Tables(TableStr).Rows.Count = 1 Then
                            For Each row As DataRow In sqlDS.Tables(TableStr).Rows
                                year = row("cal_year")
                                term = row("cal_term")
                                getbregister = CStr(row("cal_bregister")).Split("/")
                                geteregister = CStr(row("cal_eregister")).Split("/")
                            Next
                    ...
                    If Val(getmydate(2)) > Val(getbregister(2)) Then
                        tmp1 = True
                    ElseIf Val(getmydate(2)) = Val(getbregister(2)) Then
                        If Val(getmydate(1)) > Val(getbregister(1)) Then
                            tmp1 = True
                        ElseIf Val(getmydate(1)) = Val(getbregister(1)) Then
                            If Val(getmydate(0)) > Val(getbregister(0)) Then
                                tmp1 = True
                            ElseIf Val(getmydate(0)) = Val(getbregister(0)) Then
                                tmp2 = True
                            Else
                                Response.Redirect("../Result.aspx?msg=ตอนนี้ยังไม่ถึงกำหนดการลงทะเบียน&user=Student")
                            End If
                        Else
                            Response.Redirect("../Result.aspx?msg=ตอนนี้ยังไม่ถึงกำหนดการลงทะเบียน&user=Student")
                        End If
                    ...
                    End If
                    If tmp1 = True Then
                        If Val(getmydate(2)) > Val(geteregister(2)) Then
                            Response.Redirect("../Result.aspx?msg=ปิดการลงทะเบียน&user=Student")
                        ElseIf Val(getmydate(2)) = Val(geteregister(2)) Then
                            If Val(getmydate(1)) > Val(geteregister(1)) Then
                                Response.Redirect("../Result.aspx?msg=ปิดการลงทะเบียน&user=Student")
                            ElseIf Val(getmydate(1)) = Val(geteregister(1)) Then
                                If Val(getmydate(0)) > Val(geteregister(0)) Then
                                    Response.Redirect("../Result.aspx?msg=ปิดการลงทะเบียน&user=Student")
                                ElseIf Val(getmydate(0)) = Val(geteregister(0)) Then
                                    tmp3 = True
                                Else
                                    tmp3 = True
                                End If
                        ...
                        If FillCommQ(QueryStr, TableStr) = True Then
                            If sqlDS.Tables(TableStr).Rows.Count = 1 Then
                                ...
                                If Val(getreg) = 1 Then
                                    Response.Redirect("../Result.aspx?msg=คุณได้ลงทะเบียนเรียบร้อยแล้ว&user=Student")
    ...
    Protected Sub submit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles submit.Click
        ...
        If status = True Then
            ...
            QueryStr = "insert into Register(std_id,reg_year,reg_term) values('" & Session("user") & _
                "','" & year & "','" & term & "')"
            ...
            If FillCommQ(QueryStr, TableStr) = True Then
                If IsNothing(sqlDS) Then
                    warning.Text = "Dataset has not found . There is no currentID." & "<br />" & "ไม่สามารถลงทะเบียนได้ กรุณาติดต่อผู้ดูแลระบบ"
                    Exit Sub
                Else
                    If sqlDS.Tables(TableStr).Columns.Count > 0 Then
                        If sqlDS.Tables(TableStr).Rows.Count > 0 Then
                            ...
                            Next
                        Else
                            warning.Text = "Dataset has no row. There is no currentID." & "<br />" & "ไม่สามารถลงทะเบียนได้ กรุณาติดต่อผู้ดูแลระบบ"
                            Exit Sub
                        End If
                    Else
                        warning.Text = "Dataset has no column. There is no currentID." & "<br />" & "ไม่สามารถลงทะเบียนได้ กรุณาติดต่อผู้ดูแลระบบ"
                        Exit Sub
                    End If
                End If
            Else
            ...
            For j = 0 To arrSubID.Length - 1
                QueryStr = "insert into Result(reg_id,sub_id,sec_id,res_grade,res_score,res_assess) values('" & _
                    currentID & "','" & arrSubID(j) & "','" & arrsec(j) & "','-','0.0','0')"
                sqlComm = New SqlCommand(QueryStr, sqlConn)
                sqlComm.CommandType = CommandType.Text
                sqlComm.ExecuteNonQuery()
            Next
            CloseConn()
            Response.Redirect("../Result.aspx?msg=การลงทะเบียนเสร็จเรียบร้อยแล้ว&user=Student")
        Else
