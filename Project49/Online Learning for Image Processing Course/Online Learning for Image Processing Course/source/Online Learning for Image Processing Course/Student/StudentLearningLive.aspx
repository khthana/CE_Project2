<%@ Page Language="VB" AutoEventWireup="false" CodeFile="StudentLearningLive.aspx.vb" Inherits="Student_StudentLearningLive" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml" >
<head runat="server">
    <title>Student Learning Live</title>
                ...
                <td valign="top" width="600">
                    <table style="margin-top:10px;margin-bottom:10px;margin-left:10px;margin-right:10px" width="550">
                        ...
                        <tr height="30" style="font-weight:bold">
                            <td colspan="2">การเรียนแบบถ่ายทอดสด</td>
                        </tr>
                        <tr height="10"><td colspan="2"></td></tr>
                        <tr height="30">
                            <td colspan="2">
                                <asp:SqlDataSource ID="subjectDTS" runat="server" ConnectionString="<%$ ConnectionStrings:ProjectDBConnectionString %>"
                                    SelectCommand="select s.sub_id, s.sub_name, ss.sec_dlearn, ss.sec_tlearn from subject s, subjectsection ss 
                                                    where s.sub_id=ss.sub_id and s.sub_id in (select sub_id from subjectsection where sec_id in (select sec_id 
                                                    from result where reg_id = (SELECT reg_id FROM [register] WHERE reg_year = (SELECT MAX(reg_year) FROM [register])
                                                    and reg_term = (select max(reg_term) from register where reg_year = (select max(reg_year) from register)) 
                                                    and ([std_id] = @std_id)))) and ss.sec_id in (select sec_id from result where reg_id = (SELECT reg_id FROM [register] 
                                                    WHERE reg_year = (SELECT MAX(reg_year) FROM [register]) and reg_term = (select max(reg_term) from register 
                                                    where reg_year = (select max(reg_year) from register)) and ([std_id] = @std_id))) order by s.sub_id">
                                    <SelectParameters>
                                        <asp:SessionParameter Name="std_id" SessionField="user" Type="Int64" />
                                    </SelectParameters>
                                </asp:SqlDataSource>
                                <asp:DataList ID="DataList1" runat="server" DataKeyField="sub_id" DataSourceID="subjectDTS" CellPadding="4" ForeColor="#333333">
                                    <HeaderTemplate>
                                        <table width="550">
                                            <tr align="center">
                                                <td style="font-weight:bold">วิชา</td>
                                            </tr>
                                        </table>
                                    </HeaderTemplate>
                                    <ItemTemplate>
                                        <table width="550">
                                            <tr align="center" height="30">
                                                <td width="50"><img src="../Image/viewicon.gif" align="middle" /></td>
                                                <td width="500" align="left">
                                                    <%#"<a href=StudentLearningLivePage.aspx?sub_id=" & Container.DataItem("sub_id") & _
                                                       "&date=" & Container.DataItem("sec_dlearn") & "&time=" & Container.DataItem("sec_tlearn") & _
                                                       " target=_blank>" & Container.DataItem("sub_name") & "</a>"%>
                                                </td>
                                            </tr>
                                        </table>
                                    </ItemTemplate>
