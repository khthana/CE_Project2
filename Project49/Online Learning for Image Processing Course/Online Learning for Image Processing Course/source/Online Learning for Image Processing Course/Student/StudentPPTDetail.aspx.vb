...
Partial Class Student_StudentPPTDetail
    Inherits System.Web.UI.Page

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'Dim todate As String = Today
        Dim todate As Date = System.DateTime.Now.ToLongDateString
        Dim to_date As String = todate
        Dim mydate(2) As String
        Dim dateshow(2) As String
        mydate = to_date.Split("/")
        dateshow = Request.QueryString("date").Split("/")
        If Session("loginstatus") = True And Session("user") <> 0 Then
            If Session("usertype") = "Student" Then
                ...
                End If
                '*************************
                Dim setlearning As Boolean = False
                If Val(mydate(2)) > Val(dateshow(2)) Then
                    setlearning = True
                ElseIf Val(mydate(2)) = Val(dateshow(2)) Then
                    If Val(mydate(1)) > Val(dateshow(1)) Then
                        setlearning = True
                    ElseIf Val(mydate(1)) = Val(dateshow(1)) Then
                        If Val(mydate(0)) >= Val(dateshow(0)) Then
                            setlearning = True
                        Else
                            Response.Redirect("../Result.aspx?msg=คุณยังไม่สามารถดูไฟล์นี้ได้&action=close")
                        End If
                    Else
                        Response.Redirect("../Result.aspx?msg=คุณยังไม่สามารถดูไฟล์นี้ได้&action=close")
                    End If
                ...
                Dim mytime As String = System.DateTime.Now.ToLongTimeString

                If setlearning = True Then
                    QueryStr = "select lowppt_topic,sub_id from LearningOnWebPPT where lowppt_id = '" & Request.QueryString("lowppt_id") & "'"
                    TableStr = "gettopic"
                    If FillCommQ(QueryStr, TableStr) = True Then
                        If sqlDS.Tables(TableStr).Rows.Count = 1 Then
                            For Each row As DataRow In sqlDS.Tables(TableStr).Rows
                                topic = "สื่อการเรียนการสอน Power Point เรื่อง " & CStr(row("lowppt_topic"))
                                subid = row("sub_id")
                            Next
                        Else
                            Response.Redirect("../DBFailed.aspx")
                    ...
                    QueryStr = "insert into Learning(cal_id,sub_id,std_id,lrn_content,lrn_date,lrn_time) values('" & _
                        calid & "','" & subid & "','" & Session("user") & "','" & topic & "','" & myday & "','" & mytime & "')"
