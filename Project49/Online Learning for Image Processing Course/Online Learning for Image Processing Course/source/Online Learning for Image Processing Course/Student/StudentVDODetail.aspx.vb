...
Partial Class Student_StudentVDODetail
        ...
        If Session("loginstatus") = True And Session("user") <> 0 Then
            If Session("usertype") = "Student" Then
                ...
                If setlearning = True Then
                    QueryStr = "select lowvdo_topic,sub_id from LearningOnWebVDO where lowvdo_id = '" & Request.QueryString("lowvdo_id") & "'"
                    TableStr = "gettopic"
                    If FillCommQ(QueryStr, TableStr) = True Then
                        If sqlDS.Tables(TableStr).Rows.Count = 1 Then
                            For Each row As DataRow In sqlDS.Tables(TableStr).Rows
                                topic = "สื่อการเรียนการสอน Clip VDO เรื่อง " & CStr(row("lowvdo_topic"))
                                subid = row("sub_id")
                            Next
                        Else
                            Response.Redirect("../DBFailed.aspx")
