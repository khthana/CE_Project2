...
Partial Class Administrator_AdministratorGeneralWB
    ...
    Protected Sub GridView1_DeleteRow(ByVal sender As Object, ByVal e As GridViewDeleteEventArgs)
        ...
        'QueryStr = "delete from ReplyWB where top_id = '" & GridView1.DataKeys(Convert.ToInt32(e.RowIndex)).Value.ToString() & "'"
        'If ExecuteComm(QueryStr) = True Then
        '    QueryStr = "delete from TopicWB where top_id = '" & GridView1.DataKeys(Convert.ToInt32(e.RowIndex)).Value.ToString() & "'"
        '    If ExecuteComm(QueryStr) = True Then
        '        Response.Redirect("../Result.aspx?msg=ได้ลบกระทู้ที่ " & GridView1.DataKeys(Convert.ToInt32(e.RowIndex)).Value.ToString() & " ของกระดานสนทนาทั่วไปเสร็จเรียบร้อยแล้ว&user=Administrator")
        '    Else
    ...
    Private Sub DeleteRecordByID(ByVal TopID As Integer)
        ...
        If ExecuteComm(QueryStr) = True Then
            ...
            If ExecuteComm(QueryStr) = True Then
                Response.Redirect("../Result.aspx?msg=ได้ลบกระทู้ที่ " & TopID & " ของกระดานสนทนาทั่วไปเสร็จเรียบร้อยแล้ว&user=Administrator")
            Else
                Response.Redirect("../DBFailed.aspx")
            End If
