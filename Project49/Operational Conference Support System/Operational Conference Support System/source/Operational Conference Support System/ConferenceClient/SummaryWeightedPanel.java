...
public class SummaryWeightedPanel extends JPanel{
	...
	// For weighted vote(More than yes/no vote)
	private static int numChoice;			// Number of choice
	private static int valPoint;			// Total points
...
//	private static JLabel[] labelChoice2 = new JLabel[10];
	...
	{	
		...
		JLabel labelValPoint = new JLabel("คะแนนรวมทั้งหมด: ", SwingConstants.LEFT);
		labelValPoint.setFont(new Font("",0,20));
		labelValPoint.setOpaque(true);
		labelValPoint.setBackground(Color.ORANGE);
		gc.gridx = 0; gc.gridy = 8; // position of grid [like array two dimention]
		p2.add(labelValPoint, gc);

		gc.gridx = 3; gc.gridy = 8; // position of grid [like array two dimention]
		p2.add(labelValPoint2, gc);
		...
		gc.gridx = 3; gc.gridy = 10; // position of grid [like array two dimention]
		p2.add(labelChoiceT, gc);
		
		gc.gridx = 6; gc.gridy = 10; // position of grid [like array two dimention]
		...
		gc.gridx = 1; gc.gridy = 12; // position of grid [like array two dimention]
		p2.add(labelBlank,gc);
		
		gc.gridx = 2; gc.gridy = 12; // position of grid [like array two dimention]
		p2.add(labelVoteNow,gc);
		
		gc.gridx = 3; gc.gridy = 12; // position of grid [like array two dimention]
		p2.add(labelCancel,gc);
		
		gc.gridx = 4; gc.gridy = 12; // position of grid [like array two dimention]
	...
	public static void setTextDisplay(String mname, String iname, String tname,
										...
										int nChoice, int vPoint, String[] choice) {
		meetingNameSt = mname;
		itemNameSt = iname;
		typeSt = tname;
		topicSt = toname;
		...
		labelValPoint2.setText(vPoint + "");
		labelValPoint2.setFont(new Font("",0,25));
    		...
    		try {
    		  ...
    		  if(e.getSource() == labelVoteNow) {
				 	...
				 	cvote.getValueFromSVChairman(meetingNameSt, itemNameSt, typeSt, 
				 		topicSt, itemNoSend, comboItemName, numChoice, valPoint, allChoice);
    		  }
