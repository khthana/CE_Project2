...
//			cout << ".";
			...
			{
				cout << "damage is: " << pack_recieve->blood << endl;
				char string_query[200];
				sprintf(string_query,"update `account` set `BLOOD`=`BLOOD`-'%f' where `USERNAME`='%d'",pack_recieve->blood,pack_recieve->number_mon);
				query(string_query);
			}
...
//				strcat(string_query,"\";");
			...
			else if(pack_recieve->str_query == "logout")
			{
				...
				cout << "Message Recieve is: " << pack_recieve->str_query << endl;
				cout << "username is: " << pack_recieve->username_login << endl;
				sprintf(string_query_logout,"update `account` set `LOGIN`='0',`POSITIONX`='%f',`POSITIONY`='%f',`ROTATE`='%f',`STATUS`='%d',`LEVEL`='%d',`STR`='%d',`DEX`='%d',`AGI`='%d',`HP`='%d',`BLOOD`='%f',`EXP`='%d',`MAP`='%d' where `USERNAME`='%s'",pack_recieve->positionX,pack_recieve->positionY,pack_recieve->rotate,pack_recieve->status,pack_recieve->lv,pack_recieve->str,pack_recieve->dex,pack_recieve->agi,pack_recieve->hp,pack_recieve->blood,pack_recieve->exp,pack_recieve->map,pack_recieve->username_login);
				database3.doData(string_query_logout);
				num_player--;
				if(num_player >= 0)
				{
					cout << "decrease player" << endl;
					char string_query_logout[500];
					sprintf(string_query_logout,"update `balance` set `server1`= `server1`-'1';");
					database3.doData(string_query_logout);
				}
			}
			else if(pack_recieve->str_query == "Play")
			{
				num_player++;
				if(num_player > 0)
				{
					cout << "add player" << endl;
					char string_query_play[200];
					sprintf(string_query_play,"update `balance` set `server1`= `server1`+'1';");
					database3.doData(string_query_play);
				}
			}
			break;
		}
...
//			MessageBox(NULL,"INDICATE_CONNECT","Server",MB_OKCANCEL);
			cout << "INDICATE_CONNECT" << "\tnum_player" << endl;
			break;
		}
...
int main2() throw()
{
	...
	flag_check = database4.connectDatabase();
		if(flag_check != 0) return 0;
	cout << "Connect Database Complete" << endl;
	if (flag_check != 0) {return -1;}
	database4.doData("update `balance` set `server1`= '0';");
	Init_Directplay();
	cout << "---------------------------------" << endl;
	MSG msg; 
	...
	while( msg.message!=WM_QUIT )
	{
		if(kbhit() != 0)
		{
			database4.doData("update `balance` set `server1`= '999';");
			TerminateDirectplay();
			cout << "Terminate" << endl;
			getch();
			return 0;
		...
		Sleep(time);
		database4.doData("select * from account where `LOGIN`!='0' and `MAP`='0';");
...
int main()
{
	...
	catch(exception e)
	{
		cout << "hhh" << endl;
		int flag_check;
		flag_check = database5.connectDatabase();
			if(flag_check != 0) return 0;
		database5.doData("update `balance` set `server1`= '999';");
		getch();
		return 0;
	}
	return 0;
