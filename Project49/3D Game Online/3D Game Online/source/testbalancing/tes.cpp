...
public:
	...
	{
		MYSQL_ROW row;
		while( row = mysql_fetch_row(database.getData()) ) 
		{
			if(strcmp(password_check,row[1]) == 0)
			{
				if(strcmp("0",row[2]) == 0)
				{
					...
					if(map == 0)
					{
						sprintf(string_query,"select * from balance");
						database2.doData(string_query);
						MYSQL_ROW row2;
						row2 = mysql_fetch_row(database2.getData());
						int serv1 = atoi(row2[0]);
						int serv2 = atoi(row2[1]);
						if(serv1 <= serv2)
						{
							size = 1;
						}
...
HRESULT WINAPI DirectPlayMessageHandlerServer( PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer )
{
	...
	switch (dwMessageId)
	{
		case DPN_MSGID_RECEIVE:
		{
			...
			if(pack_recieve->str_query == "login")
			{
...
//				strcat(string_query,"\";");
				...
				database.doData(string_query_login);
