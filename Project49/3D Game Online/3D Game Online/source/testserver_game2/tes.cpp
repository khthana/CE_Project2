...
//				strcat(string_query,"\";");
			...
			else if(pack_recieve->str_query == "logout")
			{
				...
				if(num_player >= 0)
				{
					...
					char string_query_logout[500];
					sprintf(string_query_logout,"update `balance` set `server3`= `server3`-'1';");
					database3.doData(string_query_logout);
				}
			}
			else if(pack_recieve->str_query == "Play")
			{
				...
				if(num_player > 0)
				{
					...
					sprintf(string_query_play,"update `balance` set `server3`= `server3`+'1';");
...
int main2() throw()
{
	...
	database4.doData("update `balance` set `server3`= '0';");
	Init_Directplay();
	cout << "---------------------------------" << endl;
	MSG msg; 
	...
	while( msg.message!=WM_QUIT )
	{
		if(kbhit() != 0)
		{
			database4.doData("update `balance` set `server3`= '999';");
			TerminateDirectplay();
			cout << "Terminate" << endl;
			getch();
			return 0;
		...
		Sleep(time);
		database4.doData("select * from account where `LOGIN`!='0' and `MAP`='1';");
...
int main()
{
	...
	catch(exception e)
	{
		...
		flag_check = database5.connectDatabase();
			if(flag_check != 0) return 0;
		database5.doData("update `balance` set `server3`= '999';");
		getch();
		return 0;
	}
	return 0;
