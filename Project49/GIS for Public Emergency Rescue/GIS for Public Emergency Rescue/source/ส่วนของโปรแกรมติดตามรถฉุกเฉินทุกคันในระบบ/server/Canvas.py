...
class TrackDraw:
    ...
    def __init__(self, LineStyle=None, Points= None, InForeground=True):
        ...
        self.txtrect = wx.Image( "picture/rect.png", wx.BITMAP_TYPE_PNG ).ConvertToBitmap()
    ...
    def SetDetail( self, flag ):
        self.flagDetail = flag
        
    def AddPoint(self, Point):
        """ Point is (x,y) """
        self.itype = []
        self.pfield = 5
        self.numberCar = int(len(Point)/self.pfield)
        self.Points = None
        for i in range(len(Point)/self.pfield):
            
            if self.Points is None:
                self.Points = array( [(float(Point[2]),float(Point[3]))], Float)
                self.itype.append([Point[0],Point[1],Point[4]]) 
            else:
                self.Points = concatenate( ( self.Points, array( [(float(Point[i*self.pfield+2]),float(Point[i*self.pfield+3]))], Float) ) )
                self.itype.append([Point[i*self.pfield],Point[i*self.pfield+1],Point[i*self.pfield+4]])
...
##                    ccolor = "RED"
                ...
                #XY = WorldToPixel( self.Points[-1+(i*-1)] )
##                dc.SetBrush( wx.Brush( "RED" ) )
##                dc.DrawCircle( XY[0], XY[1], self.cccr )
                
                if self.flagDetail == 'Show':
                    dc.DrawBitmap(self.txtrect,XY[0]+5, XY[1]+5)
                    if self.itype[-1+(i*-1)][0] == '00000001':
                        dc.DrawText("ประเภท : รถพยาบาล",XY[0]+20, XY[1]+10)
                        dc.DrawText("รหัสรถ : "+self.itype[-1+(i*-1)][1],XY[0]+20, XY[1]+25)
                        #dc.DrawBitmap(self.hicon,XY[0], XY[1])
                    elif self.itype[-1+(i*-1)][0] == '00000002': 
##                        if jhg:
                            dc.DrawText("ประเภท : รถตำรวจ",XY[0]+20, XY[1]+10)
                            dc.DrawText("รหัสรถ : "+self.itype[-1+(i*-1)][1],XY[0]+20, XY[1]+25)                  

                            
                        #dc.DrawBitmap(self.picon,XY[0], XY[1])
                    elif self.itype[-1+(i*-1)][0] == '00000003': 
                        dc.DrawText("ประเภท : รถดับเพลิง",XY[0]+20, XY[1]+10)
                        dc.DrawText("รหัสรถ : "+self.itype[-1+(i*-1)][1],XY[0]+20, XY[1]+25)                   
                        #dc.DrawBitmap(self.ficon,XY[0], XY[1])
...
class RequestPoint(TrackDraw):
    def __init__( self, LineStyle=None, Points= None, InForeground=True):
        ...
        self.fire = wx.Image( "picture/fire.png", wx.BITMAP_TYPE_PNG ).ConvertToBitmap()
        self.gun = wx.Image( "picture/gun.png", wx.BITMAP_TYPE_PNG ).ConvertToBitmap()
    
    def AddPoint(self, Point):
        """ Point is (x,y) """
        self.nfield = 4
        self.numberReq = int(len(Point)/self.nfield)
        self.reqType = []
        self.Points = None
        for i in range(len(Point)/self.nfield):
            
            if self.Points is None:
                self.Points = array( [(float(Point[2]),float(Point[3]))], Float)
                self.reqType.append(Point[0]) 
            else:
                self.Points = concatenate( ( self.Points, array( [(float(Point[i*self.nfield+2]),float(Point[i*self.nfield+3]))], Float) ) )
                self.reqType.append(Point[i*self.nfield])
            self.CalcBoundingBox()
    ...
    def _Draw(self, dc, WorldToPixel, ScaleWorldToPixel, BBCheck):
        ...
        if self.BoundingBox is not None:            
            if self.pointStatus == 0 :
                ...
                for i in range(self.numberReq):
                    XY = WorldToPixel(self.Points[-1+(i*-1)])
                    if self.reqType[-1+(i*-1)] == '00000001':                        
                        dc.DrawBitmap(self.requester,XY[0]-18, XY[1]-19)
                    elif self.reqType[-1+(i*-1)] == '00000002':
                        dc.DrawBitmap(self.gun,XY[0]-18, XY[1]-19)
                    elif self.reqType[-1+(i*-1)] == '00000003':
                        dc.DrawBitmap(self.fire,XY[0]-18, XY[1]-19)
