...
class FloatCanvas(wx.Panel):
    ...
    def __init__(self, parent, id = -1,
        ...
        self.SetInvProjectionFun(InvProjectionFun)
...
###            return ObjectCallbackCalled
    ...
    def RightUpEvent(self,event):
        if self.GUIMode == "Mouse":
            ...
            self._RaiseMouseEvent(event, EVT_FC_RIGHT_UP)
...
###           self._ForegroundHTdc = None 
    ...
    def OnSize(self,event):
        ...
        self.HalfPanelSize = self.PanelSize / 2.0 # lrk: added for speed in WorldToPixel
    ...
    def OnPaint(self, event):
        ...
        if self._ForegroundBuffer:
            ...
            try:
                wx.BufferedPaintDC(self, self._ForegroundBuffer)
            except:
                try:
    ...
    def Draw(self, Force=False):
        ...
        if self._BackgroundDirty or Force:
            ...
            self._DrawObjects(
                                dc, 
                                self._ShouldRedraw(self._DrawList, ViewPortBB), 
                                ScreenDC, 
                                None)
        ...
        if self._ForeDrawList:
            ...
            self._DrawObjects(
                                dc, 
                                ##self._ShouldRedraw(self._ForeDrawList, ViewPortBB), 
                                self._ForeDrawList,
                                ScreenDC, 
                                None)
    ...
    def _DrawObjects(self, dc, DrawList, ScreenDC, HTdc = None):
        """
        ...
        ###for i, Object in enumerate(DrawList):
...
##            return False
    ...
    def SetInvProjectionFun(self, InvProjectionFun):
        if InvProjectionFun == 'FlatEarth':
            self.InvProjectionFun = self.InvFlatEarthProjection 
        elif type(InvProjectionFun) == types.FunctionType:
            self.InvProjectionFun = InvProjectionFun 
        elif InvProjectionFun is None:
            self.InvProjectionFun = lambda x: x
        else:
            raise FloatCanvasException('InvProjectionfun must be either: "FlatEarth", None, or a function that takes the ViewPortCenter and returns a MapProjectionVector')

    def FlatEarthProjection(self, Coords):
        points = array( Coords, Float)
        if len(points.shape) < 2 :
            points[0] = points[0] * cos(points[1]*degTorad)  
        else:
            points[:,0] = points[:,0] * cos(points[:,1]*degTorad) 
        return points
        #return array((cos(radians(CenterPoint[1])),1),Float)
    
    def InvFlatEarthProjection(self, Coords):
        points = array( Coords, Float )
        #print points
        if len(points.shape) < 2 :
            points[0] = points[0] / cos(points[1]*degTorad) 
    ...
    def Move(self,shift,CoordType):
        ...
        if CoordType == 'Panel':# convert from panel coordinates
            shift = self.InvProjectionFun(shift * array((-1,1),Float) *self.PanelSize)/self.TransformVector
        elif CoordType == 'Pixel': # convert from pixel coordinates
            shift = self.InvProjectionFun(shift)/self.TransformVector
    ...
    def Zoom(self,factor,center = None):
        ...
        if not center is None:
            self.ViewPortCenter = asarray(center,Float)
        ## OLD
        ##self.MapProjectionVector = self.ProjectionFun(self.ViewPortCenter)
        ##self.TransformVector = array((self.Scale,-self.Scale),Float) * self.MapProjectionVector
        
        ## NEW
        self.TransformVector = array((self.Scale,-self.Scale),Float) #* self.ViewPortCenter
    ...
    def ZoomToBB(self, NewBB = None, DrawFlag = True):
        ...
        if not BoundingBox is None:
            ...
            #BoundingBox = self.ProjectionFun(BoundingBox)
            try:
            ...
            self.ViewPortCenter= array( (0,0), Float)
            ## OLD self.MapProjectionVector = array( (1,1), Float) # No Projection to start!
            self.TransformVector = array( (1,-1), Float) # default Transformation
            self.Scale = 1
        #wx.EVT_MOUSEWHEEL(self, self.WheelEvent )
    ...
    def PixelToWorld(self,Points):
        ...
        """
        ##return  (((asarray(Points,Float) - (self.HalfPanelSize))/self.TransformVector) + self.ViewPortCenter)
        return  (self.InvProjectionFun((asarray(Points,Float) - (self.HalfPanelSize))/self.TransformVector) + self.ViewPortCenter)
    ...
    def WorldToPixel(self,Coordinates):
        ...
        return ((self.ProjectionFun(asarray(Coordinates,Float) - self.ViewPortCenter) * self.TransformVector) + (self.HalfPanelSize)).astype('i')
...
##        #NumBetweenBlits = self.NumBetweenBlits # for speed
##        #for i, Object in enumerate(self._ShouldRedraw(DrawList, ViewPortBB)):
##        Object._Draw(dc, WorldToPixel, ScaleWorldToPixel, self._HTdc)
##        #if i % NumBetweenBlits == 0:
##        Blit(0, 0, PanelSize0, PanelSize1, dc, 0, 0)
##        dc.EndDrawing()
##        if self.PrevRBBox:
