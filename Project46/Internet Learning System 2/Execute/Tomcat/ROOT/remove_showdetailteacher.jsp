...
<%
		...
		return;
		   ...
		   int myresult=0;		
			...
			String sqla = "SELECT * FROM teacher_personal where name='"+key1+"'";
			ResultSet rs = stmt.executeQuery(sqla);
				while(rs.next())
			{	
				if (rs.getString("name").equals(key1))	
				{	
						Class.forName("org.gjt.mm.mysql.Driver");		
						Connection mycon2 = 				DriverManager.getConnection("jdbc:mysql://localhost/info?user=root");
			            Statement stmt2=mycon2.createStatement();
						String sql2 = "DELETE FROM teacher_personal WHERE name='"+key1+"'";
						myresult = stmt2.executeUpdate(sql2);
						stmt2.close();
						mycon2.close();
					      String name1 = new String(key1.getBytes("ISO8859_1"),"MS874");
						out.println("Delete teacher "+name1+" complete <br>รายละเอียดดู <a href='show_afterremove.jsp'  onClick='return popup(this, 'show_afterremove')'>ดูได้ที</a>" );
				}
