package src;
//This file is contains what client will do when services called
import src.Data;
import src.Subjob;
...
public class Client{
  ...
  public static void main (String args[]) {
    ...
    String file2 = args[1];
    String file3 = args[2];
int inpt4 = Integer.parseInt(args[3]);
...
//      Data.print_all();
      ...
      Subjob[] s = new Subjob[inpt4]; 
      //for(int i = 0; i < 10 ;i++)
	//    s[i] = null;
      int i = 0;
      int j = 0;
      int jobPointer = 0;
      while(i<client.rowm1)
      {
    	  while(j<client.colm1)
    	  {
    		  if(s[jobPointer] != null)
    		  {
			if(s[jobPointer].isAlive() == false)
			{
			System.out.println("thread No."
					   +jobPointer+" At row "+s[jobPointer].resultrow()
					   +" Col "+s[jobPointer].resultcol()
					   +" Completed and Result = "
					   +s[jobPointer].resultvalue());
				s[jobPointer] = new Subjob(i,j,machine);
				s[jobPointer].start();
				j++;
	System.out.println("					 "+i+" " +j+" At machine= "+machine+" At thread No."+jobPointer);
				
				if (machine ==1){ machine = 2;}
				else if (machine ==2){ machine = 3;}
				else if (machine ==3){ machine = 4;}
				else if (machine ==4){ machine = 5;}
   ...
   /*   for (int i=0;i<client.rowm1;i++)
        for (int j=0;j<client.colm2;j++)  {
          // choose machine to execute subjob seperated to each machine in same priority
//          if (machine ==1){ machine = 2;}
//          else if (machine ==2){ machine = 3; }
//          else { machine = 1; };
...
*/
	...
	//System.out.println("Data.checkcom"+" "+tmp );
      }while (tmp != true);
	for(int k = 0 ; k < inpt4 ; k++)
			System.out.println("thread No."
                                           +k+" At row "+s[k].resultrow()
                                           +" Col "+s[k].resultcol()
                                           +" Completed and Result = "
                                           +s[k].resultvalue());
