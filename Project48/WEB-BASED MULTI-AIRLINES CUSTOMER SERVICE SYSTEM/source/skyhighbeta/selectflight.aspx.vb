...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents BtnSortReturn As System.Web.UI.WebControls.Button
...
#End Region
    ...
    Private Sub Booking()
        ...
        Dim CabinOne As String = radCabinOne.Items(radCabinOne.SelectedIndex).Value
        Dim CabinReturn As String = radCabinReturn.Items(radCabinReturn.SelectedIndex).Value
        Dim WebFareOne As String = radWebFareOne.Items(radWebFareOne.SelectedIndex).Value
        Dim WebFareReturn As String = radWebFareReturn.Items(radWebFareReturn.SelectedIndex).Value

        '++++Send info to make new booking for each-airline & skyhigh==========================
        Dim NewBookingSet As DataSet = Airline.MakeBookingSet(TxID, Session("BookRef"), Session("FID"), Session("FID2"), Session("FID3"), Request("DeptMDY"), Session("DeptMDY2"), Session("DeptMDY3"), Session("ReFID"), Session("ReFID2"), Session("ReFID3"), Request("ReMDY"), Session("ReMDY2"), Session("ReMDY3"), CabinOne, WebFareOne, CabinReturn, WebFareReturn)
        Dim NewBookDetailsString As String = "INSERT INTO bookeddetails VALUES ('" & TxID & "','" & Session("BookRef") & "','" & Session("MemberID") & "'," & Request("Adult") & "," & Request("Child") & "," & CalculateTotalCost() & ",'N','" & BookDateTime & "','0')"

        '++++Changing old flight and booking new flight for.....each-airline & skyhigh=============================
        Dim AllChanging As New SkyHighTP.TwoPhaseWS
        If AllChanging.Changing(Session("BookRef"), NewBookingSet, NewBookDetailsString) = True Then

            '+++Sending e-mail to our customer============================
            Dim QueryString As String = "SELECT email FROM member WHERE memberid='" & Session("MemberID") & "'"
            Dim EMail As String = Airline.Query(QueryString).Tables(0).Rows(0).Item("email")

            Dim Content As String = "Your Booking Reference is " & Session("BookRef") & "<br>Changing old flight and booking new flight is succesful.<br>See your trip at www.skyhigh.com<br>...Thank you for visit us."
            Airline.MailTo(EMail, "SkyHigh Changing Flight", Content)

            '++++Update Seat of SkyHigh=========================================
            Dim UpdateSeatSet As DataSet = Airline.MakeUpdateSeatSet(Session("BookRef"), "-")
            Dim eSeatAvai As New SkyHighSeat.SeatbySkyHigh
            If eSeatAvai.SeatAvai(UpdateSeatSet) = True Then
    ...
    Private Sub SelectingReturn3(ByVal source As System.Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles grdReturn3.ItemCommand
    ...
    Function CalculateTotalCost() As Integer
        ...
        Dim FareDataSet As DataSet = Airline.ShowItinerary(Session("FID"), Session("FID2"), Session("FID3"), Request("DeptMDY"), Session("DeptMDY2"), Session("DeptMDY3"), Session("ReFID"), Session("ReFID2"), Session("ReFID3"), Request("ReMDY"), Session("ReMDY2"), Session("ReMDY3"), CabinOne, CabinReturn, WebFareOne, WebFareReturn)
        FareDataSet = Airline.ShowFare(FareDataSet, Request("Adult"), Request("Child"), CabinOne, WebFareOne, CabinReturn, WebFareReturn)
        Dim TotalCost As Integer = FareDataSet.Tables(0).Rows(0).Item("AllTotal")
    ...
    Private Sub btnBack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBack.Click
        Response.Redirect("itineray.aspx")
    End Sub

    Private Sub BtnSortCost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        SortCost()
    End Sub
    ...
    Private Sub SortCost()
        If Request("ReMDY") = "null" Then
            Response.Redirect("selectflight.aspx?DeptMDY=" & Request("DeptMDY") & _
                                "&From=" & Request("From") & _
                                "&To=" & Request("To") & _
