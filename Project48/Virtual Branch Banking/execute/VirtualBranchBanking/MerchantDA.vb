'*********************************************************************************
'Class MerchantDA  : Definition Class for Database connection to merchant table  *
'*********************************************************************************
Imports System.Data
...
Public Class MerchantDA
    ...
    Public Function MerchantInsert(ByVal mer As Merchant) As Integer
        ...
        Else
            ...
            If dsMerInsert.Tables("mem_insert").Rows.Count = 0 Then
                oDR.Item("MERCHANT_ID") = 1
            Else
                oDR.Item("MERCHANT_ID") = dsMerInsert.Tables("mer_insert").Rows(0).Item("MERCHANT_ID") + 1
            End If
    ...
    End Function
    '*******************************************************************
    Public Function MerSelectUniDataset() As DataSet
        Dim invalidStr As String = ""
        Dim Oradb As String = "Data Source=(DESCRIPTION=(ADDRESS_LIST=" _
                           + "(ADDRESS=(PROTOCOL=TCP)(HOST=thawatchai)(PORT=1521)))" _
                          + "(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=ORCL)));" _
        ...
        Dim Conn As New OracleConnection(Oradb) ' VB.NET
        Dim sqlMerSelectDataset As String = "SELECT * FROM MERCHANT WHERE MERCHANT_TYPE LIKE 'Education' ORDER BY MERCHANT_ID" '"
        Dim daMerSelectDataset As OracleDataAdapter = New OracleDataAdapter(sqlMerSelectDataset, Conn)
        Dim dsMerSelectDataset As New DataSet("mer_select_dataset")
        daMerSelectDataset.Fill(dsMerSelectDataset, "mer_select_dataset")
