...
//#define REQ_FILE "/root/careClient/clientreq3.pem"
//#define CERT_FILE "/root/careClient/clientcert3.pem"
...
void create_cert(char username[255])
{
		sprintf(REQ_FILE,"[CR]%s.pem",username);
  sprintf(CERT_FILE,"[CE]%s.pem",username);
...
/* read in the CA private key */
  ...
  if (!(CApkey = PEM_read_PrivateKey (fp, NULL, NULL, "care"))) // put PEM pass phase of RootCA
...
/* sign the certificate with the CA private key */
  ...
  if (EVP_PKEY_type (CApkey->type) == EVP_PKEY_RSA)
    digest = EVP_sha1 ();
  else  if (EVP_PKEY_type (CApkey->type) == EVP_PKEY_DSA)
    digest = EVP_dss1 ();
  else
