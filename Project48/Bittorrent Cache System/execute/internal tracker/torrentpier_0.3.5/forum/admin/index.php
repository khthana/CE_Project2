		...
		for($i = 0; $i < count($onlinerow_reg); $i++)
		{
			if( !inarray($onlinerow_reg[$i]['user_id'], $reg_userid_ary) )
			{
				...
				if( $onlinerow_reg[$i]['user_session_page'] < 1 )
				{
					switch($onlinerow_reg[$i]['user_session_page'])
					{
						...
						case PAGE_TRACKER:
							...
							$location_url = "tracker.$phpEx?pane=right";
							break;
						//bt end
				...
				$template->assign_block_vars("reg_user_row", array(
					...
					"ROW_CLASS" => $row_class,
					"USERNAME" => $username,
					"STARTED" => create_date('H:i', $onlinerow_reg[$i]['session_start'], $board_config['board_timezone']),
					"LASTUPDATE" => create_date('H:i', $onlinerow_reg[$i]['user_session_time'], $board_config['board_timezone']),
	...
	if( count($onlinerow_guest) )
	{
		...
		for($i = 0; $i < count($onlinerow_guest); $i++)
		{
			...
			$template->assign_block_vars("guest_user_row", array(
				...
				"ROW_CLASS" => $row_class,
				"USERNAME" => $lang['Guest'],
				"STARTED" => create_date('H:i', $onlinerow_guest[$i]['session_start'], $board_config['board_timezone']),
				"LASTUPDATE" => create_date('H:i', $onlinerow_guest[$i]['session_time'], $board_config['board_timezone']),
				"FORUM_LOCATION" => $location,
				"IP_ADDRESS" => $guest_ip,
...
////		"U_WHOIS_IP" => "http://network-tools.com/default.asp?host=$guest_ip",
	...
	if ($fsock /* = @fsockopen('www.phpbb.com', 80, $errno, $errstr) */ )
	{
		...
		else
		{
			...
			$version_info .= '<br />' . sprintf($lang['Latest_version_info'], $latest_version) . '<br />' . sprintf($lang['Current_version_info'], '2' . $board_config['version']) . '</p>';
		}
	}
	else
	{
