...
#End Region
    ...
    Private Function EditClik() As Boolean
        Dim sql As String = ""
        sql = "SELECT a.HID, a.H_TRADE, a.H_STATUS, b.DAYS, b.AGES, b.BODYS, b.H_Hospt from Health a ,Date b "
        sql = sql + "where a.H_DAY = b.H_DAY and a.H_DAY = " + CStr(H_DAY)
        ...
        TradeCur.Text = CStr(ds.Tables("select").Rows(0).Item("H_TRADE"))
        CboStateCur.Text = CStr(ds.Tables("select").Rows(0).Item("H_STATUS"))
        HospitalCur.Text = CStr(ds.Tables("select").Rows(0).Item("H_Hospt"))
    ...
    Private Sub BSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BSave.Click
        ...
        If MessageBox.Show("ต้องการเพิ่มข้อมูลสุขภาพลงในฐานข้อมูล", "คำยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            If checkitem() Then
                sql = "insert into Date(H_DAY,DAYS,AGES,BODYS,H_Hospt)values("
                sql = sql + ID + ",'"
                ...
                sql = "insert into Health(H_NUMBER,H_DAY,H_TRADE,H_STATUS,HID)values("
                sql = sql + H_number + ","
                sql = sql + ID + ",'"
    ...
    Private Function genId() As Integer
        ...
        If ds.Tables("gen").Rows.Count <> 0 Then
            sql = "select max(H_DAY) as MAXExp from Date"
            da = New SqlDataAdapter(sql, Conn)
            ds.Tables.Clear()
            da.Fill(ds, "gen")
            If ds.Tables("gen").Rows.Count <> 0 Then
    ...
    Private Sub BEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BEdit.Click
        ...
        If MessageBox.Show("ต้องการแก้ไบข้อมูลใหม่", "คำยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            If checkitem() Then
                sql = "Update Date set AGES = " + AgeCur.Text + ","
                sql = sql + "BODYS = '" + BodyCur.Text + "',"
                sql = sql + "H_Hospt = '" + HospitalCur.Text + "' "
                sql = sql + "where H_DAY = " + CStr(H_DAY)
                da = New SqlDataAdapter(sql, Conn)
                ds.Tables.Clear()
                da.Fill(ds, "Update")

                sql = "Update Health set H_TRADE = '" + TradeCur.Text + "',"
                sql = sql + "H_STATUS = '" + CboStateCur.Text + "',"
