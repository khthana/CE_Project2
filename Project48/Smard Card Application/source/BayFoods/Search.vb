...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'CboType
        '
        Me.CboType.Items.AddRange(New Object() {"รหัสอาหาร", "ชื่ออาหาร", "ราคาอาหาร", "รหัสนักศึกษา", "ชื่อนักศึกษา", "นามสกุลนักศึกษา"})
        ...
        Me.SearchShow.Size = New System.Drawing.Size(776, 376)
        ...
        Me.BnExit.Location = New System.Drawing.Point(680, 480)
        ...
        Me.PrPreView.Location = New System.Drawing.Point(552, 480)
...
#End Region
    ...
    Private Sub BnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BnSearch.Click
        ...
        If DateStart.Value > DateEnd.Value Then
            ...
            Exit Sub
        End If
        Dim sql As String = "Select a.Std_ID,a.Name,a.SurName,a.DateCurrent,b.FoodID,b.FoodName,b.FoodCost from BayFoods a,Foods b "
        sql = sql + " Where a.FoodID = b.FoodID"
        PrCondition = " Where a.FoodID = b.FoodID"
        If TextStatus Then
            Select Case CboType.Text
                Case "รหัสอาหาร"
                    If IsNumber() Then
                        sql = sql + " and b.FoodID =" + TxtData.Text
                        PrCondition = PrCondition + " and b.FoodID =" + TxtData.Text
                    Else
                        MessageBox.Show("ใส่ข้อมูลไม่ตรงกับ ประเภทที่เลือก กรุณาทำการเลือกใหม่ ", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    End If
                Case "ชื่ออาหาร"
                    sql = sql + " and b.FoodName LIKE '%" + TxtData.Text + "%'"
                    PrCondition = PrCondition + " and b.FoodName LIKE '%" + TxtData.Text + "%'"
                Case "ราคาอาหาร"
                    If IsNumber() Then
                        sql = sql + " and b.FoodCost =" + TxtData.Text
                        PrCondition = PrCondition + " and b.FoodCost =" + TxtData.Text
                    Else
                        MessageBox.Show("ใส่ข้อมูลไม่ตรงกับ ประเภทที่เลือก กรุณาทำการเลือกใหม่ ", "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    End If
                Case "รหัสนักศึกษา"
                    sql = sql + " and a.Std_ID LIKE '%" + TxtData.Text + "%'"
                    PrCondition = PrCondition + " and a.Std_ID LIKE '%" + TxtData.Text + "%'"
                Case "ชื่อนักศึกษา"
                    sql = sql + " and a.Name LIKE '%" + TxtData.Text + "%'"
                    PrCondition = PrCondition + " and a.Name LIKE '%" + TxtData.Text + "%'"
                Case "นามสกุลนักศึกษา"
                    sql = sql + " and a.SurName LIKE '%" + TxtData.Text + "%'"
                    PrCondition = PrCondition + " and a.SurName LIKE '%" + TxtData.Text + "%'"
            End Select
        End If

        sql = sql & " and (a.DateCurrent BETWEEN '" & DateStart.Value.ToString("d", InfoDate)
        sql = sql & "' and '" & DateEnd.Value.ToString("d", InfoDate) & "')"
        PrCondition = PrCondition & " and (a.DateCurrent BETWEEN '" & DateStart.Value.ToString("d", InfoDate)
        PrCondition = PrCondition & "' and '" & DateEnd.Value.ToString("d", InfoDate) & "')"
        FormatDataGrid(sql)
