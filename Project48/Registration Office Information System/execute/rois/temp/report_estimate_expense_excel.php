...
$fname = tempnam("/tmp", "estimate_expense.xls");
...
# Title
#
$worksheet1->write('B1', "รายงานประมาณการรายจ่าย ประจำปี ". $year, $main_head);
																	...
																	while($object_expense_type_sub = mysql_fetch_array($ret_expense_type_sub)) {																				
																			...
																			while($object_expense_sub = mysql_fetch_array($ret_expense_sub)) {
																				...
																				if( ($temp != $object_expense_sub["EXPENSE_SUB_ID"]) || ($object_account["ID"] == '51000') ) {
																					...
																					// print money gov																										
																					$amount_gov = mysql_fetch_array(database_query("select sum(amount) from estimate_expense where side_id = '".$side."' and expense_sub_id = '".$object_expense_sub["EXPENSE_SUB_ID"]."' and year = '".$year."' and project_id = '".$object_project["ID"]."' and activity_id = '".$object_activity["ID"]."' and fund_id = '".$object_fund["ID"]."' and money_type_id = '1'"));
 	       																		$worksheet1->write($line_num, 1, $amount_gov[0], $normal_right_format);
 	       																		
				
																					// print money ins																										
																					$amount_ins = mysql_fetch_array(database_query("select sum(amount) from estimate_expense where side_id = '".$side."' and expense_sub_id = '".$object_expense_sub["EXPENSE_SUB_ID"]."' and year = '".$year."' and project_id = '".$object_project["ID"]."' and activity_id = '".$object_activity["ID"]."' and fund_id = '".$object_fund["ID"]."' and money_type_id = '2'"));
        																			$worksheet1->write($line_num, 2, $amount_ins[0], $normal_right_format);
...
header("Content-Type: application/x-msexcel; name=\"report_estimate_expense.xls\"");
header("Content-Disposition: inline; filename=\"report_estimate_expense.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
