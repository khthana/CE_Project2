<?php
	...
	if($mode == "edit") {
		...
		if($opass["PASSWORD"] == md5($old_pass)) {
			$str = "UPDATE `user_profile` SET `FIRST_NAME` = '".$first_name."',`LAST_NAME` = '".$last_name."',`USER_NAME` = '".$user_name."',`PASSWORD` = '".md5($new_pass)."',`STATUS` = '".$status."' WHERE `ID` = ".$id."";
		...
		$str = "INSERT INTO `user_profile` ( `ID` , `FIRST_NAME` , `LAST_NAME` , `USER_NAME` , `PASSWORD` , `STATUS` ) VALUES ('', '".$first_name."', '".$last_name."', '".$user_name."', '".md5($pass)."', '".$status."')";
		if(database_query($str)) {
