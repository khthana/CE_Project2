...
$normal_center_format = & $workbook->addformat(array('font' => 'AngsanaNew' , 'size' => 10 , 'align' => 'center'));
...
# Title
#
$worksheet1->write('B1', "รายงานรายจ่ายจริง ประจำปี ". $year, $main_head);
...
$worksheet1->write('D4', "วันที่", $center_bold_10);
$worksheet1->write('D4', "อ้างอิง", $center_bold_10);
...
# ฝ่าย
#
	$side_txt = mysql_fetch_array(database_query("select * from side where id = '".$side."'"));		
	$worksheet1->write($line_num, 0, $side_txt["SIDE"], $left_bold_10);
								...
								$str_activity_sub = "select * from activity_sub where id = any".
									...
									while($object_activity_sub = mysql_fetch_array($ret_activity_sub)) {
											if($object_activity_sub[0] != '00') {
													...
													// print activity
													$worksheet1->write($line_num, 0, $object_activity_sub["ACTIVITY_SUB"], $left_bold_10);
											...
											while($object_account = mysql_fetch_array($ret_account)) {
													...
													while($object_expense_type = mysql_fetch_array($ret_expense_type)) {
															...
															if($object_expense_type["ID"] != "00000" && $object_expense_type["ID"] != "00001" && $object_expense_type["ID"] != "00002") {
																	...
																	// print expense type
																	$worksheet1->write($line_num, 0, "                 " . $object_expense_type["EXPENSE_TYPE"], $left_bold_10);
															...
															$str_expense_type_sub ="select * from expense_type_sub where id = any".
																	...
																	while($object_expense_type_sub = mysql_fetch_array($ret_expense_type_sub)) {
																			...
																			if($object_expense_type_sub["ID"] != "00000" && $object_expense_type_sub["ID"] != "00001" && $object_expense_type_sub["ID"] != "00002" && $object_expense_type_sub["ID"] != "00003" && $object_expense_type_sub["ID"] != "00004" && $object_expense_type_sub["ID"] != "00005" && $object_expense_type_sub["ID"] != "00006") {
																					...
																					$worksheet1->write($line_num, 0, "                     " . $object_expense_type_sub["EXPENSE_TYPE_SUB"], $left_bold_10);
																			...
																			while($object_expense_sub = mysql_fetch_array($ret_expense_sub)) {
																					...
																					// print money all		
																					$worksheet1->write($line_num, 3, $int, $normal_right_format);																				
																					
																					// print date
																					$worksheet1->write($line_num, 4, $object_expense_sub["DATE"], $normal_center_format);
																					
																					// print ref
																					$worksheet1->write($line_num, 5, $object_expense_sub["REF_DOCUMENT"], $normal_center_format);
