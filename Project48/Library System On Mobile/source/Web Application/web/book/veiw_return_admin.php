...
border=1>
  ...
  <TR>
    <TD bgColor=#f0f0f0><FONT face=Verdana size=2></FONT> <DIV align=right><!-- <b>วันนี้วันที่:</b> --> <?  
    ...
    <OPTION  value = year_print>ปีที่พิมพ์!!</OPTION>
	 <OPTION  value = price>ราคา!!</OPTION>
      ...
      src="webboard/line1.gif"> <BR>
       [ <A href="index.php" target=_parent>Home</A> ] [ <A href="formaddbook.htm?login_name=<?echo"$login_name";?>&login_password=<?echo"$login_password";?>">เพิ่มหนังสือใหม่</A>] 
      [ <A  href="borrow.htm">ยืมหนังสือ</A>] [ <A  href="return.htm">คืนหนังสือ</A>] [ <A  href="book_reserv.htm">จองหนังสือ</A>]
      [ <A  href="veiw_return_admin.php">ดูหนังสือที่ถูกยืมไปแล้ว</A>] [ <A  href="formmember.htm">สมัครสมาชิกใหม่</A>] 
      [<a href="list_guestbook.php" target="_self">แสดงข้อมูลนำเสนอ</a>] 
...
$q_sql = "select * from book  where name_borrow !=''";
...
$sql_total_book = "select * from book ";
	$db_query_total_book = mysql_db_query ($dbname, $sql_total_book );
	$num_rows_total_book = mysql_num_rows($db_query_total_book);
...
?>
          <TD>จำนวนหนังสือที่แสดง : <B><FONT color=#3399cc><?echo"$pagesize\n";?></FONT></B> 
            เล่ม/หน้า -  หนังสือทั้งหมดมี : <B><?echo"$num_rows_total_book\n";?></B>  เล่ม -  หนังสือที่ถูกยืมทั้งหมดมี : <B><?
...
<TABLE borderColor=#cccccc cellSpacing=0 cellPadding=2 width="100%" border=1>
  ...
  <TR bgColor=#cce9fd>
	...
	<TD align=middle width="20%"><FONT color=#000000 size=1><B>สำนักพิมพ์ </B></FONT></TD>
...
$sql_borrow_view = "select * from book where name_borrow !=''";
	$db_query_borrow_view = mysql_db_query ($dbname, $sql_borrow_view );
...
$swap="#fafafa";
					...
					for ($i=0;$i<$nums_rows;$i++)	// อ่านข้อมูลแต่ละเรกคอร์ด
						{
							$result = mysql_fetch_array($db_query_borrow_view);
...
echo"<TD width=35%><DIV align=center><FONT color=#000000 size=1><A href=view_addmin.php?id=$id target=_blank>$name_book  </A></font></div></TD>\n";
...
echo"<TD width=10%><div align=center><FONT color=#000000 size=1>$return_book</font></div> \n";
