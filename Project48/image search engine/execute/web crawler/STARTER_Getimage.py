...
def getimage (e):
    try:
        ...
        for line in urllib2.urlopen(e):
            ...
            if m:
                match_jpg = m.group() 
                #match_jpg_name = match_jpg[5:-1]
                match_jpg = replace(match_jpg,"""src=""","""""")
                match_jpg = replace(match_jpg,"""\"""","""""")
                match_jpg = replace(match_jpg,"""\'""","""""")
                fullurl = urljoin(site,match_jpg)
                #print fullurl
...
def thread (site):
    try:
        #cursor.execute("TRUNCATE TABLE `table_id_process`")
        ...
        if xx == -1:
            ...
            getmax_id = cursor.fetchone()

            cursor.execute("SELECT `stop` FROM `table_id_process` WHERE id = '%s'" %(getmax_id[0]))
            stop_id = cursor.fetchone()

            # get and display one row at a time
            aaa = 1
            for record in range(0,stop_id[0]):
                follow_level_1(aaa)
                aaa=aaa+1
                sleep(0.03)
