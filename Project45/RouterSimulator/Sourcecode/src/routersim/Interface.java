...
public class Interface implements java.io.Serializable{
  ...
  private Wire wireConnected = null ; // -- connect with thaht wire ( tobe null with no wire connected )
  //---------------------- Access List ---------------------------------------------------
  private String accessNumber = "";   //-- access list type number ( -1 is no has any type on this interface )
  private String condition = "";   //-- { in | out }

  public String getAccessListNumber(){
    return accessNumber;
  }

  public void setAccessListNumber( String i ) {
    accessNumber = i ;
  }

  public void setConditionAccessList( String s ){
    condition = s;
  }

  public String getConditionAccessLsit( ) {
    return condition ;
  }
  //-------Set Bandwidth------//
  ...
  //------Set passive----//
 public void setPassive(boolean t){
   passive = t;
 }
...
//    if (){
      for (int a=0; a < NewRouter.vRouter.size(); a++){
        Router newr = (Router)NewRouter.vRouter.elementAt(a);
        j=0;
...
//  if ((r.getEth() <= r.getMaxEth()) && (r.getSerial() <= r.getMaxSerial()) && (r.getToken() <= r.getMaxToken())){
      ...
      for(int k=0;k<NewRouter.vRouter.size();k++)
      {
        Router rt = (Router)NewRouter.vRouter.elementAt(k);
        if(rt.getName().equalsIgnoreCase(r.getName()))
        {
          rt.getVInt().addElement(newInt);
          System.out.print("Vintsize_inadd: "+rt.getVInt().size());
        }// end if
      }// end for*/
    }
  else {
    switch(s.charAt(0)){
      case 'e' : r.setEth(r.getEth()-1);output+= "Ethernet ";break;
      case 's' : r.setSerial(r.getSerial()-1);output+= "Serial ";break;
      case 't' : r.setToken(r.getToken()-1);output+= "Token Ring ";break;
      default : break;
    }//end switch
    output += "has limited\n";
  }
