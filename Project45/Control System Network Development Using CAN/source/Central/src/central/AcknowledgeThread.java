...
class AcknowledgeThread
  ...
  public void run() {
    while (true) {
      try {
        highbuffer = MainFrame.RAx[2].getShareData();                           // RAx number 4 for only Acknowledge Thread
  ...
  public void setAndCheckAck(int ID, int acknum) {
    if (ID == 1 || ID == 2 || ID == 3) {
      if ((acknum - (sequence[ID - 1].getValue())) == 1) {
        ...
        MainFrame.messagestatus.setText(MainFrame.messagestatus.getText() + "Bad Sequence Number Message ID " + ID + "\n" + "Replace " + (sequence[ID - 1].getValue() + 1) + " With " + acknum + "\n");
        sequence[ID - 1].setValue(acknum);
      }
    }
  }
