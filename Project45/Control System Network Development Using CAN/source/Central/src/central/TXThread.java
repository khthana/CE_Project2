...
class TXThread extends Thread{
  ...
  private SequenceCounter sequenceNumber[] = {new SequenceCounter(), new SequenceCounter(), new SequenceCounter()};
        ...
        while (index != str.length()) {
          ...
          if ((str.length() - index) > 7) {                                     // is data more than 7 byte ?
            ...
            if (((highbuffer.getID()) == 1) || ((highbuffer.getID()) == 2) || ((highbuffer.getID()) == 3)) {
              data += (char) sequenceNumber[highbuffer.getID() - 1].getSequence();
            }
            else {
              data += (char) 0;
            }
            ...
            canbuffer.setData(EncrypData(highbuffer.getID(), highbuffer.getFlag(), data));            // Encryption Data
            index += 7;
            ...
            if (((highbuffer.getID()) == 1) || ((highbuffer.getID()) == 2) || ((highbuffer.getID()) == 3)) {
              ...
              if ((highbuffer.getFlag() & 0x40) == 0x00) {
                timeoutmanage.SetTimeout(sequenceNumber[highbuffer.getID() - 1].getSequenceComp(), ConfigTab.timeout.getValue(), highbuffer.getID(), highbuffer.getFlag());  // Set Timeout and run
              }
            }
            else {
              data += (char) 0;
  ...
  private String EncrypData(int ID, int Flag, String data) {
    String encdata = "";
    if (ID == 1 || ID == 2 || ID == 3) {
      MainFrame.database.GenCode(ID, Flag, data);
      for (int i = 0; i < 8; i++)
        cleartext[i] = data.charAt(i);
      char cp[] = MainFrame.Des[(highbuffer.getID() - 1)].Encryption(cleartext);                    // Encryption Data
      for (int i = 0; i < 8; i++)
        encdata += cp[i];
