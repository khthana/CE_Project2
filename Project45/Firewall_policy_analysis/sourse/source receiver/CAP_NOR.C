/**********************************************************************
* Compile with: -lpcap
* a.out (# of packets) "filter protocol" EX. TCP
* This program is normal packet capture
* write data to tcp.cap udp.cap icmp.cap
**********************************************************************/
        ...
        packet)
{
	...
	if (ipptr->ip_p == 1)
	file_out = fopen("icmp.cap","a");
	if (ipptr->ip_p == 6)
	file_out = fopen("tcp.cap","a");
	if (ipptr->ip_p == 17)
	file_out = fopen("udp.cap","a");
	
	fprintf(file_out,"$%d ",ipptr->ip_p);
