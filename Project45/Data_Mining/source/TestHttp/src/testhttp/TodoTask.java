...
public class TodoTask extends TimerTask {
  // refer class StockEntry
  private StockEntry stockentry;
  // refer class Testcon
  private TestCon testcon;
  // refer class CheckTag
  private CheckTag checktag;
  // refer InputStream
  private InputStream inputstream;
  // refer Vector contain http response data
  private Vector http;
  // refer Vector contain Stock name
  //private Vector stock_name;
  // refer class AccessDatabse
  AccessDatabase database;
  // refer boolean check etc
  private boolean check;
  // refer class CheckData
  private CheckData checkdata;
  // refer class CheckRealTime
  private CheckRealTime checkreal = new CheckRealTime();
  // refer constructor
  public TodoTask(){
  }
  public final void run(){
    ...
    try{
      ...
      //stock_name = database.findStockName();
      ...
      //if( stock_name != null ){
        ...
        for( int i=0;i<vec.conStock.size();i++ ){
          CheckTag checktag = new CheckTag();
          //inputstream = testcon.connHttp( stock_name.elementAt(i).toString() );
          inputstream = testcon.connHttp( vec.conStock.elementAt(i).toString() );
          http = checktag.CheckTag( inputstream );
          ///////////////////////////////////////////
          if( http != null ){
            tim = http.elementAt(11).toString();
            // refer noflag
            if( ( http.size()== 93 )|| ( http.size()== 98 ) ){
      ...
      // refer have flag
      else if( ( http.size()== 94 )|| ( http.size()== 99 ) ){
        ...
        deciss = checkreal.checkReal(i,stockentry.getPrice_Open1(),
                                     stockentry.getPrice_Last(),stockentry.getPrior(),
                                     stockentry.getBuy_Vol(),stockentry.getSell_Vol());
        stockentry.setDeciss( deciss );
      }
          }
          ///////////////////////////////////////////
          check = database.stockIsNot( stockentry.getDay_trade(),stockentry.getStock() );
          if( check )
            database.updateData( stockentry );
          else
            database.insertData( stockentry );
          check = false;
          //System.out.println( stock_name.elementAt(i) );
          System.out.println( vec.conStock.elementAt(i).toString() );
        }// refer end for
      //}// refer end if
    }// refer end try
    catch( Exception e ){
