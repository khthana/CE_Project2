...
void CDibV1View::OnCharacternext() 
{
	...
	for (ParagraphIndex =0;ParagraphIndex<ParagraphCount;ParagraphIndex++)
	{
		...
		MakeHistogramAndLine(ParagraphIndex);
		CharLinePt = Paragraph[ParagraphIndex].CharLineList;
		CNormalBound normalbound;
		while(CharLinePt != NULL)
		{
			if(CharLinePt->Linelevel==3)
			{
				normalbound = LineWalkAround(ParagraphIndex,CharLinePt->Top+(int)((CharLinePt->Bottom-CharLinePt->Top)*0.5));
				if(language == 1)
				{
					normalbound.Top--;
					normalbound.Bottom++;
					LineWalkAround((long)ParagraphIndex,normalbound.Bottom+(long)((normalbound.Bottom-normalbound.Top)*0.375)); // *0.75/2 =*0.375
					LineWalkAround((long)ParagraphIndex,normalbound.Top-(long)((normalbound.Bottom-normalbound.Top)*0.375)); // *0.75/2 =*0.375
					LineWalkAround((long)ParagraphIndex,normalbound.Top-(long)((normalbound.Bottom-normalbound.Top)/2)-(long)((normalbound.Bottom-normalbound.Top)*0.375));
				}
			}
			CharLinePt=CharLinePt->NextCharLine;
		}
		MarkDivideParagraphmark(ParagraphIndex);		
	}
	BittobmPixel(2);
	finish=clock();
	CString bufferx;
...
void CDibV1View::MakeHistogramAndLine(int ParagraphIndex)
{
	BOOLEAN tig=false;
	long countinmoutain =0;
	Paragraph[ParagraphIndex].Histogram = new double[(Paragraph[ParagraphIndex].Bottom-Paragraph[ParagraphIndex].Top+1)*BlockHeight];
	long i,j,x,y,count,countline;
	double histo;
	for(j=0,y=Paragraph[ParagraphIndex].Top*BlockHeight;j<(Paragraph[ParagraphIndex].Bottom-Paragraph[ParagraphIndex].Top+1)*BlockHeight;j++,y++)
	{
		...
		{
			if(BitPixelParagraph[y*PicWidth+x]==ParagraphIndex+100)
			{
		...
		histo = (double)count/countline;
		// Make LINE

		if(tig)
		{
			if (countinmoutain>5) // if have size more enough
			{
				if(count<10)
				{
					CharLine* newLine;
					newLine = new CharLine;
					newLine->Top = y-countinmoutain;
					newLine->Bottom	= y;
					if ((countinmoutain>20)&&(language==1))
					{
						newLine->Linelevel = 3;
					}
					if ((countinmoutain>15)&&(language==2))
					{
						newLine->Linelevel = 3;
					}
					if(Paragraph[ParagraphIndex].CharLineList == NULL)
					{
						Paragraph[ParagraphIndex].CharLineList = newLine;
						CharLinePt = newLine;
					}
					else
					{
						CharLinePt->NextCharLine = newLine;
						newLine->BeforeCharLine = CharLinePt;
						CharLinePt=CharLinePt->NextCharLine;
					}
					tig=false;
					countinmoutain=0;
		...
		else  // find have moutain
		{			
			if (countinmoutain>0) // if have size more enough
			{
				if(count>10)
				{
					tig=true;
		...
		Paragraph[ParagraphIndex].Histogram[j] = histo;
		countinmoutain++;
	}
}
...
CNormalBound CDibV1View::LineWalkAround(int ParagraphIndex, long row)
{
	CNormalBound normalbound;
	long i;
	i=Paragraph[ParagraphIndex].Left*BlockWidth;
	int x;
	int y;
	LinkListChar* tmpChar;
	int countchar = 0;
	long Left,Right,Top,Bottom;
	while(i<Paragraph[ParagraphIndex].Right*BlockWidth+(BlockWidth-1))
	{
		//if (BitPixelParagraph[row*PicWidth+i]==ParagraphIndex+100) {AddonPixel[row*PicWidth+i]=1;}
		AddonPixel[row*PicWidth+i]=1;
		if ((BitPixel[row*PicWidth+i]==1)&&(BitPixelParagraph[row*PicWidth+i]==ParagraphIndex+100))
		{
			int begindirection = 7;
			BOOLEAN MeetBlack = false;
			long Pixely = row;
			long Pixelx = i;
			long PointPixel;
			long PixelSeed = row*PicWidth+i;
			Left = i;
			Right = i;
			Top = row;
			Bottom = row;
			BOOLEAN IsPoint = false;
			do  //do until finish 1 char
			{
				int directionb = begindirection;
				int rundirection = begindirection;
				MeetBlack = false;
				do
				{
					switch (rundirection)
					...
					rundirection++;
					if (rundirection==8) {rundirection=0;}
				}while((PointPixel!=PixelSeed) && (rundirection != directionb) && (MeetBlack==false));
				...
				{
					y = (int)PointPixel/PicWidth;
					x = PointPixel - (y*PicWidth);
					Pixely=y;
					Pixelx=x;
					if(x<Left){Left=x;}
					if(x>Right){Right=x;}
			...
			}while((PointPixel!=PixelSeed)&&(IsPoint==false));
			
			// Record to List
				
			if (!IsPoint)
			{
				if (Paragraph[ParagraphIndex].HeadCharLine == NULL)
				{
					PttoCharLineList = new LinkListCharLine;
					Paragraph[ParagraphIndex].HeadCharLine = PttoCharLineList;
				}
				else
				{
					LinkListCharLine* newLinkListCharLine = new LinkListCharLine;
					PttoCharLineList->NextCharLine = newLinkListCharLine;
				}
				tmpChar = new LinkListChar; 
				tmpChar->Left= Left;
				tmpChar->Right= Right;
				tmpChar->Top= Top;
				tmpChar->Bottom= Bottom;
				if (((Bottom-Top>20)&&(language==1))||((Bottom-Top>15)&&(language==2)))
				{
					if(normalbound.Top == NULL)
					{
						normalbound.Top = Top;
					}
					else
					{
						if(Top>normalbound.Top){normalbound.Top	=Top;}
					}
					if(normalbound.Bottom == NULL)
					{
						normalbound.Bottom = Bottom;
					}
					else
					{
						if(Bottom<normalbound.Bottom){normalbound.Bottom=Bottom;}
					}
					countchar++;
				}
				if (countchar>3){row=normalbound.Bottom-2;}  // Change cause row slope
				if (PttoCharLineList->HeadCharinLine == NULL)
				{
					PttoCharLineList->HeadCharinLine = tmpChar;
					PttoCharinLine = tmpChar;
				}
				else
				{
					PttoCharinLine->NextLinkList = tmpChar;
					PttoCharinLine=PttoCharinLine->NextLinkList;
				}
				// MarkPixelCheck 
				for(y=tmpChar->Top;y<=tmpChar->Bottom;y++)
				{
					for(x=tmpChar->Left;x<=tmpChar->Right;x++)
					{
						BitPixelParagraph[y*PicWidth+x]=ParagraphIndex;
						if ((y==tmpChar->Top)||(y==tmpChar->Bottom)||(x==tmpChar->Left)||(x==tmpChar->Right))
						{
							AddonPixel[y*PicWidth+x]=1;
						}
					}
				}
			}
			...
			{
				IsPoint = false;
				BitPixelParagraph[Top*PicWidth+Left] = ParagraphIndex;
			}
		}
		i++;
	}
...
void CDibV1View::MarkDivideParagraphmark(int ParagraphIndex)
{
	Paragraph[ParagraphIndex].divideparagraphmark = new BOOLEAN[(Paragraph[ParagraphIndex].Right-Paragraph[ParagraphIndex].Left+1)*BlockWidth];
	long i,x,y;
	for(i=0,x = Paragraph[ParagraphIndex].Left*BlockWidth;x<=Paragraph[ParagraphIndex].Right*BlockWidth+(BlockWidth-1);i++,x++)
	{
		Paragraph[ParagraphIndex].divideparagraphmark[i]=false;
		y=Paragraph[ParagraphIndex].Top*BlockHeight;
		while(y<(Paragraph[ParagraphIndex].Bottom+1)*BlockHeight)
		{
			if ((BitPixel[y*PicWidth+x]==1)&&(BitPixelParagraph[y*PicWidth+x]==ParagraphIndex))
			{
				Paragraph[ParagraphIndex].divideparagraphmark[i] = true;
				break;
			}
			y++;
		}
