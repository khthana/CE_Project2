...
/*#  Edit by sua : 090146                                         #*/
/*#################################################################*/
extern struct LinkList ProcNameList;
...
/*######################################################################*/
  ...
  int rc,j,chk;
  long id,pid;
  char sample[80],filename[200],strid[10],strpid[10];
  char path[]="/usr/local/ids/temp/";
  struct LinkListNode * travel = ProcNameList.root;

  	chk = 0;
 	while(travel && (!chk)){
		/*printf("\n %s\n",travel->value);*/
		if(!strcmp(args[0],travel->value)){
			chk = 1;
		}
		travel = travel->next;
	}
...
/* if(!strcmp(args[0],"ls"))*/
  if(chk)
  {	
	for(rc=0;args[rc] != NULL;rc++)
		;
  	args = (char **)xrealloc ((char *)args, (4 + rc) * sizeof (char *));
  	for(j = rc+1 ;j >=0;j--)
	  args[j+3]=args[j];
