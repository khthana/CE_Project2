...
/*# Edit by sua : 090146                                        #*/
/*###############################################################*/
#include "./addnew/t_linklist.h"
struct 	LinkList ProcNameList;
//	ProcNameList.root = NULL;
...
/*# Edit by sua :090446                                       #*/
/*#############################################################*/
static void MakeProcNameList(struct LinkList * list,char * value);
static void ReadConfigFile(struct LinkList *list);
...
/*# Edit by sua :090446                                       #*/
/*#############################################################*/
/*struct  LinkListNode * travel_proc_name,* tmp_proc_name;
*/
/*#############################################################*/
...
/*# Edit by sua : 090146                                        #*/
/*###############################################################*/
/* 	travel_proc_name = ProcNameList.root;
	while(travel_proc_name){
		tmp_proc_name = travel_proc_name;
		travel_proc_name = travel_proc_name->next;
		if(tmp_proc_name->value)
			free(tmp_proc_name->value);
		printf(" %s ",tmp_proc_name->value);
		if(tmp_proc_name)
			free(tmp_proc_name);
	}
*/
...
/*#############################################################*/
static void MakeProcNameList(struct LinkList * list,char * value){
	struct LinkListNode *tmp_ptr,*travel;
	char buff[50];
	int i,j;
	i=-1;
...
//	printf("%s",value);
	...
	while(value[i] != '\0'){
		...
		if(j){
			tmp_ptr = (struct LinkListNode *)malloc(sizeof(struct LinkListNode));
			tmp_ptr->next = NULL;
			tmp_ptr->value = (char *)malloc(strlen(buff) * sizeof(char));
			strcpy(tmp_ptr->value,buff);
...
static void ReadConfigFile(struct LinkList *list){
	...
	FILE * configfile;
	char buff[200],var_name[200],*re;
	int i,j,status = 1;
	configfile = fopen ("./sc.conf","rt");
	if( configfile == NULL){
		printf("\nConnot open sc.conf\n");
	}else{
		fgets(buff,200,configfile);
		while(!feof(configfile) && status){
			i = 0;
			while(buff[i] == ' '){
				printf("%i",i);
				i++;
			...
			if((buff[i] != '#') && (buff[i] != 10) &&(buff[i] != 13)){
				...
				if(!strcmp(var_name,"DetectProcessName")){
					j=0;
					while(buff[i] != '\"'){
						i++;
					}
	...
	if(!status){
		MakeProcNameList(list,var_name);
	}
	else{
		printf("\nCannot Make ProcNameList");
	}
}
