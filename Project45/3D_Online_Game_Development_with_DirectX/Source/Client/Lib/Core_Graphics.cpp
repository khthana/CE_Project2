...
BOOL cMesh::Load(cGraphics *Graphics, char *Filename, char *TexturePath, BOOL TransparentBlack)
{
    ...
    if(m_Meshes == NULL || m_Meshes->FindMesh(Name) == NULL) {
      ...
      if(!Mesh->m_NumMaterials) {
        ...
        for(i=0;i<Mesh->m_NumMaterials;i++) {
		  ...
		  if ( m_TransparentBlack == TRUE ){
				if ( FAILED( D3DXCreateTextureFromFileEx( m_Graphics->GetDeviceCOM(), 
													  Path,
													  D3DX_DEFAULT,
													  D3DX_DEFAULT,
													  D3DX_DEFAULT,
