...
typedef struct sMesh
{
  ...
  ~sMesh()
  {
	...
	if(m_Mesh)			{ (m_Mesh)->Release();		(m_Mesh)=NULL; }
	if(m_SkinMesh)		{ (m_SkinMesh)->Release();	(m_SkinMesh)=NULL; }
    if(m_Materials)		{ delete[] (m_Materials);   (m_Materials)=NULL; }
    if(m_Textures != NULL) {
      for(DWORD i=0;i<m_NumMaterials;i++) {
		if(m_Textures[i])	{ (m_Textures[i])->Release();	(m_Textures[i])=NULL; }
      }
	  if(m_Textures)		{ delete[] (m_Textures);		(m_Textures)=NULL; }
    }
	if(m_BoneNames)			{ (m_BoneNames)->Release();		(m_BoneNames)=NULL; }
    if(m_BoneTransforms)	{ (m_BoneTransforms)->Release(); (m_BoneTransforms)=NULL; }
    if(m_Matrices)			{ delete[] (m_Matrices);		(m_Matrices)=NULL; }
	if(m_FrameMatrices)		{ delete[] (m_FrameMatrices);   (m_FrameMatrices)=NULL; }
    if(m_Next)				{ delete (m_Next);				(m_Next)=NULL; }
  }
...
typedef struct sFrame
{
  ...
  {
	  if(m_Name)		{ delete (m_Name);		(m_Name)=NULL; }
	  if(m_MeshList)	{ delete (m_MeshList);  (m_MeshList)=NULL; }
	  if(m_Child)		{ delete (m_Child);     (m_Child)=NULL; }
	  if(m_Sibling)		{ delete (m_Sibling);   (m_Sibling)=NULL; }
  }
...
class cMesh
{
  ...
  public:
	...
	BOOL	IsTransparentBlack()	{ return m_TransparentBlack; }

    BOOL    Load(cGraphics *Graphics, char *Filename, char *TexturePath = ".\\", BOOL TransparentBlack = FALSE);
    BOOL    Free();
};
...
typedef struct sAnimation
{
  ...
  {
	if(m_Name)			{ delete[] (m_Name);		(m_Name)=NULL; }
	if(m_FrameName)		{ delete[] (m_FrameName);   (m_FrameName)=NULL; }
	if(m_PositionKeys)	{ delete[] (m_PositionKeys); (m_PositionKeys)=NULL; }
	if(m_RotateKeys)	{ delete[] (m_RotateKeys);  (m_RotateKeys)=NULL; }
	if(m_ScaleKeys)		{ delete[] (m_ScaleKeys);   (m_ScaleKeys)=NULL; }
	if(m_MatrixKeys)	{ delete[] (m_MatrixKeys);  (m_MatrixKeys)=NULL; }
	if(m_Next)			{ delete (m_Next);			(m_Next)=NULL; }
  }
...
typedef struct sAnimationSet
{
  ...
  {
	if(m_Name)			{ delete[] (m_Name);	(m_Name)=NULL; }
	if(m_Animation)		{ delete (m_Animation);	(m_Animation)=NULL; }	
	if(m_Next)			{ delete (m_Next);		(m_Next)=NULL; }	
  }
