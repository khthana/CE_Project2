...
idata unsigned char packet[20];
code unsigned char home_key[8]="beerbeer";
idata unsigned char index,maxlength,state,dev1,dev2,sen1,sen2,encpy;
xdata unsigned char Key_Schedule[16][48];
void init(void);
void flowcontrol(void);

/* left shift table for key schedule LS */
code  unsigned char LS[16] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
...
code unsigned char E[48] = {
  32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9,10,11,
  12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,
  22,23,24,25,24,25,26,27,28,29,28,29,30,31,32, 1 };

/* substitution tables ST */
code unsigned char ST[8][64] = {
...
/* permutation table P */
code unsigned char PP[32] = {
  16, 7,20,21,29,12,28,17, 1,15,23,26, 5,18,31,10,
   2, 8,24,14,32,27, 3, 9,19,13,30, 6,22,11, 4,25 };

/* final permutation FP */
code unsigned char FP[64] = {
...
{
int i,j;
unsigned char lr[64], rsto[32], si[48], so[32]; // test test test
#define LEFT  lr
#define RIGHT  &lr[32]

	perm(lr, IPP, in, 64); /* initial permutation */
	for (i = 0; i < 16; i++)
	{ /* 16 rounds */
		...
		perm(RIGHT, PP, so, 32); /* permute substitution output: end F*/
...
/* des_ecb_encrypt(in, out, ks, enc) -- des electronic xdata book mode */
...
void copystr(unsigned char* a,unsigned char* b,unsigned char j) {
unsigned char i;
	for(i=0;i<j;i++,a++,b++)
		*a=*b;
}
	...
	while(1){
		if(EA==0) {
			if(encpy==1){
				...
				temp[2]=dev1;
				temp[3]=dev2;
				copystr(&temp[4],"beer",4);
				DesECBEncrypt(home_key,temp,cipher);
				copystr(&packet[2],cipher,8);	
				maxlength=10;
				index=10;
				packet[0]=':';
				packet[1]=10;
				...
				copystr(cipher,&packet[5],8);
				DesECBDecrypt(home_key,cipher,temp); // key,cipher,data
				dev1=temp[0];
				dev2=temp[1];				
				P1_4=0;
				P1 |= 0x0c; // set ----11--
...
void flowcontrol(void) {
		switch(state){
			case 0 : //wait recive from com--------->send
				if(packet[3]=='h'){ // id of home
//					application();
					if(packet[4]=='3') { //------------command to device of home node
						encpy=2;
						EA=0;
					}else
	 				if (packet[4]=='1') { //-------check status of home node 
						encpy=1;
						P1_4=0;
						P1 &= 0xf3;// set 00
						sen1=P0;
