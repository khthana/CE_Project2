...
#include <semaphore.h>
	sem_t mind_sem;
#else  // POSIX_SEMAPHORE
#include <sys/sem.h>
	struct sembuf sem_wait_op[1];	// wait
	struct sembuf sem_post_op[1];	// post
#endif // POSIX_SEMAPHORE
	
int fdesc;	// This fdesc was use in 'lfd_encrypt.c'
#endif // HAVE_SSL
...
int main(int argc, char *argv[], char *env[])
{
     ...
     sem_wait_op[0].sem_num	= 0;
     sem_wait_op[0].sem_op 	= -1;	// wait	// dec
     sem_wait_op[0].sem_flg	= 0;

     sem_post_op[1].sem_num	= 0;
     sem_post_op[1].sem_op	= 1;	// post	// inc
     sem_post_op[1].sem_flg	= 0;
     
     id = semget(KEY, 1, 0666 | IPC_CREAT);
     if (id < 0) {
	     syslog(LOG_ERR, "Unable to obtain semaphore.\n");
	     exit(id);
     }
...
//     sem_ret = semop(id, sem_wait_op, 1);	// wait()
#else
	if (sem_init(&mind_sem, 0, 1) < 0) {
		syslog(LOG_ERR, "Can't initialize semaphore");
		return -1;
	}
#endif
	// initialize File
 ...
 * Overrides existing file. optionally adds a tag to the pidfile name
 */
void write_pid(char *tag)
{
  char fn[1024];
     FILE *f;
  ...
  if (tag!=NULL)
  {
    snprintf(fn,sizeof(fn),"%s/vtund.%s.pid",
            VTUN_PID_DIR,tag);
  }
  else
  {
