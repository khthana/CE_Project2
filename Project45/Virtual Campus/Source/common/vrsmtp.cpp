...
bool CVRSmtp::AddRecipient(const char email[], const char name[])
{
    assert(email);
...
bool CVRSmtp::AddCCRecipient(const char email[], const char name[])
{
    assert(email);
...
bool CVRSmtp::AddBCCRecipient(const char email[], const char name[])
{
    assert(email);
...
bool CVRSmtp::ConnectServer(const char server[], const unsigned short port)
{
    assert(server);
...
SOCKET CVRSmtp::_connectServerSocket(const char server[],
...
const char* const CVRSmtp::GetLocalHostIp() 
{
    in_addr    *iaHost;
...
const char* const CVRSmtp::GetLocalHostname() 
{    
...
bool CVRSmtp::GetConnectStatus()
{
    return (bCon);
}

unsigned const int CVRSmtp::GetBCCRecipientCount()
{
    return (BCCRecipients.size());
}

unsigned const int CVRSmtp::GetCCRecipientCount() 
{
    return (CCRecipients.size());
}

unsigned const int CVRSmtp::GetSocket()
{
    return (hSocket);
}

const char* const CVRSmtp::GetMessageBody() 
{
    return (m_pcMsgBody);
}

unsigned const int CVRSmtp::GetRecipientCount()
{
    return (Recipients.size());
}

const char* const CVRSmtp::GetReplyTo()  
{
    return (m_pcReplyTo);
}

const char* const CVRSmtp::GetSenderEmail() 
{
    return (m_pcFromEmail);
}

const char* const CVRSmtp::GetSenderName() 
{
    return (m_pcFromName);
}

const char* const CVRSmtp::GetSubject() 
{
    return (m_pcSubject);
}

const char* const CVRSmtp::GetXMailer() 
{
    return (m_pcXMailer);
}

void CVRSmtp::SetMessageBody(const char body[])
{
    assert(body);
    int s=strlen(body);
    if (m_pcMsgBody)
...
void CVRSmtp::SetReplyTo(const char replyto[])
{
    assert(replyto);
...
void CVRSmtp::SetSenderEmail(const char email[])
{
    assert(email);
    int s=strlen(email);
    if (m_pcFromEmail)
...
void CVRSmtp::SetSenderName(const char name[])
{
    assert(name);
    int s=strlen(name);
    if (m_pcFromName)
...
void CVRSmtp::SetSubject(const char subject[])
{
    assert(subject);
...
void CVRSmtp::SetXMailer(const char xmailer[])
{
    assert(xmailer);
