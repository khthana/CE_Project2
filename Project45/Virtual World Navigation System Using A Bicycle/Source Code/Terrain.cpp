...
CTerrain::CTerrain(LPDIRECT3DDEVICE8 pD3DDevice, WORD wRows, WORD wCols, WORD xs, WORD zs, float rTileSize, WORD wMaxHeight)
{
...
bool CTerrain::CreateIndexBuffer()
{
	...
	if(FAILED(m_pIndexBuffer->Lock(0, m_dwNumOfIndices * sizeof(WORD), (BYTE**)&pBufferIndices, 0)))
	{
...
bool CTerrain::SetPosition(WORD xs, WORD xe ,WORD zs, WORD ze)
{
	posXStart = xs;
	posXEnd = xe;
	posZStart = zs;
...
/*
	zStart = (float)posZStart;
	zEnd = (float)posZEnd;
	xStart = (float)posXStart;
	xEnd = (float)posXEnd;
*/
	zStart = 1.0f;
	zEnd = 21.0f;
