...
#ifdef  DEBUGARP
typedef struct {                /* ARP statistics, mainly for debugging */
  u_int32_t requests;
  u_int32_t localsearch;
  u_int32_t noreceiver;
  u_int32_t nobuffer;
  u_int32_t replies;
} ARPSTATS;
typedef ARPSTATS far * ARPSTATS_FP;
#endif  /* DEBUGARP */
...
static u_char driver_function = 2; /* basic+extended functionality present */
static u_short majver         = 11;
static u_short address_len    = EADDR_LEN;

/* Pseudo-Ethernet address to return in 'get_address()' function. */
...
static u_char your_address[MAX_ADDR_LEN] = {
...
/* Default (and only) receive mode */
static u_char rcv_mode_num = 3;

#ifdef  DEBUGARP
/* ARP statistics */
u_short arp_rpl_idx, arp_req_idx;
ARP snd_arp_rpl[16];
ARP rcv_arp_req[16];
ARPSTATS arpcounters;
#define MAX_ARP_IDX ((sizeof(rcv_arp_req) / sizeof(ARP)) - 1)
#endif  /* DEBUGARP */
...
int handle_ip_packet(int unit, const u_char *data, int count)
{
    ...
    /* first put a fake ethernet header */
                                        /* destination address */
    dptr->ethdestin[0] = ((u_short *)my_address)[0];
    dptr->ethdestin[1] = ((u_short *)my_address)[1];
    dptr->ethdestin[2] = ((u_short *)my_address)[2];
...
void arp_reply(ARP_FP sptr, u_short len)
{
...
#ifdef  DEBUGARP
    arpcounters.requests++;
    _fmemcpy(&rcv_arp_req[arp_req_idx++], sptr, sizeof(ARP));

    if ( arp_req_idx > MAX_ARP_IDX )
        arp_req_idx = 0;
#endif  /* DEBUGARP */

    /* If the packet is not an ARP request, don't reply */
    if ( sptr->opcode != 0x100 )
        return;
    ...
    /* Load source hardware address in ARP reply packet. It seems that
       some apps look into the 'ethsource' field, so load it in there too. */
    _fmemcpy(dptr->shwaddr, your_address, sizeof(dptr->shwaddr));
    _fmemcpy(dptr->ethsource, your_address, sizeof(dptr->ethsource));

    /* Set opcode to REPLY. */
    dptr->opcode = 0x0200;

#ifdef  DEBUGARP
    arp_rpl_idx = (arp_req_idx > 0) ? arp_req_idx - 1 : MAX_ARP_IDX;
    _fmemcpy(&snd_arp_rpl[arp_rpl_idx], dptr, sizeof(ARP));
#endif  /* DEBUGARP */
...
int bootp_reply(BOOTP_FP sptr, u_short len)
{
    ...
    /* check protocol, must be IP */
         sptr->ethproto != 0x0008
         ||
    /* check if it is a broadcast ethernet packet */
         sptr->ethdestin[0] != 0xFFFF ||
         sptr->ethdestin[1] != 0xFFFF ||
         sptr->ethdestin[2] != 0xFFFF
         ||
    /* check if IP protocol is UDP */
         sptr->protocol != UDP_PROTO
         ||
    /* check BOOTP client/server UDP port numbers */
         sptr->udpsrc != 0x4400 || sptr->udpdest != 0x4300
         ||
    /* check if BOOTP operation is request */
         sptr->op != BOOTREQUEST )
        /* not a legal BOOTP request, packet requires further processing */
        return 0;
    ...
    /* fill in the 'vendor' field if required. Hacked to
       always fill in vendor field, even if client didn't
       provide us with a valid magic cookie, KA9Q/JNOS ??? */
    if ( 1 /* sptr->cookie == 0x63538263LU */ ) {
        dptr->cookie    = 0x63538263LU;
        ...
        /* if 1st namsrv IP was missing, use gateway IP for it */
        if ( ! (vendinf.dns1val = namsrvaddr[ifunit][0]) )
            vendinf.dns1val = vendinf.gateval;

        /* if 2nd namsrv IP was missing, use 1st namsrv IP for it */
        if ( ! (vendinf.dns2val = namsrvaddr[ifunit][1]) )
            vendinf.dns2val = vendinf.dns1val;
...
static u_char f_send_pkt(INTERRUPT_REGS far *r)
{
    ...
    if ( 0x0608 == sptr->ethproto ) {
        ...
        /* see if it is a BOOTP request */
        if ( ! bootp_reply((BOOTP_FP)sptr, len) ) {
            /* get the real IP packet length, packets may come padded to 60
               bytes from upper layers; it seems that VJ code fails in that
               case, because IP sequence number increments can be deduced
               from the IP packet length. */
            len = ntohs(((struct iphdr far *)(sptr->ethdata))->tot_len);

            /* check for PPP channel availability */
            if ( set_pppsem(comopen) > 0 ) {
                /* return error code if can't send packet */
                if ( ppp_dev_xmit(ifunit, sptr->ethdata, len) )
                    retcode = CANT_SEND;

                /* release PPP channel semaphore */
                free_pppsem(comopen);
            }
            else
                /* PPP channel is busy, return error code */
                retcode = CANT_SEND;
        }
    }
...
/*
 * This function handles the SET RECEIVER MODE command for the packet driver.
 * Only mode 3 is supported (packets sent to this interface plus broadcasts).
 */
static u_char f_set_rcv_mode(INTERRUPT_REGS far *r)
{
    u_char retcode;
    PER_HANDLE *phandle = (PER_HANDLE *)(r->dr.w.bx);
    ...
    /* ensure that they want mode 3 */
    if ( r->dr.w.cx < 1 || r->dr.w.cx > 3 )
        return BAD_MODE;

    rcv_mode_num = r->dr.w.cx;
...
/*
 * This function handles the GET RECEIVER MODE command for the packet driver.
 */
static u_char f_get_rcv_mode(INTERRUPT_REGS far *r)
{
    u_char retcode;
    PER_HANDLE *phandle = (PER_HANDLE *)(r->dr.w.bx);
...
/*
 * This function handles the GET STATISTICS command for the packet driver.
 */
static u_char f_get_statistics(INTERRUPT_REGS far *r)
{
    u_char retcode;
    PER_HANDLE *phandle = (PER_HANDLE *)(r->dr.w.bx);
    void far *statsptr;

    void far *get_ppp_stats(int);
    /* ensure that their handle is real */
    if ( (retcode = verify_handle(phandle)) != NO_ERROR )
        /* no, fail */
        return retcode;

    statsptr = get_ppp_stats(ifunit);
    r->ds = FP_SEG(statsptr);
    r->si = FP_OFF(statsptr);
...
void pktdriver(INTERRUPT_REGS far *r)
{
    ...
    switch ( r->dr.b.ah ) {
        ...
        case SET_RCV_MODE:
            if ( (retcode = f_set_rcv_mode(r)) != NO_ERROR ) {
                r->dr.b.dh = retcode;
                r->flags   |= CARRY_FLAG;
            }
        break;
        ...
        case GET_RCV_MODE:
            if ( (retcode = f_get_rcv_mode(r)) != NO_ERROR ) {
                r->dr.b.dh = retcode;
                r->flags   |= CARRY_FLAG;
            }
        break;
        ...
        case GET_STATISTICS:
            if ( (retcode = f_get_statistics(r)) != NO_ERROR ) {
                r->dr.b.dh = retcode;
                r->flags   |= CARRY_FLAG;
            }
        break;
...
#ifdef  ALLOWVJ
        /* User extension, returns pointer to struct slcompress
           to be used by VJCSTATS program. Displays VJ statistics.
           The caller pass the struct size in BX, and receives a far
           pointer to the struct in DS:SI (if struct size is OK). */
        case GET_VJSTATS:
            /* Make sure the caller uses the correct calling convention */
            if ( r->dr.w.bx != sizeof(struct slcompress) ) {
                r->ds = r->si = 0;
                r->dr.b.dh    = BAD_COMMAND;
                r->flags      |= CARRY_FLAG;
            }
            ...
            {
                void far *get_ppp_vjstats(int);
                void far *tfp = get_ppp_vjstats(ifunit);
                r->ds = FP_SEG(tfp);
                r->si = FP_OFF(tfp);
#ifdef  DEBUGARP
                /* ugly hack for passing up ARP statistics, the caller
                   should assume both pointers refer to the same segment */
                if ( r->dr.w.cx == sizeof(ARPSTATS) )
                    r->dr.w.bx = (u_short)&arpcounters;
#endif  /* DEBUGARP */
            }
        break;
#endif  /* ALLOWVJ */
