...
#ifdef  ALLOWVJ
extern unsigned _heaplen = 14656 /* 17408 */;
#else
extern unsigned _heaplen = 9216 /* 11264 */;
#endif
...
void far everytick(void)
{
    ...
    if ( modem && ! hungup && ! get_rlsd_asy(comopen) ) {
...
void exit_fn1(void)
{
    ...
    if ( ! going_resident ) {
        ...
        if ( comopen != -1 ) {
            ...
            if ( modem ) {
                ...
                if ( asy_ioctl(comopen, PARAM_DOWN, 0, (int32)0) < 0 ) {
                    MAINDEBUG((LOG_ERR,
                               "main: COM%d PARAM_DOWN ioctl error.\n",
                               comopen + 1));
                }
                ...
                if ( asy_ioctl(comopen, PARAM_UP, 0, (int32)0) < 0 ) {
                    MAINDEBUG((LOG_ERR,
                               "main: COM%d PARAM_UP ioctl error.\n",
                               comopen + 1));
                }

                /* send hangup string to modem */
                if ( asy_write(comopen, (u_char *)"+++", 3) < 0 ||
                        (sleep(1),
                         asy_write(comopen, (u_char *)"ATH0\r", 5) < 0) ) {
                    MAINDEBUG((LOG_ERR,
                               "main: COM%d\n asy_write() error.\n",
                               comopen + 1));
                }
        ...
        /* disable further messages via 'syslog()' */
        disable_syslog = 1;
    ...
    /* The following code should restore DOS critical handler and DOS break
       handler. Geoff Chappel (DOS internals) suggest that you don't want
       to do this, as DOS can fail or be interrupted during the going resident
       stage. Anyway DOS itself will restore these vectors with the ones saved
       in the program PSP. */
    ...
    /* instruct timer handler to execute periodic PPP processing */
    if ( going_resident == 1 )
        going_resident = 2;
}
...
int main(int argc, char *argv[])
{
    ...
    StktopC = ((uint16)_SP - 1024 /* 1280 */) & 0xFFFE;
...
*/
    ...
    for ( phase = PHASE_ESTABLISH ; phase != PHASE_DEAD ; ) {
        ...
        if ( kill_link == 1 ) {
            lcp_close(ifunit);
            ++kill_link;
            syslog(LOG_NOTICE, "User abort, closing link...\n");
        }
...
static int runconnector(char *path, char *command)
{
    ...
    cmdline[0] = min(strlen(pargs), 126);
    strncpy(&cmdline[1], pargs, 126);
    strcat(&cmdline[1], "\r");
...
int rcv_proto_unknown(struct ppp *ppp, u_short protocol, u_char *p, int len)
{
    ...
    if ( protocol != PPP_LCP && lcp_fsm[ppp->line].state != OPENED )
    {
        MAINDEBUG(
            (LOG_INFO,
             "rcv_proto_unknown: Received non-LCP packet when LCP not open.\n")
        );
    ...
    MAINDEBUG((LOG_WARNING,
               "rcv_proto_unknown: unknown protocol (0x%x) received.\n",
               protocol));
...
void calltimeout(void)
{
    ...
    while ( callout != NULL ) {
        ...
        /* check the status of 'pppsem' before allowing the call to
           the timeout function, in order to avoid reentrance problems. */
        if ( set_pppsem(comopen) <= 0 )
            break;

        callout = p->c_next;
        MAINDEBUG((LOG_DEBUG,
                   "calltimeout, calling %x:%x.\n", p->c_func, p->c_arg));
        (*p->c_func)(p->c_arg);
        free((char *)p);
        free_pppsem(comopen);
    }
