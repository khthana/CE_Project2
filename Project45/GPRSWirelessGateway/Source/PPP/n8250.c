             ...
             long speed, int cts, int rlsd, int chain)
{
    ...
    asyp->save.lcr = inportb(base + LCR);
    asyp->save.ier = inportb(base + IER);
    asyp->save.mcr = inportb(base + MCR);
    asyp->msr = asyp->save.msr = inportb(base + MSR);
    asyp->carrier = ((asyp->msr & MSR_RLSD) != 0);
    asyp->save.iir = inportb(base + IIR);

    /* save speed bytes */
    setbit(base + LCR,LCR_DLAB);
    asyp->save.divl = inportb(base + DLL);
    asyp->save.divh = inportb(base + DLM);
...
int asy_stop(int dev)
{
    ...
    if ( asyp->is_16550a ) {
        ...
        if ( (asyp->save.iir & IIR_FIFO_ENABLED) != IIR_FIFO_ENABLED )
            outportb(base + FCR, 0);
    }
    ...
    /* Restore speed regs */
    setbit(base + LCR, LCR_DLAB);
    outportb(base + DLL, asyp->save.divl);   /* Low byte */
    outportb(base + DLM, asyp->save.divh);   /* Hi byte */
    clrbit(base + LCR, LCR_DLAB);

    /* Restore control regs */
    outportb(base + LCR, asyp->save.lcr);
    outportb(base + IER, asyp->save.ier);
    outportb(base + MCR, (asyp->save.mcr & (~MCR_DTR)));
...
static int rlsdstat(struct asy *asyp)
{
    if ( asyp->rlsd )
        ...
        return ((asyp->msr & MSR_RLSD) != 0);
    else
        return ((inportb(asyp->addr + MSR) & MSR_RLSD) != 0);
}
