...
/* $Id: ppp.c,v 1.2 2003/01/13 13:34:10 suriyan Exp $
...
#ifdef ALLOWVJ
    ppp->cbuf = ppp_alloc_buf(ppp->mtu + PPP_HARD_HDR_LEN, BUFFER_TYPE_VJ);
...
static int rcv_proto_vjc_comp(struct ppp *ppp,
                              ...
                              u_char *data, int count)
{
    if ( (ppp->flags & SC_REJ_COMP_TCP) == 0 ) {
        ...
        PPPDEBUG((LOG_DEBUG, "ppp: error in VJ decompression\n"));
    }
...
static int rcv_proto_vjc_uncomp(struct ppp *ppp,
                                unsigned short proto,
                                u_char *data, int count)
{
    if ( (ppp->flags & SC_REJ_COMP_TCP) == 0 ) {
        ...
        PPPDEBUG((LOG_DEBUG, "ppp: error in VJ memorizing\n"));
    }
...
static int ppp_dev_xmit_lower(struct ppp *ppp, struct ppp_buffer *buf, const u_char *data, int count, int non_ip)
{
    ...
    extern volatile u_short timercount;
    extern volatile int kill_link;
    extern volatile int hungup;
/*
 ...
 */
    ...
    if ( ((ppp->flags & SC_COMP_RUN) != 0) &&
        ...
        new_count =
            bsd_compress(ppp->sc_xc_state, data, new_data, count, count);
 ...
 */
    retcode = ppp_dev_xmit_lower(ppp, buf, data, count, (proto & 0xFF00) != 0);
...
/*
 * Send an IP frame to the remote with (optional) VJ header compression.
 *
 * Return 0     if frame was queued for transmission.
 *        1     if frame must be re-queued for later driver support.
 *        < 0   if serious error.
...
int ppp_dev_xmit(int unit, const u_char far *indata, int len)
{
    struct ppp *ppp;
    struct ppp_hdr *hdr;
    u_char *data;
 ...
 */
    if ( ! (ppp->flags & SC_ENABLE_IP) ) {
        PPPDEBUG((LOG_DEBUG, "ppp_dev_xmit: packet sent when interface ppp%d is down for IP.\n", ppp->line));

        return 1;
    }

    PPPDEBUG((LOG_DEBUG, "ppp_dev_xmit [ppp%d]: indata %fp.\n", unit, indata));
/*
 * Send an IP frame. Ensure that the caller does not wish to send too much.
 */
    if ( len > ppp->mtu ) {
        PPPDEBUG((LOG_DEBUG, "ppp_dev_xmit: truncating user packet from %u to mtu %d\n", len, ppp->mtu));

        len = ppp->mtu;
    }

    data = ip_pkt_buf + PPP_HARD_HDR_LEN;
    _fmemcpy((u_char far *)data, indata, len);
/*
 * Print the frame being sent
 */
#ifdef DEBUGPPP
    if ( ppp->flags & SC_LOG_OUTPKT )
        ppp_print_buffer("ppp_dev_xmit:", data, len);
#endif  /* DEBUGPPP */
 ...
 */
    ...
    if ( ppp->flags & SC_COMP_TCP ) {
        len = slhc_compress(ppp->slcomp,
                            data, len,
                            BUF_BASE(ppp->cbuf) + PPP_HARD_HDR_LEN,
                            &data, (ppp->flags & SC_NO_TCP_CCID) == 0);
 ...
 */
    ...
    answer = ppp_dev_xmit_frame(ppp, ppp->tbuf, (const u_char *)hdr, len);
/*
...
/*
 * get_ppp_stats - returns pointer to pppstat structure for a given unit.
 */

void far *get_ppp_stats(int unit)
{
    struct ppp *ppp;
/*
 * Validate the pointer to the PPP structure
 */
    if ( unit >= PPP_MAX_DEV ) {
        MAINDEBUG((LOG_ERR, "get_ppp_stats: invalid unit.\n"));

        return NULL;
    }

    if ( (ppp = PPPDEV(unit)) == NULL || ppp->magic != PPP_MAGIC ) {
        MAINDEBUG((LOG_ERR, "get_ppp_stats: unit not opened.\n"));

        return NULL;
    }

    return (void far *)&(ppp->stats);
}

#ifdef  ALLOWVJ
/*
 * get_ppp_vjstats - returns pointer to slcompress structure for a given unit.
 */

void far *get_ppp_vjstats(int unit)
{
    struct ppp *ppp;
/*
 * Validate the pointer to the PPP structure
 */
    if ( unit >= PPP_MAX_DEV ) {
        MAINDEBUG((LOG_ERR, "get_ppp_vjstats: invalid unit.\n"));

        return NULL;
    }

    if ( (ppp = PPPDEV(unit)) == NULL || ppp->magic != PPP_MAGIC ) {
        MAINDEBUG((LOG_ERR, "get_ppp_vjstats: unit not opened.\n"));

        return NULL;
    }

    return (void far *)(ppp->slcomp);
}
#endif  /* ALLOWVJ */
...
int sifvjcomp(int unit, int vjcomp, int cidcomp, int maxcid)
{
    ...
    if ( vjcomp ) {
        ...
        if ( (ppp->slcomp = slhc_init (16, maxcid)) == NULL ) {
            vjcomp = 0;
            PPPDEBUG((LOG_DEBUG, "sifvjcomp: slhc_init() failed.\n"));
        }
    }
    ...
    ppp->flags = x;
    PPPDEBUG((LOG_DEBUG, "sifvjcomp: set flags %lx.\n", x));
