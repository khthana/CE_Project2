//********************************************************
//                                                            
//	FILENAME	: CGradientMenu.cpp
//	CONTENTS	: Defines the class behaviors for the CGradientMenu class.
//
//  Copyright (c), 1999-2000.					  
//	All rights reserved									  
//                                                        
//  Author(s):  jim-k@rocketmail.com (CGradientMenu)
//				Dipti Deogade (original CBmpMenu)
//				zhubin@bigfoot.com (Gradient drawing code) from ChartZhuDemo.zip
//                                                        
//*********************************************************
...
CGradientMenu::CGradientMenu(int nBannerW, BOOL bShow4SubMenu)
{
...
CGradientMenu::~CGradientMenu()
{
}

IMPLEMENT_DYNAMIC( CGradientMenu, CWnd )

BEGIN_MESSAGE_MAP(CGradientMenu, CWnd)
	//{{AFX_MSG_MAP(CGradientMenu)
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CGradientMenu message handlers

BOOL
CGradientMenu :: Attach( HMENU hMenu )
{
	return CMenu::Attach(hMenu);
}

HMENU
CGradientMenu :: Detach()
{
	return CMenu::Detach();
}


BOOL CGradientMenu::DestroyWindow() 
{
	...
	if(::IsWindow(GetParent()->GetSafeHwnd()) && 
		GetParent()->IsKindOf(RUNTIME_CLASS(CGradientMenu)))
	{
		GetParent()->SetFocus();
	}
...
BOOL
CGradientMenu :: TrackPopupMenu( UINT nFlags, int x, int y, CWnd* pParentWnd, CRect* pItemRect)
{
	...
	if(m_bSubMenu)
	{
		((CGradientMenu*)GetParent())->m_pSubMenuWnd = this;
	}
...
void CGradientMenu::PopupSubMenu(WPARAM wp, BOOL bSelectFirstItem)
{
	...
	CGradientMenu oSubMenu(m_bShowBmpAll?m_nTBOffSet:0, m_bShowBmpAll);

	oSubMenu.SetGradientColors(m_Color1, m_Color2, m_ColorText);

	oSubMenu.SetTitle(strTitle); //tbb.iString);
	
	//intialize data of popup menu
	oSubMenu.m_pOwnerWnd = m_pOwnerWnd;

	//When user created an instance of CGradientMenu, automatically a handle was associated with root menu
...
void CGradientMenu::InitToolBarData(CToolBar *pToolBar, CPoint pt, CRect* pItemRect)
{
	//enable tooltips
...
LRESULT CGradientMenu::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	case WM_ACTIVATE:
		{
			...
			if(lParam && IsWindow(HWND(lParam)) && (LOWORD(wParam) == WA_INACTIVE))
				if(!oWnd.FromHandle(HWND(lParam))->IsKindOf(RUNTIME_CLASS(CGradientMenu)))
					DestroyRootMenu();
...
void CGradientMenu::DestroyRootMenu()
{
	if(m_hWnd && IsWindow(m_hWnd))
	{
		...
		{
			if(pParent->IsKindOf(RUNTIME_CLASS(CGradientMenu)))
				((CGradientMenu*)pParent)->DestroyRootMenu();	
		}
...
void CGradientMenu::DestroySubMenus()
{
	if(m_pSubMenuWnd)
	{
...
BOOL CGradientMenu::PreCreateWindow(CREATESTRUCT& cs) 
{
	// TODO: Add your specialized code here and/or call the base class
...
//		GetTitle
	...
	rect2.left = 2;
//		dc.DrawText ( GetTitle(), rect, DT_BOTTOM | DT_CENTER | DT_SINGLELINE );
...
void CGradientMenu::PositionMenuWindow(CPoint pt, CRect* pItemRect, CRect menuRect)
{
	CRect deskRect;
...
BOOL
CGradientMenu::PositionSubMenu(CPoint pt, CRect menuRect, BOOL bRtAlign, BOOL bDnAlign)
{
	CRect deskRect;
	...
	if(lpNMCustomDraw->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
	{
		...
		if(lpNMCustomDraw->nmcd.uItemState & CDIS_HOT)
		{
			...
			if(m_nSelectedItem != (int)lpNMCustomDraw->nmcd.dwItemSpec)
			{
				...
				((CGradientMenu*)GetParent())->m_pOwnerWnd->SendMessage(WM_MENUSELECT, 
									MAKEWPARAM(ndx, menuInfo.fState), 
									(LPARAM)((CGradientMenu*)GetParent())->m_hMenu);
...
void MenuToolBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(nBtnIndex >= 0)
	{
		if(m_nLastLBDownIndex != nBtnIndex)
		{
			...
			((CGradientMenu*)GetParent())->DestroySubMenus();
...
void MenuToolBar::OnPostLbuttonMsg(UINT nFlags, LPARAM lp)
{
	...
	{
		((CGradientMenu*)GetParent())->SendMessage(WM_POPUPSUBMENU, lp, (MENU_SELECTFIRSTITEM & nFlags)?MENU_SELECTFIRSTITEM:0);
		return;
	}

	//If this is a valid button and no submenu then send wm_command message to owner and close all menus
	((CGradientMenu*)GetParent())->m_pOwnerWnd->PostMessage(WM_COMMAND, MAKEWPARAM(tbb.idCommand, 0), 0);
	((CGradientMenu*)GetParent())->DestroyRootMenu();
}
...
BOOL MenuToolBar::KeyboardFilter(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if(nChar == VK_LEFT)
	{
		...
		if(pWnd && IsWindow(pWnd->m_hWnd) && pWnd->IsKindOf(RUNTIME_CLASS(CGradientMenu)))
		{
			((CGradientMenu*)pWnd)->DestroySubMenus();

			(CGradientMenu*)pWnd->SendMessage(WM_RESETDATA, 0, 0);
		}
		return TRUE;
	}
...
void MenuToolBar::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if (nFound == 0)
	{
		...
		UINT lRet = (((CGradientMenu*)GetParent())->m_pOwnerWnd)->SendMessage(WM_MENUCHAR, MAKEWPARAM(nChar, MF_POPUP), 
													(LPARAM)((CGradientMenu*)GetParent())->m_hMenu);
		...
		case MNC_EXECUTE:
			(((CGradientMenu*)GetParent())->m_pOwnerWnd)->SendMessage(WM_COMMAND, LOWORD(lRet), 0);			
			break;
		}
...
void CGradientMenu::PaintGradiantRect(CDC *pDC, const RECT &rect,COLORREF clrFrom, COLORREF clrTo, 
