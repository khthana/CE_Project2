...
Begin VB.Form PNWageReportCvtBAY 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Convert Data for Bank of Ayudhya PCL"
   ClientHeight    =   5070
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "PNWageReportCvtBAY.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton Command1 
      Height          =   615
      Left            =   7440
      Picture         =   "PNWageReportCvtBAY.frx":0442
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   3960
      Width           =   615
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.CommandButton Command3 
            Height          =   315
            Left            =   6840
            Picture         =   "PNWageReportCvtBAY.frx":0884
            Style           =   1  'Graphical
            TabIndex        =   1
            Top             =   300
            Width           =   375
   ...
   Begin VB.Image Image1 
      Height          =   1200
      Left            =   120
      Picture         =   "PNWageReportCvtBAY.frx":0986
      Top             =   50
      Width           =   6900
   End
   ...
   End
End
Attribute VB_Name = "PNWageReportCvtBAY"
...
If Combo2.Text = Combo2.List(0) Then FNX$ = "A:\ATPKO12.DAT"
If Combo2.Text = Combo2.List(1) Then FNX$ = "B:\ATPKO12.DAT"

FileQ$ = Trim(ProgDir) + "\ATPKO12.TMP"
Open FileQ$ For Random As #15 Len = Len(ATPKO)

PID = Label2(0).Caption

JH$ = Right(PID, 3)
MQ$ = Mid(PID, 5, 2)

CXQ$ = JH$ + MQ$ + Mid(PID, 3, 2)

XTL = 0
i = 1

Cri$ = "ReportID = '" + Label2(0).Caption + "' and Balance > 0 and BankID like 'BAY???'"
If PNwrTB.RecordCount > 0 Then
PNwrTB.FindFirst Cri$
Do While Not PNwrTB.NoMatch
 i = i + 1
 VX$ = Format(PNwrTB.Fields("Balance").Value, "##0.00")
 ...
 ENM$ = Left(UCase(PNwrTB.Fields("EngName").Value), 20)
 ENM$ = ENM$ + Space(20 - Len(ENM$))

 ATPKO = Mid(Combo1.Text, 4, 3) + Label2(1).Caption + BKIX$ + ENM$ + VX$ + Space(26) + CXQ$ + Space(48) + Chr$(13) + Chr$(10)

 StatusBar1.Panels(1).Text = "Processing...Transaction No. " + Format(i, "#,##0")
 ProgressBar1.Value = (i / (PNwrTB.RecordCount + 1)) * 100
...
ATPKO = Mid(Combo1.Text, 4, 3) + Left(Label2(1).Caption, 3) + MaskEdBox1.Text + UCase(Left(SysCITB.Fields("E_Company_Name").Value, 30)) + "712                           A" + CXQ$ + Nx$ + VX$ + Space(26) + Chr$(13) + Chr$(10)
...
Close #15
Open FileQ$ For Random As #15 Len = Len(ATPKO3)
...
If Mid(ATPKO, 7, 10) < Mid(ATPKO2, 7, 10) Then
...
PNBankBranchInfo.CallNotice "CBAY"
...
Do While Not PNbbTB.EOF
 If Left(PNbbTB.Fields("BankBRID").Value, 3) = "BAY" And PNbbTB.Fields("UsePRSystem").Value = True Then
