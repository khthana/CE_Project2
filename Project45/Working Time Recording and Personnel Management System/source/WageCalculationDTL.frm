...
Begin VB.Form WageCalculationDTL 
   ...
   ClientTop       =   330
   ClientWidth     =   8775
   Icon            =   "WageCalculationDTL.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame Frame2 
      Caption         =   "List of Wage Calculation Detail"
      Height          =   4575
      Left            =   240
      TabIndex        =   8
      Top             =   1560
   ...
   End
End
Attribute VB_Name = "WageCalculationDTL"
...
tCmd.CommandText = "pr_wage_calc_detail_select_by_CalDate"
...
Do While Not tRs.EOF
    ...
    MSHFlexGrid1.AddItem tRs("EID") + Chr(9) + NM$ + Chr(9) + Format(tRs("TotalIncomes"), "#,##0.00") + Chr(9) + Format(tRs("TotalDeduction"), "#,##0.00") + Chr(9) + Format(tRs("NetPay"), "#,##0.00") + Chr(9) + Format(tRs("PaidAmount"), "#,##0.00") + Chr(9) + Format(tRs("NetPay") - tRs("PaidAmount"), "#,##0.00")
...
Set tCmd.ActiveConnection = sConn
tCmd.CommandType = adCmdStoredProc
tCmd.CommandText = "pr_wage_calc_detail_amount_summary"

Set tRs = tCmd.Execute

If tRs.EOF = False Then
    Label2(3).Caption = Format(tRs("TotalIncomes"), "#,##0.00")
    Label2(4).Caption = Format(tRs("TotalDeduction"), "#,##0.00")
...
frmCalled.ModifyDetail Label2(0).Caption, MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 0)
Me.Enabled = False
...
frmCalled.Show
Set frmCalled.Caller = Me
frmCalled.ViewDetail Label2(0).Caption, MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 0)
Me.Enabled = False
...
Dim frmCalled As New WageCalculationDTLCreate
...
tCmd.CommandType = adCmdStoredProc
tCmd.CommandText = "pr_wage_calc_detail_delete"

Set tPr(0) = tCmd.CreateParameter("CID", adVarChar, adParamInput, 3, CurCmp)
Set tPr(1) = tCmd.CreateParameter("BeginDate", adDBTimeStamp, adParamInput, 8, CDateX(Label2(1).Caption))
Set tPr(2) = tCmd.CreateParameter("EndDate", adDBTimeStamp, adParamInput, 8, CDateX(Label2(2).Caption))
Set tPr(3) = tCmd.CreateParameter("CalDate", adDBTimeStamp, adParamInput, 8, CDateX(Label2(0).Caption))
...
Public Sub DTLDisplay(ByVal CalDate As String, ByVal BeginDate As String, ByVal EndDate As String)
Dim i As Long

Label2(0).Caption = CalDate
Label2(1).Caption = BeginDate
Label2(2).Caption = EndDate
