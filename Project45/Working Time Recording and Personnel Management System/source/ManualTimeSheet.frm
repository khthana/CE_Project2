...
Begin VB.Form ManualTimeSheet 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CommandButton Command1 
         ...
         Height          =   315
         Left            =   7680
         Picture         =   "ManualTimeSheet.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   300
         Width           =   375
   ...
   Begin VB.Frame Frame1 
      Caption         =   "List of Manual Time Sheet"
      Height          =   4575
      Left            =   240
      TabIndex        =   16
      Top             =   1920
      ...
      Begin VB.CommandButton Command6 
         Caption         =   "&Disapprove All Above Records"
         Height          =   375
         Left            =   4680
         TabIndex        =   15
         Top             =   4080
      ...
      Begin VB.CommandButton Command2 
         Caption         =   "&Approve All Above Records"
         Height          =   375
         Left            =   2280
         TabIndex        =   14
         Top             =   4080
   ...
   End
End
Attribute VB_Name = "ManualTimeSheet"
...
If Check1.Value = Unchecked Then
    ...
    tCmd.CommandText = "ta_manual_time_sheet_select_by_WorkDate"
    ...
    If Check2.Value = Unchecked Then
        aPr(1).Value = Null
    Else
        aPr(1).Value = DateFilter(DTPicker1.Value)
    End If
    
    If Check3.Value = Unchecked Then
        aPr(2).Value = Null
    Else
        aPr(2).Value = DateFilter(DTPicker2.Value)
    End If
...
Else
    ...
    Set tCmd.ActiveConnection = sConn
    tCmd.CommandType = adCmdStoredProc
    tCmd.CommandText = "ta_manual_time_sheet_select_by_EID"
    ...
    If Check2.Value = Unchecked Then
        bPr(2).Value = Null
    Else
        bPr(2).Value = DateFilter(DTPicker1.Value)
    End If
    
    If Check3.Value = Unchecked Then
        bPr(3).Value = Null
    Else
        bPr(3).Value = DateFilter(DTPicker2.Value)
    End If
...
Do While Not tRs.EOF
    ...
    MSHFlexGrid1.AddItem tRs("EID") + Chr(9) + Format(tRs("WorkDate"), "dd/MM/yyyy") + Chr(9) + Format(tRs("FromTime"), "HH:mm:ss") + Chr(9) + Format(tRs("ToTime"), "HH:mm:ss") + Chr(9) + tRs("IncomesCode") + Chr(9) + tRs("JobCode") + Chr(9) + Sec2HourDSP(tRs("WorkSec")) + Chr(9) + Sec2HourDSP(tRs("ApprovedWorkSec")) + Chr(9) + Sec2HourDSP(tRs("LateSec")) + Chr(9) + Sec2HourDSP(tRs("ApprovedLateSec")) + Chr(9) + Format(tRs("Approved"), "Yes/No") + Chr(9) + Format(tRs("Transferred"), "Yes/No") + Chr(9) + Format(tRs("FromTime"), "dd/MM/yyyy HH:mm:ss")
...
Co = MsgBox("Do you want to approve these records?", vbQuestion + vbYesNo)
If Co = vbYes Then
    Approve (True)
    Command5_Click
End If
...
Dim frmCalled As New ManualTimeSheetDetail
...
frmCalled.ModifyDetail MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 0), MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 1), MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 12)
...
Dim frmCalled As New ManualTimeSheetGenerate
Load frmCalled
frmCalled.Show
Set frmCalled.Caller = Me
...
Co = MsgBox("Do you want to disapprove these records?", vbQuestion + vbYesNo)
If Co = vbYes Then
    Approve (False)
    Command5_Click
End If
...
Dim tRs As New ADODB.Recordset
Dim tCmd As New ADODB.Command
Dim aPr(5) As ADODB.Parameter
Dim bPr(6) As ADODB.Parameter
Dim i As Long
...
If Check1.Value = Unchecked Then
    ...
    Set tCmd.ActiveConnection = sConn
    tCmd.CommandType = adCmdStoredProc
    tCmd.CommandText = "ta_manual_time_sheet_approve_by_WorkDate"
    
    Set aPr(0) = tCmd.CreateParameter("CID", adVarChar, adParamInput, 3)
    Set aPr(1) = tCmd.CreateParameter("FromDate", adDBTimeStamp, adParamInput, 8)
    Set aPr(2) = tCmd.CreateParameter("ToDate", adDBTimeStamp, adParamInput, 8)
    Set aPr(3) = tCmd.CreateParameter("TransferOption", adTinyInt, adParamInput, 1)
    Set aPr(4) = tCmd.CreateParameter("ApproveFlag", adBoolean, adParamInput, 1)
    Set aPr(5) = tCmd.CreateParameter("LastModifyUserID", adVarChar, adParamInput, 10)
    ...
    If ApproveFlag = False Then aPr(4).Value = 0
    If ApproveFlag = True Then aPr(4).Value = 1
...
Else
    ...
    Set tCmd.ActiveConnection = sConn
    tCmd.CommandType = adCmdStoredProc
    tCmd.CommandText = "ta_manual_time_sheet_approve_by_EID"
    ...
    Set bPr(3) = tCmd.CreateParameter("ToDate", adDBTimeStamp, adParamInput, 8)
    Set bPr(4) = tCmd.CreateParameter("TransferOption", adTinyInt, adParamInput, 1)
    Set bPr(5) = tCmd.CreateParameter("ApproveFlag", adBoolean, adParamInput, 1)
    Set bPr(6) = tCmd.CreateParameter("LastModifyUserID", adVarChar, adParamInput, 10)
    ...
    If ApproveFlag = False Then bPr(5).Value = 0
    If ApproveFlag = True Then bPr(5).Value = 1
