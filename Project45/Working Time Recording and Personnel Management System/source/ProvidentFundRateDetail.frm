...
Begin VB.Form ProvidentFundRateDetail 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Provident Fund Rate Information"
   ClientHeight    =   7500
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8415
   Icon            =   "ProvidentFundRateDetail.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "ProvidentFundRateDetail"
...
Dim iPr(10) As New ADODB.Parameter
Dim i As Long
Dim j As Long
...
Select Case frmStatus
    ...
    Case Is = FormState.Modify
        ...
        tCmd.CommandText = "pr_pfr_delete"
...
For i = 1 To numRec
    ...
    tCmd.CommandType = adCmdStoredProc
    tCmd.CommandText = "pr_pfr_insert"
    ...
    Set iPr(3) = tCmd.CreateParameter("PFLevel", adTinyInt, adParamInput, 1)
    Set iPr(4) = tCmd.CreateParameter("PFBaseBasis", adCurrency, adParamInput, 8)
    Set iPr(5) = tCmd.CreateParameter("PFBaseCutOff", adCurrency, adParamInput, 8)
    Set iPr(6) = tCmd.CreateParameter("PFRateEmp", adCurrency, adParamInput, 8)
    Set iPr(7) = tCmd.CreateParameter("PFAccumulatedBFEmp", adCurrency, adParamInput, 8)
    Set iPr(8) = tCmd.CreateParameter("PFRateEmp", adCurrency, adParamInput, 8)
    Set iPr(9) = tCmd.CreateParameter("PFAccumulatedBFEmp", adCurrency, adParamInput, 8)
    Set iPr(10) = tCmd.CreateParameter("CreateUserID", adVarChar, adParamInput, 10)
    ...
    iPr(8).Value = CFC(MSHFlexGrid1.TextMatrix(i, 5))
    iPr(9).Value = CFC(MSHFlexGrid1.TextMatrix(i, 6))
    iPr(10).Value = CurUser
...
'======== Insert new row
MSHFlexGrid1.AddItem TTi(Text1.Text) + Chr(9) + TTc(Text2.Text) + Chr(9) + TTc(Text3.Text) + Chr(9) + TTc(Text4.Text) + Chr(9) + TTc(Text5.Text) + Chr(9) + TTc(Text6.Text) + Chr(9) + TTc(Text7.Text)
...
tCmd.CommandType = adCmdStoredProc
tCmd.CommandText = "pr_pfr_select"
...
If tRs.EOF = False Then
    ...
    Do While Not tRs.EOF
        ...
        MSHFlexGrid1.AddItem TTi(tRs("PFLevel")) + Chr(9) + TTc(tRs("PFBaseBasis")) + Chr(9) + TTc(tRs("PFBaseCutOff")) + Chr(9) + TTc(tRs("PFRateEmp")) + Chr(9) + TTc(tRs("PFAccumulatedBFEmp")) + Chr(9) + TTc(tRs("PFRateCmp")) + Chr(9) + TTc(tRs("PFAccumulatedBFCmp"))
...
Text6.Text = MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 5)
Text7.Text = MSHFlexGrid1.TextMatrix(MSHFlexGrid1.Rowsel, 6)
