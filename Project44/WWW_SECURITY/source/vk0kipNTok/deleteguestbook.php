...
if ($HTTP_POST_VARS["submit"] == "delete") {		// มีการกดปุ่ม delete
		$array = $HTTP_POST_VARS["array"];
		if (count($array) == 0)  // เช็คว่าได้ทำการ check guestbook ที่จะเปลี่ยน 1 ช่องหรือไม่
			echo "<center><font  size=4  color=red><b>คลิกที่ช่องสี่เหลี่ยมเพื่อเลือก guestbook ที่ต้องการจะแก้ไข</b></font></center><br>";
		else  // ถ้ามีการคลิกเพื่อจะลบอย่างน้อย 1 ช่อง
		{
			foreach($array as $key => $val) {
					$sql3 = "DELETE FROM guestbook WHERE ID='$val'";
					mysql_query($sql3) or die ("ไม่สามารถ DELETE ข้อมูลในตาราง guestbook ได้");
			}  // end foreach
		}  // end else
		...
		echo "ขออภัยไม่พบข้อมูลดังกล่าว";
			...
			if ( ($pg >= 1 && ($foundfloat == false)) || ($pg == "") ) {
				...
				if ($Page_no <= $Maxcount) {		// $Page_no คือ เลขหน้าที่จะดู   ส่วน $Maxcount คือ จำนวนหน้าที่มีทั้งหมด
					...
					$sql2 = "SELECT ID, name, email_addr, suggest, date_format(date, '%e %b %Y, %H:%i') as date2
...
?>					
					<center><font  color=purple  size=4>ลบคำแนะนำติชม</font></center>
...
?>
							<table  border=0  cellpadding=1  cellspacing=1  width=70%  align=center>
								<input  type="checkbox"  name="array[]"  value="<? echo "$id"; ?>">
								<? echo $number; ?>
								<tr  id=w3>  <td  width=20%><b>&nbsp;ชื่อ</b></td>  <td  width=80%><? echo "&nbsp;$name"; ?></td>  </tr>
								<tr  id=w2>  <td><b>&nbsp;อีเมลล์</b></td>  <td  width=80%><a href="mailto:$email_addr"><? echo "&nbsp;$email_addr"; ?></a></td>  </tr>
								<tr  id=w3>  <td><b>&nbsp;วันที่</b></td>  <td  width=80%><? echo "&nbsp;$date2"; ?></td>  </tr>
								<tr  id=w2>  <td><b>&nbsp;ข้อเสนอแนะ</b></td>  <td  width=80%><? echo "&nbsp;$suggest"; ?></td>  </tr>
		        				</table>
<?
							$Recs = $Recs + 1;
?>
...
<?				}  // end if ($Page_no <= $Maxcount)
	...
	if ( ($pg == "") || ( ($pg >= 1) && ($Page_no <= $Maxcount) && ($foundfloat == false) ) ) {		    // ถ้าเปิดหน้านี้และยังไม่ได้คลิกหน้าที่จะดู empty($pg)  $Page_no คือ เลขหน้าที่จะดู   ส่วน $Maxcount คือ จำนวนหน้าที่มีทั้งหมด
		...
		$Scriptname = "deleteguestbook.php";
...
//				echo " ".$Ref." ";
	...
	}	// end if ( ($pg == "") || ( ($pg >= 1) && ($Page_no <= $Maxcount) && ($foundfloat == false) ) )
