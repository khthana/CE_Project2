...
// closes last opened tag
	if (normalmode(theform))
		...
		if (tags[0]) {
			theform.content.value += "[/"+ arraypop(tags) +"]";
			}
		else {
			stat('no_tags');
			}
...
// insert [x]yyy[/x] style markup
	if ((normalmode(theform)) || (vbcode=="IMG")) {
		...
		if ((inserttext != null) && (inserttext != ""))
			theform.content.value += "["+vbcode+"]"+inserttext+"[/"+vbcode+"] ";
		}
	else {
		donotinsert = false;
...
// insert two-parameter markup - [x=y]zzz[/x]
	if (normalmode(theform)) {
		if (thevalue != 0) {
			...
			if ((inserttext != null) && (inserttext != ""))
				theform.content.value += "["+thetype+"="+thevalue+"]"+inserttext+"[/"+thetype+"] ";
			}
		}
	else {
		theform.content.value += "["+thetype+"="+thevalue+"]";
...
// inserts named url or email link - [url=mylink]text[/url]
	...
	if ((linkurl != null) && (linkurl != "")) {
		if ((linktext != null) && (linktext != ""))
			theform.content.value += "["+thetype+"="+linkurl+"]"+linktext+"[/"+thetype+"] ";
		else
			theform.content.value += "["+thetype+"]"+linkurl+"[/"+thetype+"] ";
		}
	theform.content.focus();
}
...
// inserts smilie text
	document.vbform.content.value += thesmilie+" ";
	document.vbform.content.focus();
}
