...
if($HTTP_POST_VARS["submit"]=="Update")
{
	  ...
	  if (($Filepdf != "") and ($Filepdf_type!="application/x-zip-compressed"))
		{  //if (!ereg("^.+\.pdf$",$Filepdf_name))								
						$Error=1;			
						print "ชื่อไฟล์ไม่ถูกต้อง (pdf) เท่านั้น<BR>";				
		}
		print "</CENTER></font>";
...
if(isset($HTTP_GET_VARS["id"]) or $Error==1 )
{
		...
		$query = "select * from add_project where IDProject='$id' and IDUser='$uid' ";
...
{
		$sql = "select IDProject,Year,Name_Eng from add_project where IDUser='$uid' order by Year desc";
		$result = mysql_query($sql);
			...
			while ($row=mysql_fetch_array($result))
				{
						...
						print "<tr ID=table1><td>&nbsp;$Year</td><td>&nbsp;";
		...
		{
			print "<center><font size=3><B>คุณยังไม่ได้ Add Project จึงไม่สามารถแก้ไขโปรเจคได้</B></font></center>";
		}
		else
		{
			print "<center><font size=3><B>ไม่สามารถ Query ข้อมูลจากฐานข้อมูลได้</B></font></center>";
		}
}
//----------------------------------------------------------------------------------------------------------------------
curve_close();
