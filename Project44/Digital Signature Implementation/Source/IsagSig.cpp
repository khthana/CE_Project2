...
{	// MD4=32770, MD5=32771, SHA-1=32772
	...
	HRESULT hresult;
	BYTE *DataBuff=new BYTE[BuffLen+18];
	CHAR *hashalg=new CHAR[10], *LenStr=new CHAR[4];
	FORMATETC fmetc;
	STGMEDIUM medium;
	HGLOBAL hGlobal;
	...
	//DWORD length;
	//length =8;
	//CreateOutpB(DataByte, &length, test);
	//AfxMessageBox(test);
	...
	{	
		...
		{	
			...
			//CreateOutpB(DataBuff, &BuffLen, test);
			//AfxMessageBox(test);
		}
	}
	...
	{	
		...
		{
			//AfxMessageBox("test length = 0");
			for(i=0;i<=8;i++)
			{
				DataBuff[count + i] = DataByte[i];
			}	
			count+=8;
		}
		else
	...
	//CString test;
	//DWORD length =8;
	//CreateOutpB(BytePic,&length,test);
	//AfxMessageBox(test);
...
void CIsagSig::GetPic()
{
	//AfxMessageBox("get pic");
	UINT        uNumFormats = 0;
	FORMATETC	etc;
	HGLOBAL     hgData;
	UINT        uDataSize;
	...
	ASSERT ( AfxIsValidAddress ( &theApp.pObject, sizeof(COleDataObject) ));
	
	
	theApp.pObject.BeginEnumFormats();

	//while (theApp.pObject.GetNextFormat(&etc))
    //{
	//	if ( theApp.pObject.IsDataAvailable ( etc.cfFormat))
	//	{
	//		 uNumFormats++;
 	//	}   
	//}
	...
	//while (theApp.pObject.GetNextFormat(&etc))
	//{
		//AfxMessageBox("เข้าลูป");
		etc.cfFormat = CF_METAFILEPICT;
		hgData = theApp.pObject.GetGlobalData(etc.cfFormat);
		if (hgData != NULL)
		{
		...
		DataBuff = (BYTE *)GlobalLock(hgData);

		loop  = floor(uDataSize/BuffLen);
		final = uDataSize % BuffLen;
		if (loop > 0)
		{	
			for (count = 0; count < loop; count++)
			{	
		...
		{	
			...
			{
				//Buff[i-4]=DataBuff[i];
				DataByte[i-4] = DataBuff[i];
				//DataByte++;
			}
			//for(i=1;i<=8;i++)
			//{
				//DataByte[i] = DataByte[i] + Buff[i];
			//}
			length = 8;
			//BytePic = (BYTE *)malloc((unsigned int) *DataByte);
			//BytePic = DataByte;
			//CString test;
		
			//CreateOutpB(Buff, &length, test);
			//AfxMessageBox(test);
			//CreateOutpB(DataByte, &length, test);
			//AfxMessageBox(test);
			//if(!CryptHashData(myhHash, DataByte, length, 0))
			//{	
			//	AfxMessageBox("Hash DATA has failed!");
			//}
			GlobalFree ( hgData );
