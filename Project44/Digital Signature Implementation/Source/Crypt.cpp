...
void CCrypt::ResetContext(CString ProvName)
{	CryptReleaseContext(m_hProv,0);
...
BYTE* CCrypt::SetVarB(BYTE *From, int num)
{	int i;
...
DWORD* CCrypt::SetVarDW(DWORD *From)
{	What = new DWORD;
	*What = *From;
	return What;
}

void CCrypt::CreateOutpB(BYTE *A, DWORD *len, CString &B)
{	DWORD Len, chrnum;
	CHAR *temp, *Result;
...
void CCrypt::CreateOutpDW(DWORD *A, CString &B)
{	DWORD Len;
...
void CCrypt::HashFile(CString Infile, BYTE alg)
...
void CCrypt::HashStream(BYTE alg)
{	// MD4=32770, MD5=32771, SHA-1=32772
	ALG_ID algid;
...
void CCrypt::FindHashVal(HCRYPTHASH hHash)
...
bool CCrypt::SignFile(CString ProvName, CString Infile, bool isstream)
...
BOOL CCrypt::VerifySig(CString ProvName, CString Infile, bool isstream)
...
HCRYPTHASH CCrypt::GethHash()
{	return m_hHash;
}

BYTE* CCrypt::GetSignVal()
{	return m_pSignVal;
}

DWORD CCrypt::GetSignValLen()
{	return *m_pSignValLen;
}

void CCrypt::SetSign(BYTE *sig, DWORD siglen, BYTE alg)
{	m_pSignVal = sig;
...
void CCrypt::SetPubKey(BYTE *pub, DWORD publen)
{	m_pPubVal = pub;
