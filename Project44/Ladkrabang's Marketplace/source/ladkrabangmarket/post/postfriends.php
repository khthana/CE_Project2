...
if($member){
	...
	// ป้องกันคำหยาบ
		...
		$fullname = censor($fullname);
		$detail = censor($detail);
		$nickname = censor($nickname);
		$faculty = censor($faculty);
		$e_mail = censor($e_mail);
		$department = censor($department);
		$icq = censor($icq);
		$tel = censor($tel);
		$pager = censor($pager);
		$email = censor($email);
...
mysql_connect($host,$user,$passwd);
	...
	$max_code=mysql_db_query($dbname,"select  MAX(code) from friends_data");
...
$sql2="update friends_data set fullname='$fullname',nickname='$nickname',faculty='$faculty',department='$department',e_mail='$e_mail',detail='$detail',icq='$icq',duration='$duration',tel='$tel',pager='$pager',year='$year' where code='$code' ";
...
//ตาราง  friends_data
	$sql2 = "insert into friends_data (code,fullname,nickname,faculty,department,e_mail,icq,tel,pager,detail,duration,year) values('$code','$fullname','$nickname','$faculty','$department','$e_mail','$icq','$tel','$pager','$detail','$duration','$year')";
//ตารางผู้ลงประกาศ
...
?>
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; URL=../showfriend.php?Code=<?echo $code?> ">
	</head>
  ...
  <table width=60% border=1 bordercolor=#9999FF bgcolor=#f0ffff cellpadding=2 cellspacing=0>
	...
	<?/*
		...
		echo "<font size=2 face='MS Sans Serif'>";
		echo "[ <a href='postfriends.html'>ลงประกาศใหม่</a> | ";
//echo" <a href='javascript:window.close()'>ปิดหน้านี้</a> ]";	
 echo" <a href='../showfriends.php?Code=$code'>แสดงประกาศ</a> ]";
		echo "<font>";
		echo "</center>";*/
			}
	else {
