...
<?
 ...
 if(strlen($status)==0){$status="แจ้งหาย";}
...
$date=date("ymd");
	...
	$max_code=mysql_db_query($dbname,"select  MAX(code) from lost_data");
...
$sql2="update lost_data set detail='$detail',duration='$duration' where code='$code' ";
...
//ตาราง  news_data
	$sql2= "insert into lost_data (code,detail,duration)		values('$code','$detail','$duration')";
//ตารางผู้ลงประกาศ
...
<html>
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; URL=../showlost.php?Code=<?echo $code?> ">
	</head>
  ...
  <table width=60% border=1 bordercolor=#9999FF bgcolor=#f0ffff cellpadding=2 cellspacing=0>
	...
	<font color=blue><b><? echo $code; ?></b></font> โดยไม่ต้องกดปุ่มใดๆ 
		...
		echo "<font size=2 face='MS Sans Serif'>";
		echo "[ <a href='postlost.html'>ลงประกาศใหม่</a> | ";
//	echo" <a href='javascript:window.close()'>ปิดหน้านี้</a> ]";
 echo" <a href='../showlost.php?Code=$code'>แสดงประกาศ</a> ]";
		echo "<font>";
		echo "</center>";
*/
	}
