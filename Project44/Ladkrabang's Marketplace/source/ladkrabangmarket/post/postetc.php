...
$date=date("ymd");
	...
	$max_code=mysql_db_query($dbname,"select  MAX(code) from etc_data");
...
$sql2="update etc_data set brand='$brand',model='$model',condition='$condition',detail='$detail',howsend='$howsend',duration='$duration',usedyear='$usedyear',usedmonth='$usedmonth',percent='$percent' where code='$code' ";
...
//ตาราง  etc_data
	$sql2= "insert into etc_data (code,brand,model,condition,detail,howsend,usedmonth,usedyear,duration,percent)		values('$code','$brand','$model','$condition','$detail','$howsend','$usedmonth','$usedyear','$duration','$percent')";
//ตารางผู้ลงประกาศ
...
?>
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; URL=../showetc.php?Code=<?echo $code?> ">
	</head>
  ...
  <table width=60% border=1 bordercolor=#9999FF bgcolor=#f0ffff cellpadding=2 cellspacing=0>
	...
	<?/*
		...
		echo "<font size=2 face='MS Sans Serif'>";
		echo "[ <a href='postetc.html'>ลงประกาศใหม่</a> | ";
//	echo" <a href='javascript:window.close()'>ปิดหน้านี้</a> ]";
 echo" <a href='../showetc.php?Code=$code'>แสดงประกาศ</a> ]";
		echo "<font>";
		echo "</center>";
