...
if(strlen($color)==0) { $color="ไม่ระบุ" ;}
if(strlen($license)==0) { $license="ไม่ระบุ" ;}	
...
mysql_connect($host,$user,$passwd);
	...
	$max_code=mysql_db_query($dbname,"select  MAX(code) from vehicle_data");
...
$sql2="update vehicle_data set brand='$brand',model='$model',color='$color',license='$license',condition='$condition',detail='$detail',howsend='$howsend',duration='$duration',usedyear='$usedyear',usedmonth='$usedmonth',percent='$percent' where code='$code' ";
...
//ตาราง  vehicle_data
	$sql2 = "insert into vehicle_data (code,brand,model,color,license,condition,detail,howsend,usedyear,usedmonth,duration,percent)	values('$code','$brand','$model','$color','$license','$condition','$detail','$howsend','$usedyear','$usedmonth','$duration','$percent')";
//ตารางผู้ลงประกาศ
...
?>
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; URL=../showveh.php?Code=<?echo $code?> ">
	</head>
...
/*
		...
		echo "<font size=2 face='MS Sans Serif'>";
		echo "[ <a href='postvehicle.html'>ลงประกาศใหม่</a> | ";
//	echo" <a href='javascript:window.close()'>ปิดหน้านี้</a> ]";
 echo" <a href='../showveh.php?Code=$code'>แสดงประกาศ</a> ]";
		echo "<font>";
		echo "</center>";
