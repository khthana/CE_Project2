...
if(strlen($price)==0) { $price="ไม่ระบุ"; }
if(strlen($name)==0) { $name="ไม่ระบุ"; }
if(strlen($type)==0) { $type="ไม่ระบุ" ;}
if(strlen($address)==0) { $address="ไม่ระบุ" ;}
if(strlen($no_room)==0) { $no_room="ไม่ระบุ" ;}
if(strlen($size)==0) { $size="ไม่ระบุ" ;}
if(strlen($tela)==0) { $tela="ไม่ระบุ" ;}
...
if($member){
	...
	$no_room = htmlspecialchars($no_room);
...
mysql_connect($host,$user,$passwd);
	...
	$max_code=mysql_db_query($dbname,"select  MAX(code) from address_data");
...
$sql2="update address_data set name='$name',type='$type',address='$address',no_room='$no_room',detail='$detail',tela='$tela',size='$size',duration='$duration' where code='$code' ";
...
//ตาราง post
	...
	//ตาราง address_data
	$sql2 = "insert into address_data (code,name,type,address,no_room,size,tela,detail,duration) values('$code','$name','$type','$address','$no_room','$size','$tela','$detail','$duration')";
//ตารางผู้ลงประกาศ
...
if($member && $qpic_name) {
	...
	<META HTTP-EQUIV="REFRESH" CONTENT="3; URL=../showaddress.php?Code=<?echo $code?> ">
	</head>
  ...
  <table width=60% border=1 bordercolor=#9999FF bgcolor=#f0ffff cellpadding=2 cellspacing=0>
	...
	<font color=blue><b><? echo $code; ?></b></font> โดยไม่ต้องกดปุ่มใดๆ 
		...
		echo "<font size=2 face='MS Sans Serif'>";
		echo "[ <a href='postaddress.html'>ลงประกาศใหม่</a> | ";
//		echo" <a href='javascript:window.close()'>ปิดหน้านี้</a> ]";
 echo" <a href='../showaddress.php?Code=$code'>แสดงประกาศ</a> ]";
		echo "<font>";
		echo "</center>";  */
