...
Architecture rtl of ALU_Alpha is
	...
	constant opcode_mul1		: unsigned(7 downto 0) := "00000100";
	constant opcode_mul2		: unsigned(7 downto 0) := "10010000";
	constant opcode_imul1		: unsigned(7 downto 0) := "00000101";
	constant opcode_imul2		: unsigned(7 downto 0) := "10010100";
	constant opcode_div1		: unsigned(7 downto 0) := "00000110";
	constant opcode_div2		: unsigned(7 downto 0) := "10011000";
	constant opcode_idiv1		: unsigned(7 downto 0) := "00000111";
	constant opcode_idiv2		: unsigned(7 downto 0) := "10011100";
...
function mult (a, b	: unsigned) return unsigned is
    variable a_ext	: unsigned(31 downto 0);
    variable b_ext	: unsigned(31 downto 0);
...
begin
	...
	for i in b_ext'reverse_range loop
		if  b_ext(i) = '1' then
			result := result + a_ext;
		end if;
		a_ext := shift_left(a_ext,1);
        end loop;
...
function imult (a, b	: signed) return signed is
    variable a_ext	: signed(31 downto 0);
    variable b_ext	: signed(31 downto 0);
...
procedure div (dividend, divisor : signed;opcode : unsigned;qout,remain : out signed) is
	variable A		: signed(16 downto 0);
	variable B		: signed(17 downto 0);
	variable P		: signed(17 downto 0);
	variable CNTR		: signed(17 downto 0);
	variable CNTRnext	: signed(17 downto 0);
	variable Pnext		: signed(17 downto 0);
	variable Anext		: signed(17 downto 0);
	variable A_ext		: signed(17 downto 0);
	variable test_a,test_b  : signed(15 downto 0);

  begin

	if (opcode = opcode_div1) or (opcode = opcode_div2) then
		A := '0' & dividend;
	    B := "00" & divisor;
	else
		test_a := dividend;
		test_b := divisor;
		if (test_a < 0 xor test_b < 0) then
			test_a := abs(test_a);
			test_b := abs(test_b);
		end if;
		A := '0' & test_a;
    ...
    P := (others => '0');
    CNTR(17 downto 1) := (others => '0');
    CNTR(0) := '1';
	CNTRNext(1 downto 0) := "11";
	A_ext := '0' & A;
	Anext := shift_left(A_ext,1);
	Pnext    := (others => '0');
	...
	for i in 16 downto 0 loop
		...
		if CNTR(17) = '1' then
                   ...
                   if Pnext >= B then
                      P := Pnext - B;
                      A(16 downto 1) := Anext(16 downto 1);
                      A(0) := '1';
                   else
                      -- result is negative
                      P := Pnext;
		...
		A_ext := '0' & A;
		Anext := shift_left(A_ext,1);
		Pnext := shift_left(P,1);
	        CNTRnext := shift_left(CNTR,1);  
	...
	if (opcode = opcode_div1) or (opcode = opcode_div2) then
		qout := A(15 downto 0);
		remain := P(15 downto 0);
	else
		test_a := A(15 downto 0);
		test_b := P(15 downto 0);
...
begin
	process(clk)
		...
		variable tmp_mul			: unsigned(31 downto 0);
		variable tmp_div		: signed(15 downto 0)	:= (others => 'Z');
		variable tmp_rem		: signed(15 downto 0)	:= (others => 'Z');
		...
		begin
			...
			if reset ='1' then
				...
				when opcode_updateflag =>
					psw <=	unsigned(input_a(7 downto 0));
				...
				when opcode_mul1 | opcode_mul2 =>
		  			tmp_mul := mult(unsigned(input_a),unsigned(input_b));
					temp <= signed(tmp_mul);
					if (temp < 0) then
						s <='1';
						z <='0';
					...
					parity_bit := count_parity(low);
					p <= parity_bit xor (count_parity(high));
		
				when opcode_imul1 | opcode_imul2 =>
		  			temp <= imult(input_a,input_b);
					if (temp < 0) then
						s <='1';
						z <='0';
					end if;
				...
				when opcode_div1 | opcode_div2 | opcode_idiv1 | opcode_idiv2 =>
					div(input_a,input_b,opcode,tmp_div,tmp_rem);
					low <= tmp_div;
					high <= tmp_rem;
					if (low < 0) then
						s <='1';
...
component Mux
	...
	begin
		ALU1 : ALU_Alpha port map(s_a1,s_b1,opcode1,s_output1,flag1,reset,clk_big_one);
		ALU2 : ALU_Omega port map(s_a2,s_b2,opcode2,s_output2,flag2,reset,clk_big_one,jmp);
