...
{ This method is called to determine whether to assign a default
  bg. icon to the component. Descendant classes can override the method
...
begin
  ...
  if (Assigned(FBackgroundIcon)) and not (FBackgroundIcon.Empty) then
    ...
    rc := TransparentBitmapToIcon(FTextBitmap, Ico, clNone)
  else
    // Just draw text; no background icon
    rc := BitmapToIcon(FTextBitmap, Ico, clNone);
  if rc then
  begin
    Icon.Assign(Ico);
...
begin
  ...
  try
    ...
    Bmp := TBitmap.Create;
    Bmp.Canvas.Brush.Color := clBtnFace;    // Preserves transparency of bg. icon
    BitmapImageList.GetBitmap(0, Bmp);
    Bitmap.Transparent := True;
    Bitmap.TransParentColor := FColor;
    Bmp.Canvas.Draw(0, 0, Bitmap);

    BitmapImageList.AddMasked(Bmp, MaskColor);
