...
//      RouterConsole.output += "Delete Route Success\n";
    }//end if
    else {
      RouterConsole.output += "Wrong Delete\n";
    }
  }
/*
  public void run(){
    ...
    Router rtmp = null;
    boolean found=false;
    while((a<Frame1.vRouter.size())&&(!found)){
      rtmp = (Router)Frame1.vRouter.elementAt(a);
      int b=0;
      while((b<rtmp.getVInt().size())&&(!found)){
        Interface i1 = (Interface)rtmp.getVInt().elementAt(b);
        if (i1.getIpAddress().equalsIgnoreCase(this.ipDestination)){
          found = true;
        }
        else{
          b++;
