...
public class Ospf extends Thread{
   ...
   public void sendHelloPackage(Router rin)
  {
   for (int aa=0;aa<rin.getVInt().size();aa++)
   {
	...
	if (This_intf.getStatus().equalsIgnoreCase("up"))
	{
	  ...
	  int bb=0;
	  while ((bb<Frame1.vRouter.size())&&(rin.getFlag()=='o'))
	  {
  ...
  public void shortest_path(Router rin){
    ...
    int[][] arr = new int[w][w];
    init(arr,w,0);
    Router rr = (Router)Frame1.vRouter.lastElement() ;
    //Router rr =rin;
    ...
    initFloid(arr,Q,w);
    floidAlgo(Q,path,w);
    Frame1.pathx = new Vector[w][w];
    finalFindPath(path,w);
    adjustPathX(Frame1.pathx,Q,w);
    printPathX(Frame1.pathx,w);
  }
...
//    System.out.println("************** print task ***************");
	...
	int ii = i+1;
	int jj = j+1;
	Frame1.pathx[i][j] = new Vector();
	Frame1.pathx[i][j].addElement(Frame1.vRouter.elementAt(i));
	printPathBetween(path,w,i,j,Frame1.pathx[i][j]);
	Frame1.pathx[i][j].addElement(Frame1.vRouter.elementAt(j));
      }
    }
  }
  public void printPathBetween(int[][] p,int w, int src, int des,Vector pathList){
    if (p[src][des] != -1) {
      ...
      int m=middle+1;
      pathList.addElement(Frame1.vRouter.elementAt(middle));
  ...
  public void adjustPathX(Vector[][] pathx,int[][] Q,int w){
    ...
    for(int i=0;i<w;i++){
      ...
      pathx[i][i].addElement(Frame1.vRouter.elementAt(i));
      for (int j=0;j<w;j++){
	if (Q[i][j] >= maxW){
...
//      System.out.println();
  ...
  /*---------------------------------------------------------------------------------*/
   ...
   private Router findR(String name)
   {
      ...
      {
	r = (Router)Frame1.vRouter.elementAt(i);
    ...
    //public void setWeight(Router r,Router neighbor,int w){
