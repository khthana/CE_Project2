...
Public Function RecvTSBuyer(ByVal xmlPath As String) As Boolean
        Dim conn As ADODB.Connection
        Dim rcs As ADODB.Recordset
        Dim objxml As XML
        Dim tsid As String
        ...
        rcs.Open "select * from BuyerTS where TransactionID = """ & tsid & """", conn, 1, 3
        If rcs.RecordCount > 0 Then
                Do While rcs.RecordCount = 0
                        rcs.Delete adAffectCurrent
                Loop
        Else
                rcs.AddNew
                rcs!TransactionID = tsid
                rcs!TotalPrice = objxml.query("TotalPayment", xmlPath)
                rcs!DueDate = objxml.query("DueDate", xmlPath)
                rcs.Update
        End If
        rcs.Close
...
Public Function RecvTSMarket(ByVal xmlPath As String) As Boolean
        Dim conn As ADODB.Connection
        Dim rcs As ADODB.Recordset
        Dim objxml As XML
        Dim tsid As String
        ...
        objxml.getParameter xmlPath
        tsid = objxml.query("TransactionID", xmlPath)
        rcs.Open "select * from MarketTS where TransactionID = """ & tsid & """", conn, 1, 3
        If rcs.RecordCount > 0 Then
                Do While rcs.RecordCount = 0
                        rcs.Delete adAffectCurrent
                Loop
        Else
                rcs.AddNew
                rcs!TransactionID = tsid
                rcs!InvoiceNo = objxml.query("InvoiceNo", xmlPath)
                rcs!TotalPrice = objxml.query("TotalPayment", xmlPath)
                rcs!DueDate = objxml.query("DueDate", xmlPath)
                rcs!SellerBankID = objxml.query("BuyerBankID", xmlPath)
                rcs!BuyerBankID = objxml.query("SellerBankID", xmlPath)
                rcs.Update
        End If
        rcs.Close
        conn.Close
...
Public Function GenTSPaid_Reject(ByVal tsid As String, ByRef dest As String) As Boolean
        Dim conn As ADODB.Connection
        Dim rcs As ADODB.Recordset
        Dim rcs2 As ADODB.Recordset
        Dim rcs3 As ADODB.Recordset
        Dim objxml As XML
        ...
        Set rcs3 = New ADODB.Recordset
        conn.ConnectionString = DBconString
        conn.Open
        rcs.Open "select * from BuyerTS where TransactionID = """ & tsid & """", conn, 1, 3
        rcs2.Open "select * from MarketTS where TransactionID = """ & tsid & """", conn, 1, 3
        If rcs.RecordCount > 0 Then
                If rcs2.RecordCount > 0 Then
                        rcs3.Open "select TransactionID, InvoiceNo from MarketTS where TransactionID = """ & tsid & """", conn
                        objxml.SetRootName = "TransactionPayment"
                        objxml.SetSubRootName = "Manager_market"
                        If (rcs!TotalPrice = rcs2!TotalPrice) And (rcs!DueDate = rcs2!DueDate) Then
                                '**** ∑”°“√‚Õπ‡ß‘π *****
                                       objxml.SetStatus = SPaid
                                       dest = App.Path & TSdir & "\Paid\" & "TS" & tsid & "_Paid.xml"
                                       objxml.Recordset2XMLwithoutDTD rcs3, dest, dtdTSPaid
                        Else
                                '******** Reject ********
                                       objxml.SetStatus = SReject
                                       dest = App.Path & TSdir & "\Reject\" & "TS" & tsid & "_Reject.xml"
                                       objxml.Recordset2XMLwithoutDTD rcs3, dest, dtdTSReject
                        End If
                        GenTSPaid_Reject = True
                 Else
                        MsgBox "This Market TransactionID doesn't in database"
                        GenTSPaid_Reject = False
                        dest = ""
                End If
        Else
                MsgBox "This Buyer TransactionID doesn't in database"
                GenTSPaid_Reject = False
                dest = ""
        End If
