...
public class SwitchCMD {
  ...
  private final ListCommand intCmd = new ListCommand(new String[]{"?","interface arg_intname","ip address arg_ip arg_ip","encapsulation","shutdown","no shutdown","ring-speed arg_num","description arg_string","bandwidth arg_num","clock rate arg_num","exit",""});
...
//  private int cmdNum = -1;
  ...
  //------Function--------//
  public void runCommand(Router router,String cmdline,RouterConsole rconsole){
    accept = false;
    ...
    switch (router.getMode()){
      ...
      case 6 :
                ...
                allCmd = lineCmd;
                break;
      default : RouterConsole.output  += "Error in Mode\n";
    }//end switch
    allCmd.resetList();
    String usercmd="";
...
//        System.out.println("Accept");
        ...
        RouterConsole.output += error;
//        System.out.println(error);
      }//end else
    }//end if
    else if (nummatch > 1){
      RouterConsole.output += ambigous + "\"" + cmdline + "\"\n";
...
//      System.out.println("% Invalid input detected at '^' marker.");
    ...
    if (accept){
      if (commandlist.equals("enable")){
        ...
        switch(router.getMode()){
          ...
          case 5 : c.showCmd5();break;
          case 6 : c.showCmd6();break;
          default : RouterConsole.output += error;
        }
      }
  ...
  public void sendCMD(Router router,String cmdline){
    ...
    if (cmdline.startsWith("?")){
      ...
      else {RouterConsole.output += error;}
    }
    ...
    //-----------Mode 3--------//
      ...
      if (router.getMode() == 3){
       ...
       RouterConsole.output = "";
        switch (router.getMode()){
          case 1 : router.setMode(0);RouterConsole.output="reset";break;
          case 3 : router.setMode(2);RouterConsole.output="%SYS-5-CONFIG_I: Configured from console by console\n";break;
          case 4 : router.setMode(3);RouterConsole.output="";break;
          case 5 : router.setMode(2);RouterConsole.output="%SYS-5-CONFIG_I: Configured from console by console\n";break;
          default :RouterConsole.output="";
        }//end switch
    }
    //------------Mode 4---------------//
      ...
      if (router.getMode() == 4){
        ...
        RouterConsole.output="%LINEPROTO-5-UPDOWN: Line protocol on Interface "+inf.getType() + inf.getNameInt().charAt(inf.getNameInt().length()-1)+", changed state to up\n%LINK-3-UPDOWN: Interface "+inf.getType() + inf.getNameInt().charAt(inf.getNameInt().length()-1)+", changed state to up\n";
      }
      else {RouterConsole.output = error;}
    }
      ...
      if (router.getMode() == 4){
        Interface inf = (Interface)router.getVInt().lastElement();
        inf.shutInt(router);
        RouterConsole.output="%LINEPROTO-5-UPDOWN: Line protocol on Interface "+inf.getType() + inf.getNameInt().charAt(inf.getNameInt().length()-1)+", changed state to down\n%LINK-3-UPDOWN: Interface"+ inf.getType() + inf.getNameInt().charAt(inf.getNameInt().length()-1)+", changed state to administratively down\n";
      }
      else {RouterConsole.output = error;}
    }
    //-----------Mode < 3 -------------//
...
//        System.out.println("Start trace at "+ arrCMD[1] + "\n");
        RouterConsole.output = "Start trace at "+ arrCMD[1] + "\n";
        Command c = new Command();
        if (router.getFlag() != 'o'){
          c.trace(router,arrCMD[1]);
        }
        else {
          RouterConsole.output += c.trace_ospf(router,arrCMD[1]);
        }
      }
      else{RouterConsole.output = error;}
    }
      ...
      if (router.getMode() < 3){
        ...
        if (router.getFlag() != 'o'){
          ...
          RouterConsole.output += c.ping_ospf(router,arrCMD[1]);
        }
      }
      else{RouterConsole.output = error;}
    }
...
//        c
    ...
    //--------Mode  >1 & < 3 -------------//
      ...
      if (router.getMode() == 2){
        RouterConsole.output ="";
        RouterConsole.output += "Neighbor\tID\tPri\tState\tDead Time\tAddress\tInterface\n";
        Command c = new Command();
        RouterConsole.output += c.show_ip_ospf_neighbor(router);
      }
      else {RouterConsole.output=error;}
    }
      ...
      if (router.getMode() == 2){
        if (arrCMD[4].length() != 0){
          RouterConsole.output = "";
          Command c = new Command();
          RouterConsole.output += c.show_ip_ospf_inf(router,arrCMD[4]);
        }
      }
    }
    else {RouterConsole.output=error;}
  ...
  public void setIpAddress(Router r,String ip,String subnet){
    ...
    if (dup){
      RouterConsole.output += "Allready " + ip + "\n";
    }//end ip
    else{
  ...
  public void setIpRoute(Router r,String des,String subnet,String gw){
    if ((r.getFlag() == 'c')||(r.getFlag() == 's')){
      ...
      RouterConsole.output += "You have another protocol\n";
    }
  }
  //----------Command no ip route--------------//
  public void setNoIpRoute(Router r,String des,String subnet,String gw){
    if(r.getFlag() == 's'){
      ...
      RouterConsole.output += "You don't have static route\n";
    }
  }
  //-------------Command Hostname-----------//
  public void setHostname(Router r,String name){
    ...
    if (dup){
      RouterConsole.output = RouterConsole.output + "Already " + name +"\n";
    }//end if
    else {
      r.setName(name);
      Frame1.setTab(name);
...
//      ripf.start();
      ...
      RouterConsole.output += "You have already static route\n";
    }
  }
  ...
  public void setNoRouterRip(Router r){
    if (r.getFlag() == 'r'){
      ...
      RouterConsole.output += "You don't have Protocol RIP\n";
    }
  }
  ...
  public void setNoRouterOspf(Router r){
    if (r.getFlag() == 'o'){
      ...
      RouterConsole.output += "You don't have Protocol OSPF\n";
    }
  }
  //--------Set router OSPF---------------//
  ...
  public void pingcmd(Router router,String arg){
    ...
    if (router.getFlag() != 'o'){
      ...
      RouterConsole.output += c.ping_ospf(router,arg);
    }
  }
  public void traceroutecmd(Router router,String arg){
    RouterConsole.output += "Start trace at " + arg + "\n";;
    Command c = new Command();
    if (router.getFlag() != 'o'){
      c.trace(router,arg);
    }
    else {
      RouterConsole.output += c.trace_ospf(router,arg);
    }
  }
        ...
        for (int j=0; j < router.getVInt().size(); j++){
          ...
          if (ip.equalsIgnoreCase(intf.getIpAddress())){
            ...
            if (router.getEnableTelnet()){
              int a =0;
              while ((router.getName().equalsIgnoreCase(rconsole.getRouter(a).getName()))&&(a < rconsole.getRouters().size())){
                a++;
              }//end while
...
*/
  ...
  public void exitcmd(Router router,RouterConsole rconsole){
    System.out.println(rconsole.getRouters().size());
    if (rconsole.getRouters().size() == 1){
      switch (router.getMode()){
        case 1 : router.setMode(0);RouterConsole.output+="reset";break;
        case 2 : RouterConsole.output += error;
        case 3 : router.setMode(2);RouterConsole.output+="%SYS-5-CONFIG_I: Configured from console by console\n";break;
        case 4 : router.setMode(3);RouterConsole.output+="";break;
        case 5 : router.setMode(2);RouterConsole.output+="%SYS-5-CONFIG_I: Configured from console by console\n";break;
        case 6 : router.setMode(2);router.setLineMode(-1);RouterConsole.output+="%SYS-5-CONFIG_I: Configured from console by console\n";break;
  //      default :RouterConsole.output="";
      }//end switch
    }
    else{
...
//    RouterConsole.output += "Configuring from terminal, memory, or network[terminal]?";
  ...
  public void showiproutecmd(Router router){
    ...
    if (router.getFlag() != 'o'){
      ...
      RouterConsole.output += c.show_ip_route_ospf(router,true);
    }
  }
  public void showiproutecmd(Router router,String protocol){
    ...
    if (protocol.equalsIgnoreCase("static")){
      ...
      c.showIpRoute(router,'r');
            ...
            c.show_ip_route_ospf(router,false);
    }
  }
...
/*  Comment out if not run
          System.out.println("//---------------//");
          for (int a=0; a<Frame1.vRouter.size(); a++){
            Router r = (Router)Frame1.vRouter.elementAt(a);
            if (r.getFlag() == 'o'){
              Ospf ospff = (Ospf)r.getOSPF();
              System.out.println(r.getName()+" is "+ ripff.isAlive());
              if(ospff.isAlive()){
                ospff.run();
                System.out.println(r.getName()+" run single mode");
              }
              else{
                ospff.start();
                System.out.println(r.getName()+" start single mode");
              }
            }//flag r
            else {
              System.out.println(r.getName()+ " is "+r.getFlag());
            }
          }//router
*/
        }//sigle mode
        ...
        RouterConsole.output += "You have already other protocol\n";
...
//        ospff.stop();
  ...
  public void ipaddcmd(Router router,String ip,String subnet){
    ...
    if (dup){
      RouterConsole.output = RouterConsole.output + "Allready " + ip + "\n";
    }//end ip
    else{
      try{
        Interface i1 = (Interface)router.getVInt().lastElement();
        ...
        RouterConsole.output += "Can't add ip address\n";
      }
    }
  }
  ...
  public void showospfint(Router router,String type){
    Command c = new Command();
    RouterConsole.output += c.show_ip_ospf_inf(router,type);
  }
