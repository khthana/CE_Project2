...
public class ResultInsertPublisher extends HttpServlet {

	
	private String dbTable = "PUBLISHER" ;
	...
	{
			String htmlOutput;
			String title = CharsetConverter.MS874ToUnicode("ผลการป้อนข้อมูลมหาวิทยาลัย");
			...
			//htmlOutput +="Test First Page for add Publisher to DB88";

			if( !(connectDB()) ){
					//htmlOutput += "connect DB Success";
					if( !(addRecord(request,response)) ){
							    htmlOutput += "<br><br>";
								htmlOutput +="<center><font size=\"4\" face=\"Browallia New, BrowalliaUPC\">ชื่อมหาวิทยาลัย</font></center><br>";
								htmlOutput += "<center><font size=\"4\" face=\"Browallia New, BrowalliaUPC\">"+CharsetConverter.MS874ToUnicode(request.getParameter( "bub_thai"))+"</font></center>\n";
								htmlOutput += "<center><font size=\"4\" face=\"Browallia New, BrowalliaUPC\">"+CharsetConverter.MS874ToUnicode(request.getParameter( "pub_eng"))+"</font></center><br><br>";
	...
	{		
		...
		try 
		{
	         PreparedStatement pstmt = dbConnection.prepareStatement("INSERT INTO "+dbTable+
				 " (PUBLISHER_THAI_NAME, PUBLISHER_ENGLISH_NAME)"+				 
				"VALUES (?,?)");
         //  provide values for prepared statement and execute update 
	        			
			 pstmt.setString(1,CharsetConverter.MS874ToUnicode( request.getParameter( "bub_thai" ))  );
			 pstmt.setString(2,CharsetConverter.MS874ToUnicode( request.getParameter( "pub_eng" )) );
