...
function cmdOK() {
	...
	window.close()
	OldWhereValue = window.opener.document.queryForm.elements("where").value;
	...
	{	if (OldWhereValue.indexOf("["+dimension+"]") != -1)
		...
		else window.opener.document.queryForm.elements("where").value = OldWhereValue + "," + selectedMember.id ;
	}
	else window.opener.document.queryForm.elements("where").value = selectedMember.id
