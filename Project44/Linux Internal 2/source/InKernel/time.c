...
asmlinkage void sys_newcall(void) /* Test no receive no return */
{
	printk("<0>Linux Kernel Man! \n");
}

asmlinkage void sys_newcallreceive(char * fd) /*Test receive no return*/
{
	printk("<0>Linux Internal Group : %s \n",fd);
}

asmlinkage int sys_newcallreturn(void) /*Test return Only*/
{
        return current->pid;
}
