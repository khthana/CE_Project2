...
public class Frame2 extends JFrame {
  ...
  private void jbInit() throws Exception  {
    ...
    this.setIconImage(Toolkit.getDefaultToolkit().getImage(WapBrowser.Frame2.class.getResource("editor.jpg")));
    ...
    jButtonCompile.setAlignmentX((float) 9999.0);
    jButtonCompile.setBorder(titledBorder1);
    jButtonCompile.setContentAreaFilled(false);
    jButtonCompile.setIcon(new ImageIcon(WapBrowser.Frame2.class.getResource("compile.gif")));
    jButtonCompile.setText("Display");
...
////////////////////Open file/////////////////////
  void jMenuOpen_actionPerformed(ActionEvent e) {
    ...
    if (!jFileChooser1.getSelectedFile().getPath().equalsIgnoreCase("c:/tem.wml")){Browser.InputArea.setText(jFileChooser1.getSelectedFile().getPath());}
  ...
  boolean saveFile() {
    ...
    boolean okToAbandon() {
      ...
      int value = JOptionPane.showConfirmDialog(this,"Save changes?","แว๊พบราวเซอร์ภาษาไทย 2",JOptionPane.YES_NO_CANCEL_OPTION);
      switch(value) {
  ...
  String AddTTag(String src){//----------------begin AddTTag
    int hh=1;
    if(!src.equals("")){
      while((src.substring(hh-1,hh).equalsIgnoreCase("\n"))||
      (src.substring(hh-1,hh).equalsIgnoreCase("\t"))||
      (src.substring(hh-1,hh).equalsIgnoreCase(" "))){
        if(hh<src.length()){hh++;}else{break;}
      }// if ws
    }
    // end when not found ws
    ...
    while(ii<=src.length()){
      ...
      if(state==0){
        kk=ii;
        if (src.substring(kk-3,kk).equalsIgnoreCase("<p>") ) {
          state=1;
          if (ii<src.length()){ii=kk+1;}else{break;}
        }
      }
      if(state==0){
        if (src.substring(ii-3,ii).equalsIgnoreCase("<p ") ) {
          if (ii<src.length()){ii+=1;}else{break;}
          while (!src.substring(ii-1,ii).equalsIgnoreCase(">")){
            if (ii<src.length()){ii++;}else{break;}
          }
          if (src.substring(ii-1,ii).equalsIgnoreCase(">")){
            state=1;// 0-1
            if (ii<src.length()){ii+=1;}else{break;}
          }
        }
      ...
      kk=ii;
      //  if((state==1)&&(src.substring(kk-1,kk).equalsIgnoreCase("<"))){
//      if(state==1){
//        int ll=kk;
//        if (ll+4<src.length()){ll+=5;}else{break;}
//        while(!src.substring(ll-5,ll).equalsIgnoreCase("<br/>")){
//          if (ll<src.length()){ll++;}else{break;}
//        }
//        if (src.substring(ll-5,ll).equalsIgnoreCase("<br/>")){
//          if (kk+4<src.length()){kk+=5;}else{break;}
//        }
//      }
      if((state==1)&&(src.substring(kk-1,kk).equalsIgnoreCase("<"))){
        //if (kk+1<src.length()){kk+=2;}else{break;}  //why
        if (kk<src.length()){kk+=1;}else{break;}
        while((!src.substring(kk-2,kk).equalsIgnoreCase("</"))&&(!src.substring(kk-2,kk).equalsIgnoreCase("/>"))){
          if (kk<src.length()){kk++;}else{break;}
        }
        if (src.substring(kk-2,kk).equalsIgnoreCase("</")){
          //state=2;  //if last 2 chars is "/>", 4-2 by <.../>
          cover = false;
        }
        else{
          while(!src.substring(kk-2,kk).equalsIgnoreCase("/>")){
            if (kk<src.length()){kk++;}else{break;}
          }
          if (src.substring(kk-2,kk).equalsIgnoreCase("/>")){
            state=2;  //if last 2 chars is "/>", 4-2 by <.../>
      ...
      if((state==1)&&(src.substring(ii-1,ii).equalsIgnoreCase("<"))) {//<p><blabla..>..  //1-3
      state=3;
        //if (ii+2<src.length()){ii+=3;}else{break;}//not need bcuz check in 3-0
      }
      else if((state==1)&&((src.substring(ii-1,ii).equalsIgnoreCase("\n"))||
      (src.substring(ii-1,ii).equalsIgnoreCase("\t"))||
      (src.substring(ii-1,ii).equalsIgnoreCase(" ")))){   //1-4
        state=4;
        //if (ii<src.length()){ii+=1;}else{break;}  // not need bcuz loop get ws in 4-4
      }
      else if (state==1){
        state=2;
        //ii-=1;// to the char of text next to ">" //not need bcuz curr index point to char next to ">"
      }
      if(state==4){
        while((src.substring(ii-1,ii).equalsIgnoreCase("\n"))||
        (src.substring(ii-1,ii).equalsIgnoreCase("\t"))||
        ...
        // end when not found ws
        kk=ii;
        if(src.substring(kk-1,kk).equalsIgnoreCase("<")){
          if (kk+1<src.length()){kk+=2;}else{break;}
          while((!src.substring(kk-2,kk).equalsIgnoreCase("</"))&&(!src.substring(kk-2,kk).equalsIgnoreCase("/>"))){
            if (kk<src.length()){kk++;}else{break;}
          }
          if (src.substring(kk-2,kk).equalsIgnoreCase("</")){
            //state=2;  //if last 2 chars is "/>", 1-2 by <.../>
            cover = false;
          }
          else{
            while(!src.substring(kk-2,kk).equalsIgnoreCase("/>")){
      ...
      if((state==4)&&(src.substring(ii-1,ii).equalsIgnoreCase("<"))){
        state=3;
        //if (ii+2<src.length()){ii+=3;}else{break;}  // will check in 3-0
      }
      ...
      if(state==2){
        ...
        sbuf.insert(ii,"<t>");
        src = sbuf.toString();
        if (ii+3<src.length()){ii+=4;}else{break;}
        if (ii<src.length()){ii+=1;}else{break;}
      }
      if((state==5)&&(cover)){
        while(!src.substring(ii-2,ii).equalsIgnoreCase("/>")){
          if (ii<src.length()){ii++;}else{break;}
        }
        if (src.substring(ii-2,ii).equalsIgnoreCase("/>")){
          if (ii<src.length()){ii++;}else{break;}
          cover = false;
          state=1;
          ii-=1;change=true;
          sbuf.insert(ii,"</t>");
          src = sbuf.toString();
          //if (ii+7<src.length()){ii+=8;}else{break;}
          if (ii+4<src.length()){ii+=5;}else{break;}
          //inp=true;
        // until found new text
        }

      }
      else if(state==5){
        while(!src.substring(ii-1,ii).equalsIgnoreCase("<")){//suppose that after cover,begin of tag (<) will appear
          if (ii<src.length()){ii++;}else{break;}
        }
        state=3;
        ii-=1;change=true;
      ...
      if(state==3){
        if (ii+2<src.length()){ii+=3;}else{break;}
        if ((state==3)&&(src.substring(ii-4,ii).equalsIgnoreCase("</p>"))){
          state=0;
        }
        else if (state==3){//havetag another tag
          if (cover){//will ignore by 5-1
            state = 1;
            ii-=3;change=true;  // to</t>x^
            cover = false;
          }
          else{
            while(!src.substring(ii-4,ii-2).equalsIgnoreCase("</")){
              if (ii<src.length()){ii++;}else{break;}
            }
            if(src.substring(ii-4,ii-2).equalsIgnoreCase("</")){
              state=8;//if found "</xx^...>"
              ii-=1;change = true;  // to the "</x^...>"
            }
          }
        }
      }
      ...
      //if ii not increase as for loop
      if ((ii==jj)&&(!change)){if ( ii<src.length() ) {ii++;}else{break;}}
      change = false;
    }
    //end paste
...
//-------------
  ...
  void jButtonCompile_actionPerformed(ActionEvent e) {
      try{
          ...
          String tempf = jTextPane1.getText();
          tempf = AddTTag(tempf);
     ...
     if(okToAbandon()) {
      Browser.enter_press("file:///c:/tem.wml");
     };
  }
