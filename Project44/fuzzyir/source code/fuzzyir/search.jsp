...
//function
	...
	public String rmvstr(String str,String pattern,String np){
		...
		while(i<str.length()){
			if (str.charAt(i)==pattern.charAt(j)){
				max--; j++;
				if (max==0){
					str=str.substring(0,oldd+1)+np+str.substring(i+1,str.length());
					j=0;
				}
			}
			else {
...
<jsp:useBean id="op1" class="Text.TextOp" /> <%
	...
	Vector v_q_and=new Vector();
	Vector v_q_or=new Vector();
	Vector v_q_not=new Vector();

	Vector vector_query_and = new Vector();		//ประกาศใช้ object vector
...
<form name="form1" method="get" action="search.jsp">
  <div align="center">
	...
	<% }else {%>
		<%if (from_page.equals("adv_search")){//แก้ให้หน่อยมันมีเครื่องหมายคำพูดเลยมาไม่ครบ
		%>
		    <input type="text" name="str" size="75" maxlength="100" >
    <%}else{
    			mst_t=rmd(mst_and);   //ตรงนี้มีปัญหา
	%> 
	<input type="text" name="str" size="75" maxlength="100" value="<%=mst_t%>">
    <%}%> <% } %> 
  ...
  <P>
  <div align="center">
   <a href="adv_search.jsp"><font size="-2"><u>Advance search</u></font> </a>
   </div>
</form>
<p><b><br>
  <%	if (mst_and!=null && mst_and.length()>0) { 
		...
		while (str2<v.size()) {
			...
			if(wd2.length()!=0){
				...
				v_q_and.addElement(wd2);
			}
			str2++;
		}
		String mst_temp = new String();
		//out.println(mst_and);
		//mst_and=rmvstr(mst_and,"&quot;","\"");
		//out.println(mst_and);
		mst_temp=cutt(mst_and,'"');

%> <%	if (mst_temp!=null && mst_temp.length()>0) { %> <%		
		String wd;
		int str=0;
%> <%
		...
		if (swv!=null) {
			 while((wd=op1.GWord(mst_temp,str))!=null) {	 //wd is text not porter
				...
				if (op1.ChkSW(wd,swv)==false) { 
					vector_query_and.addElement(op1.Porter(wd));
					v_q_and.addElement(wd);
				%> <%
				}
			} 
		}
...
//**************** AND *****************************
	if(vector_query_and.size()!=0){
		Vector vector_relate_and = new Vector();			//Pnumber
		Vector vector_relate_co_and = new Vector();			//input count  
		Vector vector_relate_query_and = new Vector();		//input keyword
		Vector vector_ton_and =new Vector();
		int ns=vector_query_and.size();
		for (int i=0;i<ns;i++){
			vector_ton_and.addElement(new Double(0));
		}
		for (int i=0;i<ns;i++){
			Vector v_tmp=new Vector();
			String query=(String)v_q_and.elementAt(i);
			String sql1="select TENGLISH from Dict where EWORD='"+query+"'";
			String templar;
			ResultSet resultset_sql1 = statement1.executeQuery(sql1);
			if(resultset_sql1.next()){
				v_tmp=cutter(resultset_sql1.getString("TENGLISH"),',');
				if (v_tmp!=null) {
					for (int j=0;j<v_tmp.size();j++){
						templar=(String)v_tmp.elementAt(j);
						if (templar.length()>0 && templar.compareTo(query)!=0 && templar!=null) {
							vector_query_and.addElement(op1.Porter(templar));
							vector_ton_and.addElement(new Double(1.0-t_rate));
						}
					}
				}
			}
		...
		for (int i=0;i<vector_query_and.size();i++){
			...
			while(resultset_sql1.next()){
				...
				vector_relate_co_and.addElement((Double) vector_ton_and.elementAt(i));
			}
			resultset_sql1.close();
			...
			while(resultset_sql1.next()){
				...
				Double d_tmp=(Double)vector_ton_and.elementAt(i);
				double co_relate=resultset_sql1.getDouble("Relate")*(1.0-d_tmp.doubleValue());
				...
				while(resultset_sql2.next()){
					...
					while((boo==1)&&(j<vector_relate_and.size())){
						if((vector_relate_and.elementAt(j).equals(p_num)) && (vector_relate_query_and.elementAt(j).equals(query))){		//มีข้อมูลอยู่แล้ว
							boo=0;
...
//*********** OR ***************************************
	if(vector_query_or.size()!=0){
		...
		Vector vector_ton_or =new Vector();
		int ns=vector_query_or.size();
		for (int i=0;i<ns;i++){
			vector_ton_or.addElement(new Double(0));
		}
		for (int i=0;i<ns;i++){
			Vector v_tmp=new Vector();
			String query=(String)v_q_or.elementAt(i);
			String sql1="select TENGLISH from Dict where EWORD='"+query+"'";
			String templar;
			ResultSet resultset_sql1 = statement1.executeQuery(sql1);
			if(resultset_sql1.next()){
				...
				if (v_tmp!=null) {
					for (int j=0;j<v_tmp.size();j++){
						templar=(String)v_tmp.elementAt(j);
						if (templar.length()>0 && templar.compareTo(query)!=0 && templar!=null) {
							vector_query_or.addElement(op1.Porter(templar));
							vector_ton_or.addElement(new Double(1.0-t_rate));
						}
					}
				}
			}
		...
		for (int i=0;i<vector_query_or.size();i++){
			...
			while(resultset_sql1.next()){
				...
				vector_relate_co_or.addElement((Double)vector_ton_or.elementAt(i));
			}
			resultset_sql1.close();
			statement1.close();
			...
			while(resultset_sql1.next()){
				...
				//out.println(st_relate);
				Double d_tmp=(Double)vector_ton_or.elementAt(i);
				double co_relate=resultset_sql1.getDouble("Relate")*(1.0-d_tmp.doubleValue());;
				...
				while(resultset_sql2.next()){
					...
					while((boo==1)&&(j<vector_relate_or.size())){
						if((vector_relate_or.elementAt(j).equals(p_num)) && (vector_relate_query_or.elementAt(j).equals(query))){		//มีข้อมูลอยู่แล้ว
							boo=0;
			...
			while(resultset_sql1.next()){
				...
				while(resultset_sql2.next()){
					...
					while((boo==1)&&(j<vector_relate_not.size())){
						if((vector_relate_not.elementAt(j).equals(p_num)) && (vector_relate_query_not.elementAt(j).equals(query))){		//มีข้อมูลอยู่แล้ว
							boo=0;
	...
	if(operation.length()!=0){
		if(operation.equals("and")){
			...
			for(int i = 0;i<segma.getVectorSize();i++){
				...
				double  tmp2 = tmp.doubleValue()*100;
		...
		for(int i=0;i<vector_fuzzy_and.size();i++){
			...
			double tmp3=(tmp1.doubleValue()*tmp2.intValue()*100)/(query_amount_and);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><font size="-1">Search : <%
		for(int i=0;i<v_q_and.size();i++){
			out.print((String)v_q_and.elementAt(i)+" ");
		}
		if(operation!=null&&operation.length()>0){
			out.print(operation.toUpperCase()+"( ");
		}
		for(int i=0;i<v_q_or.size();i++){
			out.print(" OR "+(String)v_q_or.elementAt(i));
		}
		if(operation!=null&&operation.length()>0){
			out.print(" ) ");
		}
		for(int i=0;i<v_q_not.size();i++){
			out.print(" NOT "+(String)v_q_not.elementAt(i));
		}
	%></font></td>
  </tr>
</table><p>
<%if(segma.getVectorSize()>0){%> 
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr> 
    <td><font size="-1" color="#3333FF">Result : <%out.println(segma.getVectorSize()+" Record(s)");%></font></td>
    <td> 
      <div align="right"><font size="-1" color="#3333FF"><%long time=x.getTime();
	x=new java.util.Date();
	time=x.getTime()-time;%> Time is : <%=time%> ms </font></div>
    </td>
  </tr>
</table>
<font size="-1" color="#3333FF"> <br>
</font> <%for(int i=0;i<segma.getVectorSize();i++){
			statement1 = dbconn.createStatement();
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
    <td><b>Title : </b> <%out.println(resultset_show.getString("PName"));%></td>
  </tr>
  <tr>
	<%  String index=resultset_show.getString("indexref");
		String ttt;
		Vector vind=new Vector();
		Vector vec_index = new Vector();
		vec_index=cutter2(index,'"');
		int tr2=0;
		while (tr2<vec_index.size()) {
			ttt=(String)vec_index.elementAt(tr2);
			if(ttt.length()!=0){
				vind.addElement(ttt);
			}
			tr2++;
		...
		index=cutt(index,'"');
		if (swv!=null) {
			 while((ttt=op1.GWord(index,tr2))!=null) {	 //wd is text not porter
				tr2=op1.getNxt();
				if (op1.ChkSW(ttt,swv)==false) { 
					vind.addElement(ttt);
				%> <%
				}
			} 
	...
	%>
	<td><b>Keyword : </b><font size="-1" > <%
			for (int ij=0;ij<vind.size();ij++){
				if (ij==vind.size()-1) {
					out.print((String)vind.elementAt(ij));
				}
				else {
					out.print((String)vind.elementAt(ij)+" , ");
				}
	...
	<tr> 
    <td><b>Abstract : </b><font size="-1" > <%out.println(resultset_show.getString("Description"));%></font></td>
  </tr>
  <tr> 
    <td><b>Author : </b><font size="-1" ><%String sqlauthor="select * from AUTHOR where PNumber='"+segma.getVectorSegma(i)+"'";
		Statement statement2 = dbconn.createStatement();
		...
		resultset_author.close();
		statement2.close();
%></font></td>
  </tr>
  <tr> 
    <td><a href=../upload/<%=resultset_show.getString("Location")%>><font size="-1">Download</font></a><font size="-1"><%out.println("  size : "+resultset_show.getString("FileSize"));%></font></td>
  </tr>
  <tr> 
