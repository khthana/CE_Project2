//----------------------------------------------------------------------------------------
// Tony.cpp: implementation of the CTony class.
//----------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------
// Including files.
//----------------------------------------------------------------------------------------
	   ...
	   m_oBullet1(BULLET1)
{
	...
	for( int i=0; i<TONY_NUM_ANIMATION; i++ ) 
	{
...
------------+-----------------------------------------------------------------------------
*/
BOOL CTony::SetAnimationID
( 
	TONY_ANIMATION	AnimID, 
	BOOL			bBlend, 
...
------------+-----------------------------------------------------------------------------
*/
BOOL CTony::LoadAnimModel()
{
	LoadModel( "Resource\\Model\\Tony\\tony.mas" );
	SetAnimationID( ANIM_Idle );
	SetAnimationID( ANIM_Idle, FALSE, 0.125f );
	m_iCurrentOp = OP_IDLE;

	SetPosition( 0, 0, 0);
	m_oBounding.LoadModel("Resource\\Model\\Tony\\tony_bounding.maf");
...
------------+-----------------------------------------------------------------------------
*/
BOOL CTony::ProcessAI()
{
	if( !m_iDeadBling )
		return TRUE;

	// Finding destonytion distance
	...
	if( IsDead()&&(m_iCurrentOp!=OP_DIE)&&(m_iCurrentOp!=OP_NULL) )
	{
		...
		CSoundContainer::s_oSound[SOUND_MAN_DIE00].Play();
	}
	else // Found destonytion change operation to action state
...
------------+-----------------------------------------------------------------------------
*/
BOOL CTony::Process( float fElapsedTime )
{
	if( !m_iDeadBling )
		return TRUE;
...
*/
				...
				case OP_PAIN:
				{
					...
					m_fWaitTime = (float)CMath::RandomizeInt(0,2);
					CSoundContainer::s_oSound[SOUND_MAN_PAIN00].Play();
...
//					CSoundContainer::s_oSound[SOUND_BACKGROUND00].Play();
		...
		if( TONY_ANIM_IS_MOVING[m_CurrentAnimationID] )
		{
...
*/				}
			...
			{
				// Check if reached destonytion point
...
------------+-----------------------------------------------------------------------------
*/
HRESULT CTony::Render( const D3DXMATRIX* matCustom )
{
	
//	glDisable( GL_STENCIL_TEST );
//	CTony::SetScale( 0.50f, 0.5f, 0.50f );
...
HRESULT CTony::ProcessKey( FLOAT fElapsedTime, RECT rcScreen, BOOL* m_arKey )
{
	// Pick object process
