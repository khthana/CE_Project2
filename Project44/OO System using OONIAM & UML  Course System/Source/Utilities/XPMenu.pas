...
type
  ...
  protected
    ...
    procedure MyPageDrawButton(Sender: TToolBar;
      Button: TToolButton; State: TCustomDrawState; var DefaultDraw: Boolean);
...
begin
  ...
  FFont.Name := 'Microsoft Sans Serif';
  FFont.Charset := THAI_CHARSET;
  FForm := TForm(Owner);
...
begin
  ...
  begin
    ...
    if FForm.Components[i] is TToolBar
    and not (FForm.Components[i] is TMyTool)
    then
    ...
    if FForm.Components[i] is TMyTool then
      if not (csDesigning in ComponentState) then
      begin
        TMyTool(FForm.Components[i]).ShowCaptions := false;
        if not TMyTool(FForm.Components[i]).Flat then
          TMyTool(FForm.Components[i]).Flat := true;

{        if Enable then
        begin
          //for x := 0 to TMyTool(FForm.Components[i]).ButtonCount - 1 do
            if (not assigned(TMyTool(FForm.Components[i]).OnCustomDrawButton))
              or (FOverrideOwnerDraw) then
            begin
              TMyTool(FForm.Components[i]).OnCustomDrawButton :=
                MyPageDrawButton;
        ...
        begin
          if addr(TMyTool(FForm.Components[i]).OnCustomDrawButton) =
            addr(TXPMenu.MyPageDrawButton) then
            TMyTool(FForm.Components[i]).OnCustomDrawButton := nil;
...
var
  ...
  var
    ...
    begin
      ACanvas.Pen.color := FFSelectBorderColor;
      ACanvas.RoundRect(WRect.Left,WRect.Top,WRect.Right,WRect.Bottom,5,5);
// edit by arth
...
begin
  ...
  //
  if TToolBar(Button.Parent).Align = alRight then
  ...
  if HasBkg then
//    ACanvas.FillRect(ARect); edit by arth
...
//-----------
  ...
  begin
    ...
    else if(TToolBar(Button.Parent).List)  //?
    and (Button.ImageIndex = -1) then
    begin
      TextFormat := DT_Center;
      //ARect.Top := ARect.Bottom - ACanvas.TextHeight(Button.Caption) - 3;
    end
...
begin
  ...
  ACanvas.Font := FFont;
  ACanvas.Font.Charset := THAI_CHARSET;
...
procedure TXPMenu.MyPageDrawButton(Sender: TToolBar; Button: TToolButton;
...
//-----------
  if not TToolBar(Button.Parent).ShowCaptions then
  begin
...
//22
    try
    if Button.Index < TMyTool(Button.Parent).Captions.Count then
    DrawTheText(TMyTool(Button.Parent).Captions[Button.Index], '',
      ACanvas, ARect,
      (cdsSelected in State), Button.Enabled, false,
      (Button.MenuItem <> nil),
      (Button.BidiMode = bdRightToLeft), TextFormat)
{    else
    DrawTheText(IntToStr(Button.Index), '',
      ACanvas, ARect,
      (cdsSelected in State), Button.Enabled, false,
      (Button.MenuItem <> nil),
      (Button.BidiMode = bdRightToLeft), TextFormat);
