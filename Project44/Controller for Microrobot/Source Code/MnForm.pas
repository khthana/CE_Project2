...
procedure TDlForm.SettingsToolButtonClick(Sender: TObject);
...
// Called when the message queue gets empty.
procedure TDlForm.IdleProc( Sender: TObject; var Done: boolean );
...
// Updates the panels on bottom of this window.
procedure TDlForm.UpdateStatusPanels;
...
// Connect
procedure TDlForm.ConnectToolButtonClick(Sender: TObject);
begin
...
// Disconnect
procedure TDlForm.DisconnectToolButtonClick(Sender: TObject);
begin
...
procedure TDlForm.OutgoingRichEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if not cpDrv.Connected then
  begin
...
procedure TDlForm.OT_ClearCmdClick(Sender: TObject);
begin
...
// Displays an error box informing the user we can't send data
procedure TDlForm.CannotSendError;
begin
...
procedure TDlForm.OutgoingRichEditKeyPress(Sender: TObject;
  var Key: Char);
begin
  // Do nothing if not connected
  if not cpDrv.Connected then
...
procedure TDlForm.cpDrvReceiveData(Sender: TObject; DataPtr: Pointer;
  DataSize: Cardinal);
...
procedure TDlForm.IT_ClearCmdClick(Sender: TObject);
begin
  IncomingRichEdit.Lines.BeginUpdate;
...
// Quits TTY
procedure TDlForm.QuitTTYToolButtonClick(Sender: TObject);
begin
  PostQuitMessage( Handle );
end;

procedure TDlForm.HelpAboutCmdClick(Sender: TObject);
...
procedure TDlForm.OT_CutCmdClick(Sender: TObject);
begin
  Memo1.CutToClipboard;
end;

procedure TDlForm.OT_CopyCmdClick(Sender: TObject);
begin
  Memo1.CopyToClipboard;
end;

procedure TDlForm.OT_PasteCmdClick(Sender: TObject);
var clp: TClipboard;
    s, ss: string;
    iLastLine, i: integer;
begin
...
procedure TDlForm.IT_CopyCmdClick(Sender: TObject);
begin
  IncomingRichEdit.CopyToClipboard;
end;

procedure TDlForm.OT_PopupMenuPopup(Sender: TObject);
begin
...
//      memo1.Lines.Add(line_num+'    '+temp);
      ...
      num := num+'Z';
      DlForm.memo1.Lines.Add(num);
      closefile(f);
    // DlForm.Memo1.Lines.LoadFromFile(OpenDialog1.Filename);
    // DlForm.Memo1.Lines.Add('Z')
       { AssignFile(Hex_file,Opendialog1.Filename);
        Reset(Hex_file,1);

        BlockRead(Hex_file,Buf,1,NumRead);
        Memo1.Lines.LoadFromFile(buf);
        CloseFile(Hex_file);

     {   AssignFile(Buf_file,Opendialog1.Filename);
      Reset(Buf_file,1);
      blockread(buf_file,buf,1,NumRead);
      Memo1.Lines.LoadFromFile(buf);
      closefile(Buf_file);}
end;

procedure TDlForm.Button2Click(Sender: TObject);
