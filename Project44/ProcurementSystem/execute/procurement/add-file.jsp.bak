...
<%
    ...
    String paper_size=MS874ToUnicode(new String(request.getParameter("paper_size")));
	...
	java.sql.Time  time=new java.sql.Time(DateTime);
     ...
     // ส่วนจัดการ DB ของ Oracle
		 try {
				...
				ResultSet rs=stmt.executeQuery("SELECT * FROM FILING WHERE CODE='"+code+"'");
				...
				String query="INSERT INTO FILING(CAT#,CODE,TYPE,BRAND,PAPER_SIZE,THICK,COLOR,DETAIL) VALUES('"+cat+"','"+code+"','"+type+"','"+brand+"',' "+paper_size+"','"+thick+"','"+color+"','"+detail+"')";
	...
	sname=UnicodeToMS874(sname);*/
	%>
       <CENTER>เพิ่มข้อมุลเสร็จสิ้น <A HREF="add-file.html">เพิ่มเติมข้อมูล</A> หรือ <A HREF="show-file-db.jsp">ดูข้อมูลในฐานข้อมูล</A> </CENTER>
