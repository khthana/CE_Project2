...
//	String  workflow=MS874ToUnicode(new String(request.getParameter("workflow")));
	...
	if ( wf_name.length()==0 || app1.length()!=8 || ( app2.length()>0 && app2.length()!=8) || (viewer.length()>0 && viewer.length()!=8)  ){
	 hasExplicit=false;
	}else { hasExplicit=true;}	
	
	
	 if(!hasExplicit){
    out.println("<b><div align=\"center\">ยังกรอกข้อความไม่ครบถ้วน หรือกรอกข้อมูลผิดพลาด กรุณาย้อนกลับไป 1 หน้าเพื่อทำการแก้ไข </div></b>");
	}else{
         %>
<BODY>
<FORM method="post" action="wf-edit3.jsp">
<TABLE cellpadding="3" bordercolor="#DF5F2D" border="1"  width="450" cellspacing="0">
<TR bgcolor="#DF572D" height="24">
	<TD colspan="2" align="left"><FONT COLOR="#FFF0E1"><B>แก้ไข Workflow</B></FONT></TD>
</TR>
<TR>
