...
<head>
<title>เพิ่มข้อมูลเสร็จสิ้น</title>
...
<%
	...
	java.sql.Time  time=new java.sql.Time(DateTime);
     ...
     // ส่วนจัดการ DB ของ Oracle
		 try {
				...
				ResultSet rs=stmt.executeQuery("SELECT * FROM PACKING_PRODUCTS WHERE CODE='"+code+"'");
			...
			/*name=UnicodeToMS874(name);
				...
				String query="INSERT INTO PACKING_PRODUCTS(CAT#,CODE,TYPE,BRAND,TAPE_SIZE,RING,THICK,COLOR,DETAIL) VALUES('"+cat+"','"+code+"','"+type+"','"+brand+"',' "+tape_size+"','"+ring+"','"+thick+"','"+color+"','"+detail+"')";
	...
	sname=UnicodeToMS874(sname);*/
	%>
       <CENTER>เพิ่มข้อมุลเสร็จสิ้น <A HREF="add-glue.html">เพิ่มเติมข้อมูล</A> หรือ <A HREF="show-glue-db.jsp">ดูข้อมูลในฐานข้อมูล</A> </CENTER>
