...
<%
	...
	java.sql.Time  time=new java.sql.Time(DateTime);
     ...
     // ส่วนจัดการ DB ของ Oracle
		 try {
				...
				ResultSet rs=stmt.executeQuery("SELECT * FROM WRITING_PRODUCTS WHERE CODE='"+code+"'");
				...
				String query="INSERT INTO WRITING_PRODUCTS(CAT#,CODE,TYPE,BRAND,COLOR,HEAD_SIZE,DETAIL) VALUES('"+cat+"','"+code+"','"+type+"','"+brand+"',' "+color+"','"+head_size+"','"+detail+"')";
	...
	sname=UnicodeToMS874(sname);*/
	%>
       <CENTER>เพิ่มข้อมุลเสร็จสิ้น <A HREF="add-pen.html">เพิ่มเติมข้อมูล</A> หรือ <A HREF="show-pen-db.jsp">ดูข้อมูลในฐานข้อมูล</A> </CENTER>
