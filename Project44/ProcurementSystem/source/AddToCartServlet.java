...
public class AddToCartServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
   ...
   if ( (getnum != null) && (getnum.trim().length() != 0) )
   {
	    getnum.trim();
	   num = Integer.parseInt(getnum);
   }	
  if ( (num==0) || (getnum==null)  || (getnum.trim().length()==0))
  {
			response.sendRedirect("../no_num.jsp");
  }else{
	// สร้าง session
	...
	ItemOrder itemOrder = new ItemOrder(code,type,cat,num,tableName);
