...
class Console:
	def __init__(self, styles=None, keymap=None, agent=None) :
		self.agent = agent
		if styles is None:
			styles = Styles()
			...
			styles.add('header', parent=basic, foreground=colors.green)
		self.styles = styles
	...
	def handleLine(self, text):
		...
		# Meng insert code around here
		if self.agent and self.agent.isConsoleToNameSpace :
			pt = PythonThread(code, self, self.agent.agentNameSpace)
		else :
			pt = PythonThread(code, self, self.locals)
...
class PythonThread(Thread):
	def __init__(self, code, console, nameSpace):
		self.code = code
		self.console = console
	...
	def run(self):
		try:
			exec self.code in self.nameSpace
