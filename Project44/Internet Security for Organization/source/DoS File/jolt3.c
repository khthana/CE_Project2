...
{
  fprintf (stderr, "Usage: %s [-s src_addr] [-p port] [-n number] dest_addr\n",
           pname);
...
int do_frags (int sck, u_long src_addr, u_long dst_addr, int port , u_long Loop)
{
  int     bs, psize;
  u_long  Counter;
  unsigned long x;
...
int main(int argc, char *argv[])
{
  ...
  while ((i = getopt (argc, argv, "s:p:n:h")) != EOF)
    {
      switch (i)
        {
        case 's':
  ...
  if (setsockopt(spf_sck, IPPROTO_IP, IP_HDRINCL, (char *)&bs,
    ...
    do_frags (spf_sck, src_addr, dst_addr, port , Loop);
